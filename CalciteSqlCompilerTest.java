...
public class CalciteSqlCompilerTest {
  ...
  public void testMaxSelectionBehavior() {
        Pql2Compiler.MAX_QUERY_SELECTION_LIMIT = 100000;
       ...
       pinotQuery =
                CalciteSqlParser.compileToPinotQuery("select * from vegetables LIMIT 1000001");
        Assert.assertEquals(pinotQuery.getLimit(), 100000);
        pinotQuery =
                CalciteSqlParser.compileToPinotQuery("select * from vegetables LIMIT 1000000");
        Assert.assertEquals(pinotQuery.getLimit(), 100000);
