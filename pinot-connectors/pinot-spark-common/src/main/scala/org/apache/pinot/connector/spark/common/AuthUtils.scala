...
private[pinot] object AuthUtils {
  ...
  /** Returns a token value with a single Bearer prefix if not already present. */
   ...
   * - If only token is provided, defaults to Authorization with Bearer prefix (idempotent)
   * - If only header is provided, returns None (callers may log a warning)
   * - If neither is provided, returns None
   */
  def buildAuthHeader(authHeader: Option[String], authToken: Option[String]): Option[(String, String)] = {
