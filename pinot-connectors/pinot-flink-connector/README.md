...
if (controllerUri.getPort() != -1) {
  controllerAddress += ":" + controllerUri.getPort();
}
...
try (PinotAdminClient client = new PinotAdminClient(controllerAddress, properties)) {
  ...
  TableConfig tableConfig = PinotConnectionUtils.getTableConfig(client, controllerUrl, "starbucksStores", TableType.OFFLINE);
...
try (PinotAdminClient client = new PinotAdminClient(controllerAddress, properties)) {
  ...
  Schema schema = PinotConnectionUtils.getSchema(client, "starbucksStores");
  // fetch Pinot table config
  TableConfig tableConfig = PinotConnectionUtils.getTableConfig(client, controllerUrl, "starbucksStores", TableType.REALTIME);
