...
public final class FlinkQuickStart {
  ...
  public static void main(String[] args)
    ...
    String controllerHost = controllerUri.getHost();
    if (controllerUri.getPort() != -1) {
      controllerHost += ":" + controllerUri.getPort();
    }
    Properties properties = new Properties();
    properties.setProperty(PinotAdminTransport.ADMIN_TRANSPORT_SCHEME, controllerUri.getScheme());

    try (PinotAdminClient client = new PinotAdminClient(controllerHost, properties)) {
      Schema schema = PinotConnectionUtils.getSchema(client, "starbucksStores");
      TableConfig tableConfig =
          PinotConnectionUtils.getTableConfig(client, DEFAULT_CONTROLLER_URL, "starbucksStores", TableType.OFFLINE);
