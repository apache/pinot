...
/**
 * Queries test for THETA_SKETCH queries.
 */
@SuppressWarnings("unchecked")
public class ThetaSketchQueriesTest extends BaseQueriesTest {
  private static final File INDEX_DIR = new File(FileUtils.getTempDirectory(), "ThetaSketchQueriesTest");
  ...
  public void testAggregationOnly() {
    // Theta-sketch
    {
      String query = "SELECT THETASKETCH(intSVColumn), THETASKETCH(longSVColumn), THETASKETCH(floatSVColumn), "
          + "THETASKETCH(doubleSVColumn), THETASKETCH(stringSVColumn), THETASKETCH(intMVColumn), "
          + "THETASKETCH(longMVColumn), THETASKETCH(floatMVColumn), THETASKETCH(doubleMVColumn), "
          + "THETASKETCH(stringMVColumn), THETASKETCH(bytesColumn) FROM testTable";
    ...
    // Raw theta-sketch
    {
      String query = "SELECT RAWTHETASKETCH(intSVColumn), RAWTHETASKETCH(longSVColumn), RAWTHETASKETCH(floatSVColumn), "
          + "RAWTHETASKETCH(doubleSVColumn), RAWTHETASKETCH(stringSVColumn), RAWTHETASKETCH(intMVColumn), "
          + "RAWTHETASKETCH(longMVColumn), RAWTHETASKETCH(floatMVColumn), RAWTHETASKETCH(doubleMVColumn), "
          + "RAWTHETASKETCH(stringMVColumn), RAWTHETASKETCH(bytesColumn) FROM testTable";
      ...
      assertEquals(aggregationResult.size(), 11);
      for (int i = 0; i < 11; i++) {
        Sketch sketch = (Sketch) aggregationResult.get(i);
        if (i < 5) {
          assertEquals(Math.round(sketch.getEstimate()), NUM_RECORDS);
        } else {
          assertEquals(Math.round(sketch.getEstimate()), 3 * NUM_RECORDS);
  ...
  public void testAggregationGroupBy() {
    // Theta-sketch
    {
      String baseQuery = "SELECT THETASKETCH(intSVColumn), THETASKETCH(longSVColumn), THETASKETCH(floatSVColumn), "
          + "THETASKETCH(doubleSVColumn), THETASKETCH(stringSVColumn), THETASKETCH(intMVColumn), "
          + "THETASKETCH(longMVColumn), THETASKETCH(floatMVColumn), THETASKETCH(doubleMVColumn), "
          + "THETASKETCH(stringMVColumn), THETASKETCH(bytesColumn) FROM testTable GROUP BY ";
    ...
    // Raw theta-sketch
    {
      String baseQuery = "SELECT RAWTHETASKETCH(intSVColumn), RAWTHETASKETCH(longSVColumn), "
          + "RAWTHETASKETCH(floatSVColumn), RAWTHETASKETCH(doubleSVColumn), RAWTHETASKETCH(stringSVColumn), "
          + "RAWTHETASKETCH(intMVColumn), RAWTHETASKETCH(longMVColumn), RAWTHETASKETCH(floatMVColumn), "
          + "RAWTHETASKETCH(doubleMVColumn), RAWTHETASKETCH(stringMVColumn), RAWTHETASKETCH(bytesColumn) "
      ...
      for (boolean groupBySV : new boolean[]{true, false}) {
        ...
        while (groupKeyIterator.hasNext()) {
          ...
          GroupKeyGenerator.GroupKey groupKey = groupKeyIterator.next();
          for (int i = 0; i < 6; i++) {
            Sketch sketch = (Sketch) aggregationGroupByResult.getResultForKey(groupKey, i);
            if (i < 5) {
              assertEquals(Math.round((sketch).getEstimate()), 1);
            } else {
              assertEquals(Math.round((sketch).getEstimate()), 3);
  ...
  public void testPostAggregation() {
    String query = "SELECT THETASKETCH(intSVColumn, '', "
        // [300, 500), [800, 900)
        ...
        // [825, 1000)
        + "'floatMVColumn >= 2825', "
        // Expected: [400, 500), [800, 825)
        + "'SET_DIFF(SET_INTERSECT($1, $2), $3)') FROM testTable";
    ...
    // Inter segments
    String[] expectedResults = new String[]{Integer.toString(125)};
    BrokerResponseNative brokerResponse = getBrokerResponseForPqlQuery(query);
    QueriesTestUtils
        .testInterSegmentAggregationResult(brokerResponse, 4 * NUM_RECORDS, 0, 4 * 7 * NUM_RECORDS, 4 * NUM_RECORDS,
            expectedResults);
  }
