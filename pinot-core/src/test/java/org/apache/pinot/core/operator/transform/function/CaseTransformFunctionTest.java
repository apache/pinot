...
public class CaseTransformFunctionTest extends BaseTransformFunctionTest {
  ...
  private void testCaseQueryWithIntResults(String predicate, int[] expectedValues) {
    ...
    Assert.assertEquals(transformFunction.getResultMetadata().getDataType(), DataType.INT);
      ...
      switch (column) {
        ...
        case BIGDECIMAL_SV_COLUMN:
          switch (type) {
            case EQUALS:
              result[i] =
                  (_bigDecimalSVValues[i].compareTo(_bigDecimalSVValues[INDEX_TO_COMPARE]) == 0) ? "aaa" : "bbb";
              break;
            case NOT_EQUALS:
              result[i] =
                  (_bigDecimalSVValues[i].compareTo(_bigDecimalSVValues[INDEX_TO_COMPARE]) != 0) ? "aaa" : "bbb";
