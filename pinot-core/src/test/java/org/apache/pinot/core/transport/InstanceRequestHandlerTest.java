...
public class InstanceRequestHandlerTest {
  ...
  public void testCancelQuery()
    ...
    InstanceRequestHandler handler =
        new InstanceRequestHandler("server01", config, queryScheduler, mock(ServerMetrics.class),
  ...
  private QueryScheduler createQueryScheduler(PinotConfiguration config, CountDownLatch queryFinishLatch) {
    ...
    return new QueryScheduler(config, "serverId", mock(QueryExecutor.class), new LongAccumulator(Long::max, 0),
        resourceManager) {
      @Override
