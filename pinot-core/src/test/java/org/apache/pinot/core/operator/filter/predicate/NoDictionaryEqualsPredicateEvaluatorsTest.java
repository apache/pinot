...
public class NoDictionaryEqualsPredicateEvaluatorsTest {
  ...
  public void testBigDecimalPredicateEvaluators() {
    ...
    BigDecimal[] randomBigDecimals = new BigDecimal[NUM_MULTI_VALUES];
    PredicateEvaluatorTestUtils.fillRandom(randomBigDecimals);
    randomBigDecimals[_random.nextInt(NUM_MULTI_VALUES)] = bigDecimalValue;

    Assert.assertTrue(eqPredicateEvaluator.applyMV(randomBigDecimals, NUM_MULTI_VALUES));
    Assert.assertFalse(neqPredicateEvaluator.applyMV(randomBigDecimals, NUM_MULTI_VALUES));
    ...
    for (int i = 0; i < 100; i++) {
      ...
      PredicateEvaluatorTestUtils.fillRandom(randomBigDecimals);
      Assert.assertEquals(eqPredicateEvaluator.applyMV(randomBigDecimals, NUM_MULTI_VALUES),
          ArrayUtils.contains(randomBigDecimals, bigDecimalValue));
      Assert.assertEquals(neqPredicateEvaluator.applyMV(randomBigDecimals, NUM_MULTI_VALUES),
          !ArrayUtils.contains(randomBigDecimals, bigDecimalValue));
    }
  }
