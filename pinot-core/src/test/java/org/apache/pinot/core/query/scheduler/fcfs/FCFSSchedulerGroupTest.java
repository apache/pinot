...
public class FCFSSchedulerGroupTest {
  ...
  public void testCompare() {
    ...
    SchedulerQueryContext firstRequest = TestHelper.createQueryRequest("groupOne", metrics, 2000);
    lhs.addLast(firstRequest);
    assertEquals(lhs.compareTo(rhs), 1);
    assertEquals(rhs.compareTo(lhs), -1);

    SchedulerQueryContext secondRequest = TestHelper.createQueryRequest("groupTwo", metrics, 3000);
