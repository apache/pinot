...
public class ExpressionTypeResolverTest {
  ...
  public void testUnsupportedExpressionFails(String expression) {
    ...
    // An invalid expression must not be mistaken for missing metadata, which would allow an unbound fallback.
    assertFalse(error instanceof TypeInferenceUnavailableException, expression);
  }

  @Test
  public void testUnknownFunctionsHaveNoSchemaOnlyType() {
    // A broker cannot see transforms registered only on servers, so these are left for the server to validate.
    for (String expression : List.of("notRegistered(stringValue)", "negate(intValue, longValue)")) {
  ...
  @Test
  public void testBuiltInVirtualColumnsResolveWithoutSchemaEntries() {
    // A server's table schema does not declare the built-in virtual columns that a broker's schema contains.
    ExpressionTypeResolver resolver = new ExpressionTypeResolver(SCHEMA);
    assertEquals(resolver.resolve(ExpressionContext.forIdentifier(BuiltInVirtualColumn.DOCID)), ColumnDataType.INT);
    assertEquals(resolver.resolve(ExpressionContext.forIdentifier(BuiltInVirtualColumn.SEGMENTNAME)),
        ColumnDataType.STRING);
    assertEquals(resolver.resolve(ExpressionContext.forIdentifier(BuiltInVirtualColumn.PARTITIONID)),
        ColumnDataType.STRING_ARRAY);
    assertEquals(resolver.resolve(ExpressionContext.forIdentifier(BuiltInVirtualColumn.CREATIONTIME)),
