...
public class StreamingSelectionOrderByOperatorTest {
   ...
   *
   * @param expectedMinBlocks the minimum number of non-null blocks the streaming operator must emit. Tail-mode cases
   *     that span multiple runs pass {@code >= 2} to prove the output is genuinely streamed; cases whose result fits in
   *     a single trimmed block pass {@code 1}.
   */
