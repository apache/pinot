...
public class ModuloTransformFunctionTest extends BaseTransformFunctionTest {
  ...
  public void testModuloTransformFunction() {
    ExpressionContext expression =
        RequestContextConvertUtils.getExpression(String.format("mod(%s,%s)", INT_SV_COLUMN, LONG_SV_COLUMN));
    ...
    expression = RequestContextConvertUtils.getExpression(String.format("mod(%s,%s)", LONG_SV_COLUMN, FLOAT_SV_COLUMN));
    ...
    expression =
        RequestContextConvertUtils.getExpression(String.format("mod(%s,%s)", FLOAT_SV_COLUMN, DOUBLE_SV_COLUMN));
    transformFunction = TransformFunctionFactory.get(expression, _dataSourceMap);
    Assert.assertTrue(transformFunction instanceof ModuloTransformFunction);
    for (int i = 0; i < NUM_ROWS; i++) {
    ...
    expression =
        RequestContextConvertUtils.getExpression(String.format("mod(%s,%s)", DOUBLE_SV_COLUMN, STRING_SV_COLUMN));
    transformFunction = TransformFunctionFactory.get(expression, _dataSourceMap);
    Assert.assertTrue(transformFunction instanceof ModuloTransformFunction);
    for (int i = 0; i < NUM_ROWS; i++) {
    ...
    expression = RequestContextConvertUtils.getExpression(String.format("mod(%s,%s)", STRING_SV_COLUMN, INT_SV_COLUMN));
    transformFunction = TransformFunctionFactory.get(expression, _dataSourceMap);
    Assert.assertTrue(transformFunction instanceof ModuloTransformFunction);
    for (int i = 0; i < NUM_ROWS; i++) {
