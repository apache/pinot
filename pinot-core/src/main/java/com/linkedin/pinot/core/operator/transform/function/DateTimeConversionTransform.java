 ...
 *     <li> 2. format(as defined in {@link DateTimeFieldSpec}) of the input column. eg: 1:HOURS:EPOCH, 1:DAYS:SIMPLE_DATE_FORMAT:yyyyMMdd </li>
 *     <li> 3. format(as defined in {@link DateTimeFieldSpec}) of the output expected. eg: 1:MILLISECONDS:EPOCH, 1:WEEKS:EPOCH, 1:DAYS:SIMPLE_DATE_FORMAT:yyyyMMdd </li>
 ...
 *   <ul>
 *     <li> if Date column is expressed in millis, and output is expected in millis but bucketed to 15 minutes, </li>
 *     <li> dateTimeConvert(Date, '1:MILLISECONDS:EPOCH', '1:MILLISECONDS:EPOCH', '15:MINUTES')</li>
 *     <li> if Date column is expressed in hoursSinceEpoch, and output is expected in weeksSinceEpoch bucketed to weeks</li>
...
public class DateTimeConversionTransform implements TransformFunction {
  ...
  public <T> T transform(int length, BlockValSet... input) {
    ...
    for (int i = 0; i < inputDateTimeValues.length; i++) {
      Long dateTimeColumnValueMS = DateTimeFieldSpecUtils.fromFormatToMillis(inputDateTimeValues[i], inputFormat);
      Long bucketedDateTimevalueMS = DateTimeFieldSpecUtils.bucketDateTimeValueMS(dateTimeColumnValueMS, outputGranularity);
      Long dateTimeValueConverted = DateTimeFieldSpecUtils.convertMillisToCustomFormat(bucketedDateTimevalueMS, outputFormat);
      _output[i] = (Long) dateTimeValueConverted;
    }
    System.out.println();
    return (T) _output;
  }
