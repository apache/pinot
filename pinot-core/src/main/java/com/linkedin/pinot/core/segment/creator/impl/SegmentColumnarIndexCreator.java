...
//    boolean hasInvertedIndex = invertedIndexCreatorMap.containsKey();
      ...
      addColumnMetadataInfo(properties, column, columnIndexCreationInfo, totalDocs, totalRawDocs, totalAggDocs,
          schema.getFieldSpecFor(column), dictionaryElementSize, hasInvertedIndex, config.getHllConfig());
    }
  ...
  public static void addColumnMetadataInfo(PropertiesConfiguration properties, String column,
    ...
    // HLL derived fields
    String originColumnName = hllConfig.getDerivedHllFieldToOriginMap().get(column);
    if (originColumnName != null) {
      properties.setProperty(V1Constants.MetadataKeys.Column.getKeyFor(column, ORIGIN_COLUMN), originColumnName);
