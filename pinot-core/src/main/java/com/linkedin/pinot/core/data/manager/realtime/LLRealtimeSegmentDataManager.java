...
public class LLRealtimeSegmentDataManager extends RealtimeSegmentDataManager {
  ...
  protected boolean consumeLoop() throws Exception {
    ...
    final long maxIdleCountBeforeStatUpdate = (3 * 60 * 1000)/(idlePipeSleepTimeMillis + _streamMetadata.getFetchTimeoutMillis());  // 3 minute count
    ...
    while(!_shouldStop && !endCriteriaReached()) {
      // Consume for the next _streamReadTime ms, or we get to final offset, whichever happens earlier,
      // Update _currentOffset upon return from this method
      MessageBatch messageBatch = null;
      try {
        messageBatch = _consumerWrapper.fetchMessages(_currentOffset, _endOffset,
            _streamMetadata.getFetchTimeoutMillis());
        ...
        segmentLogger.warn("Stream permanent exception when fetching messages, stopping consumption", e);
        throw e;
      } catch (Exception e) {
  ...
  private void processStreamEvents(MessageBatch messagesAndOffsets, long idlePipeSleepTimeMillis) {
    ...
    for (int index = 0; index < messagesAndOffsets.getMessageCount(); index ++) {
      ...
      if (!canTakeMore) {
        ...
        //    Throw an exception.
        //
        // 2. We are in CATCHING_UP state, and we legally hit this error due to stream unclean leader election where
    ...
    if (streamMessageCount != 0) {
      segmentLogger.debug("Indexed {} messages ({} messages read from {}) current offset {}", _streamType,
          indexedMessageCount, streamMessageCount, _currentOffset);
    } else {
      // If there were no messages to be fetched from stream, wait for a little bit as to avoid hammering the
      // stream broker
  ...
  public LLRealtimeSegmentDataManager(RealtimeSegmentZKMetadata segmentZKMetadata, TableConfig tableConfig,
    ...
    _streamMetadata = StreamMetadataFactory.getStreamMetadata(tableConfig);
    _pinotStreamConsumerFactory = _streamMetadata.getPinotStreamConsumerFactory();
    StreamProviderConfig _streamProviderConfig = _streamMetadata.createStreamProviderConfig();
    _streamProviderConfig.init(tableConfig, instanceZKMetadata, schema);

    _streamName = _streamMetadata.getStreamName();
    _streamType = _streamMetadata.getStreamType();
    ...
    _metricKeyName = _tableName + "-" + _streamName + "-" + _streamPartitionId;
    segmentLogger = LoggerFactory.getLogger(LLRealtimeSegmentDataManager.class.getName() +
        "_" + _segmentNameStr);
    _tableStreamName = _tableName + "_" + _streamProviderConfig.getStreamName();
    ...
    int segmentMaxRowCount = _streamProviderConfig.getSizeThresholdToFlushSegment();
    ...
    _messageDecoder = _pinotStreamConsumerFactory.getDecoder(_streamProviderConfig);
    ...
    if (segmentPartitionConfig != null) {
      try {
        int nPartitions = _consumerWrapper.getPartitionCount(_streamName, /*maxWaitTimeMs=*/5000L);
    ...
    _consumeEndTime = now + _streamProviderConfig.getTimeThresholdToFlushSegment();
  ...
  private void makeConsumerWrapper(String reason) {
    if (_consumerWrapper != null) {
      try {
        ...
        segmentLogger.warn("Could not close {} consumer wrapper", _streamType);
      }
    }
    segmentLogger.info("Creating new {} consumer wrapper, reason: {}", _streamType, reason);
