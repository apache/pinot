 ...
 *     <p>E.g. THETA_SKETCH(col, '', 'dimName=''gender'' AND dimValue=''male''',
 ...
 *   </li>
 * </ul>
 * <p>E.g. THETA_SKETCH(col, 'nominalEntries=8192')
 */
@SuppressWarnings({"rawtypes", "unchecked"})
public class ThetaSketchAggregationFunction implements AggregationFunction<List<Sketch>, Long> {
  ...
  public ThetaSketchAggregationFunction(List<ExpressionContext> arguments) {
    ...
    if (numArguments > 1) {
      ...
      Preconditions.checkArgument(paramsExpression.getType() == ExpressionContext.Type.LITERAL,
          "Second argument of THETA_SKETCH aggregation function must be literal (parameters)");
    ...
    if (numArguments < 4) {
      ...
      _inputExpressions.add(_mainExpression);
      Map<ExpressionContext, Integer> expressionIndexMap = new HashMap<>();
      expressionIndexMap.put(_mainExpression, 0);
      ...
      for (int i = 2; i < numArguments - 1; i++) {
        ...
        Preconditions.checkArgument(filterExpression.getType() == ExpressionContext.Type.LITERAL,
            "Third to second last argument of THETA_SKETCH aggregation function must be literal (filter expression)");
      ...
      Preconditions.checkArgument(postAggregationExpression.getType() == ExpressionContext.Type.LITERAL,
          "Last argument of THETA_SKETCH aggregation function must be literal (post-aggregation expression)");
      try {
  ...
  public AggregationFunctionType getType() {
    return AggregationFunctionType.THETASKETCH;
  }

  @Override
  public String getColumnName() {
    return AggregationFunctionType.THETASKETCH.getName() + "_" + _mainExpression;
  }

  @Override
  public String getResultColumnName() {
    return AggregationFunctionType.THETASKETCH.getName().toLowerCase() + "(" + _mainExpression + ")";
  }
        ...
        switch (valueTypes[0]) {
          ...
          default:
            throw new IllegalStateException(
                "Illegal single-value data type for THETA_SKETCH aggregation function: " + valueTypes[0]);
        }
      } else {
        switch (valueTypes[0]) {
          case INT:
          ...
          default:
            throw new IllegalStateException(
                "Illegal multi-value data type for THETA_SKETCH aggregation function: " + valueTypes[0]);
        }
      }
    } else {
      // Serialized sketch
   ...
   */
  protected Sketch evaluatePostAggregationExpression(ExpressionContext expression, List<Sketch> sketches) {
    if (expression.getType() == ExpressionContext.Type.IDENTIFIER) {
