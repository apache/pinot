...
public final class ExpressionTypeResolver {
  ...
  private static final Map<String, BuiltInVirtualColumnDefinitions.Definition> BUILT_IN_VIRTUAL_COLUMNS =
      BuiltInVirtualColumnDefinitions.DEFINITIONS.stream()
          .collect(Collectors.toUnmodifiableMap(BuiltInVirtualColumnDefinitions.Definition::getName, d -> d));
  ...
  public ExpressionTypeResolver(Schema schema) {
    _columnTypeResolver = name -> {
      ...
      if (field != null) {
        return ColumnDataType.fromDataType(field.getDataType(), field.isSingleValueField());
      }
      // Brokers add the built-in virtual columns to their table schemas, but a server's table schema does not declare
      // them. Resolve them from their shared definitions so direct server SQL sees the same types as the broker.
      BuiltInVirtualColumnDefinitions.Definition virtualColumn = BUILT_IN_VIRTUAL_COLUMNS.get(name);
      return virtualColumn != null
          ? ColumnDataType.fromDataType(virtualColumn.getDataType(), virtualColumn.isSingleValueField())
          : null;
    };
  }
  ...
  private ColumnDataType resolveFunction(FunctionContext function) {
    ...
    if (functionInfo == null) {
      // A broker cannot see transforms registered only on servers, so an unknown function is not necessarily invalid.
      // Existing aggregates keep their unbound execution, and the server validates the function as before.
      throw new TypeInferenceUnavailableException(name);
    }
