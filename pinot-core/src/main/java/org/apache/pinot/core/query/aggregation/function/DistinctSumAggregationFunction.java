...
public class DistinctSumAggregationFunction extends BaseSingleInputAggregationFunction<Set, Double> {
  ...
  public AggregationFunctionType getType() {
    return AggregationFunctionType.DISTINCTSUM;
  }
  ...
  public void aggregate(int length, AggregationResultHolder aggregationResultHolder,
    ...
    switch (storedType) {
      ...
      case BYTES:
        ...
        break;
      default:
        throw new IllegalStateException("Illegal data type for DISTINCTSUM aggregation function: " + storedType);
    }
  }
  ...
  public void aggregateGroupBySV(int length, int[] groupKeyArray, GroupByResultHolder groupByResultHolder,
    ...
    switch (storedType) {
      case INT:
        int[] intValues = blockValSet.getIntValuesSV();
        for (int i = 0; i < length; i++) {
          ((IntOpenHashSet) getValueSet(groupByResultHolder, groupKeyArray[i], FieldSpec.DataType.INT)).add(
              intValues[i]);
        }
        break;
      case LONG:
        long[] longValues = blockValSet.getLongValuesSV();
        for (int i = 0; i < length; i++) {
          ((LongOpenHashSet) getValueSet(groupByResultHolder, groupKeyArray[i], FieldSpec.DataType.LONG)).add(
              longValues[i]);
        }
        break;
      case FLOAT:
        float[] floatValues = blockValSet.getFloatValuesSV();
        for (int i = 0; i < length; i++) {
          ((FloatOpenHashSet) getValueSet(groupByResultHolder, groupKeyArray[i], FieldSpec.DataType.FLOAT)).add(
              floatValues[i]);
        }
        break;
      case DOUBLE:
        double[] doubleValues = blockValSet.getDoubleValuesSV();
        for (int i = 0; i < length; i++) {
          ((DoubleOpenHashSet) getValueSet(groupByResultHolder, groupKeyArray[i], FieldSpec.DataType.DOUBLE)).add(
              doubleValues[i]);
        }
        break;
      case STRING:
        String[] stringValues = blockValSet.getStringValuesSV();
        for (int i = 0; i < length; i++) {
          ((ObjectOpenHashSet<String>) getValueSet(groupByResultHolder, groupKeyArray[i],
              FieldSpec.DataType.STRING)).add(stringValues[i]);
        }
        break;
      case BYTES:
        byte[][] bytesValues = blockValSet.getBytesValuesSV();
        for (int i = 0; i < length; i++) {
          ((ObjectOpenHashSet<ByteArray>) getValueSet(groupByResultHolder, groupKeyArray[i],
              FieldSpec.DataType.BYTES)).add(new ByteArray(bytesValues[i]));
        }
        break;
      default:
        throw new IllegalStateException("Illegal data type for DISTINCTSUM aggregation function: " + storedType);
  ...
  public Set extractAggregationResult(AggregationResultHolder aggregationResultHolder) {
    ...
    if (result instanceof DistinctSumAggregationFunction.DictIdsWrapper) {
      // For dictionary-encoded expression, convert dictionary ids to values
      return convertToValueSet((DistinctSumAggregationFunction.DictIdsWrapper) result);
    } else {
  ...
  protected static RoaringBitmap getDictIdBitmap(AggregationResultHolder aggregationResultHolder,
    ...
    DistinctSumAggregationFunction.DictIdsWrapper dictIdsWrapper = aggregationResultHolder.getResult();
    if (dictIdsWrapper == null) {
      dictIdsWrapper = new DistinctSumAggregationFunction.DictIdsWrapper(dictionary);
  ...
  private static Set getValueSet(FieldSpec.DataType valueType) {
    switch (valueType) {
      ...
      default:
        throw new IllegalStateException("Illegal data type for DISTINCT_SUM aggregation function: " + valueType);
    }
  }
  ...
  protected static RoaringBitmap getDictIdBitmap(GroupByResultHolder groupByResultHolder, int groupKey,
      Dictionary dictionary) {
    DistinctSumAggregationFunction.DictIdsWrapper dictIdsWrapper = groupByResultHolder.getResult(groupKey);
    if (dictIdsWrapper == null) {
      dictIdsWrapper = new DistinctSumAggregationFunction.DictIdsWrapper(dictionary);
   ...
   */
  protected static Set getValueSet(GroupByResultHolder groupByResultHolder, int groupKey,
  ...
  private static void setValueForGroupKeys(GroupByResultHolder groupByResultHolder, int[] groupKeys, int value) {
    for (int groupKey : groupKeys) {
      ((IntOpenHashSet) getValueSet(groupByResultHolder, groupKey, FieldSpec.DataType.INT)).add(value);
    }
  }
  ...
  private static void setValueForGroupKeys(GroupByResultHolder groupByResultHolder, int[] groupKeys, long value) {
    for (int groupKey : groupKeys) {
      ((LongOpenHashSet) getValueSet(groupByResultHolder, groupKey, FieldSpec.DataType.LONG)).add(value);
    }
  }
  ...
  private static void setValueForGroupKeys(GroupByResultHolder groupByResultHolder, int[] groupKeys, float value) {
    for (int groupKey : groupKeys) {
      ((FloatOpenHashSet) getValueSet(groupByResultHolder, groupKey, FieldSpec.DataType.FLOAT)).add(value);
    }
  }
  ...
  private static void setValueForGroupKeys(GroupByResultHolder groupByResultHolder, int[] groupKeys, double value) {
    for (int groupKey : groupKeys) {
      ((DoubleOpenHashSet) getValueSet(groupByResultHolder, groupKey, FieldSpec.DataType.DOUBLE)).add(value);
    }
  }
  ...
  private static void setValueForGroupKeys(GroupByResultHolder groupByResultHolder, int[] groupKeys, String value) {
    for (int groupKey : groupKeys) {
      ((ObjectOpenHashSet<String>) getValueSet(groupByResultHolder, groupKey, FieldSpec.DataType.STRING)).add(value);
    }
  }
  ...
  private static void setValueForGroupKeys(GroupByResultHolder groupByResultHolder, int[] groupKeys, ByteArray value) {
    for (int groupKey : groupKeys) {
      ((ObjectOpenHashSet<ByteArray>) getValueSet(groupByResultHolder, groupKey, FieldSpec.DataType.BYTES)).add(value);
    }
  }
   ...
   */
  private static Set convertToValueSet(DistinctSumAggregationFunction.DictIdsWrapper dictIdsWrapper) {
    ...
    switch (storedType) {
      ...
      case BYTES:
        ...
        return bytesSet;
      default:
        throw new IllegalStateException("Illegal data type for DISTINCT_SUM aggregation function: " + storedType);
    }
  }
