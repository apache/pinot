...
public interface ResultsBlockMerger<T extends BaseResultsBlock> {
  ...
  ///
  /// NOTE: `blockToMerge` should contain the result for a segment without any exception. The errored segment
  /// results are handled by [org.apache.pinot.core.operator.combine.BaseCombineOperator].
  ///
  /// @param mergedBlock The block that accumulates previous results. It should be modified to add the information of
  ///                    the other block.
  ...
  ///
  /// The input results block might not be mergeable.
