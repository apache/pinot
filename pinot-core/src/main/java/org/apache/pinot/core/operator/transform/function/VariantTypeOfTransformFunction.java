...
public class VariantTypeOfTransformFunction extends BaseVariantTransformFunction {
  ...
  public void init(List<TransformFunction> arguments, Map<String, ColumnContext> columnContextMap,
    ...
    initVariantArguments(arguments, 1, 2, false);
  }
  ...
  protected boolean evaluateVariant(@Nullable byte[] variant, int index) {
    String typeName = VariantUtils.variantTypeOf(variant, getVariantPath(), getReusableResult());
    if (typeName != null) {
      _stringValuesSV[index] = typeName;
      return true;
    }
    return false;
  }
