...
public class InTransformFunction extends BaseTransformFunction {
  ...
  public void init(List<TransformFunction> arguments, Map<String, DataSource> dataSourceMap) {
    ...
    if (allLiteralValues) {
      ...
      Preconditions.checkArgument(_mainFunction.getResultMetadata().isSingleValue(), "The first argument for ["
          + getName() + "] transform function must be single-valued when there are non-literal values");
      ...
      for (int i = 1; i < numArguments; i++) {
        ...
        Preconditions.checkArgument(valueFunction.getResultMetadata().isSingleValue(),
            "The values for [" + getName() + "] transform function must be single-valued");
