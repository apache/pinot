...
public class DistinctCountHLLSketchMVAggregationFunction extends DistinctCountHLLSketchAggregationFunction {

  public DistinctCountHLLSketchMVAggregationFunction(List<ExpressionContext> arguments) {
    super(arguments);
  }

  @Override
  public AggregationFunctionType getType() {
    return AggregationFunctionType.DISTINCTCOUNTHLLSKETCHMV;
  }
    ...
    switch (storedType) {
      ...
      default:
        throw new IllegalStateException(
            "Illegal data type for DISTINCT_COUNT_HLL_SKETCH_MV aggregation function: " + storedType);
    }
  }
  ...
  public void aggregateGroupBySV(int length, int[] groupKeyArray, GroupByResultHolder groupByResultHolder,
    ...
    switch (storedType) {
      case INT:
        int[][] intValuesArray = blockValSet.getIntValuesMV();
        for (int i = 0; i < length; i++) {
          HllSketch hyperLogLog = getDefaultHllSketch(groupByResultHolder, groupKeyArray[i]);
          for (int value : intValuesArray[i]) {
            hyperLogLog.update(value);
          }
        }
      ...
      case INT:
        ...
        for (int i = 0; i < length; i++) {
          ...
          for (int groupKey : groupKeysArray[i]) {
            HllSketch hyperLogLog = getDefaultHllSketch(groupByResultHolder, groupKey);
