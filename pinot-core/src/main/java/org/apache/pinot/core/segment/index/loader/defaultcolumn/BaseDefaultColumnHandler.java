...
public abstract class BaseDefaultColumnHandler implements DefaultColumnHandler {
  ...
  public void updateDefaultColumns(IndexLoadingConfig indexLoadingConfig)
    ...
    Map<String, DefaultColumnAction> defaultColumnActionMap = computeDefaultColumnActionMap(_schema, _segmentMetadata);
  ...
  public static Map<String, DefaultColumnAction> computeDefaultColumnActionMap(Schema schema,
    ...
    for (String column : columnsInSchema) {
      ...
      if (columnMetadata != null || columnFieldSpec != null) {
        ...
        FieldSpec.DataType dataTypeInMetadata =
            isConsumingSegment ? columnFieldSpec.getDataType() : columnMetadata.getDataType();
        FieldSpec.DataType dataTypeInSchema = fieldSpecInSchema.getDataType();
        boolean isSingleValueInMetadata =
            isConsumingSegment ? columnFieldSpec.isSingleValueField() : columnMetadata.isSingleValue();
        boolean isSingleValueInSchema = fieldSpecInSchema.isSingleValueField();
        String defaultValueInMetadata = isConsumingSegment ? columnFieldSpec.getDefaultNullValue().toString()
            : columnMetadata.getDefaultNullValueString();
        ...
        if (fieldTypeInMetadata == FieldSpec.FieldType.DIMENSION) {
          ...
          LOGGER.warn("Skip updating column: {} with field type: {}", column, fieldTypeInSchema);
        }
      } else {
    ...
    Set<String> columnsInMetadata = segmentMetadata.getAllColumns();
    for (String column : columnsInMetadata) {
      if (!columnsInSchema.contains(column)) {
        ColumnMetadata columnMetadata = isConsumingSegment ? null : segmentMetadata.getColumnMetadataFor(column);

        FieldSpec.FieldType fieldTypeInMetadata =
            isConsumingSegment ? segmentMetadata.getSchema().getFieldSpecFor(column).getFieldType()
                : columnMetadata.getFieldType();
        // For the committed segment, only remove auto-generated columns.
        if (isConsumingSegment || columnMetadata.isAutoGenerated()) {
          if (fieldTypeInMetadata == FieldSpec.FieldType.DIMENSION) {
