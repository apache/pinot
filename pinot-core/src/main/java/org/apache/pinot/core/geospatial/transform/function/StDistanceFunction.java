...
public class StDistanceFunction extends BaseTransformFunction {
    ...
    public double[] transformToDoubleValuesSV(ProjectionBlock projectionBlock) {
        ...
        for (int i = 0; i < projectionBlock.getNumDocs(); i++) {
            ...
            if (GeometryUtils.isGeography(firstGeometry)) {
                ...
                _results[i] = firstGeometry.isEmpty() || secondGeometry.isEmpty() ? null :
    ...
    public static void validateGeographyType(String function, Geometry geometry, Set<GeometryType> validTypes) {
        ...
        if (!validTypes.contains(type)) {
            throw new RuntimeException(String.format("When applied to Geography inputs, %s only supports %s. Input " +
                    "type is: %s", function, GeometryUtils.OR_JOINER.join(validTypes), type));
        }
    }
}
