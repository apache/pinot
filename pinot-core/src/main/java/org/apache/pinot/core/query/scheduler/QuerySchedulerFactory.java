...
public class QuerySchedulerFactory {
  ...
  public static QueryScheduler create(PinotConfiguration schedulerConfig, String instanceId,
      QueryExecutor queryExecutor, LongAccumulator latestQueryTime) {
    ...
    switch (schedulerName.toLowerCase()) {
      case FCFS_ALGORITHM:
        scheduler = new FCFSQueryScheduler(schedulerConfig, instanceId, queryExecutor, latestQueryTime);
        break;
      case TOKEN_BUCKET_ALGORITHM:
        scheduler = TokenPriorityScheduler.create(schedulerConfig, instanceId, queryExecutor, latestQueryTime);
        break;
      case BOUNDED_FCFS_ALGORITHM:
        scheduler = BoundedFCFSScheduler.create(schedulerConfig, instanceId, queryExecutor, latestQueryTime);
        break;
      case BINARY_WORKLOAD_ALGORITHM:
        scheduler = new BinaryWorkloadScheduler(schedulerConfig, instanceId, queryExecutor, latestQueryTime);
        break;
      case WORKLOAD_SCHEDULER_ALGORITHM:
        scheduler = new WorkloadScheduler(schedulerConfig, instanceId, queryExecutor, latestQueryTime);
        break;
      default:
        scheduler =
            getQuerySchedulerByClassName(schedulerName, schedulerConfig, instanceId, queryExecutor, latestQueryTime);
        break;
    }
    ...
    return new FCFSQueryScheduler(schedulerConfig, instanceId, queryExecutor, latestQueryTime);
  }

  @Nullable
  private static QueryScheduler getQuerySchedulerByClassName(String className, PinotConfiguration schedulerConfig,
      String instanceId, QueryExecutor queryExecutor, LongAccumulator latestQueryTime) {
    try {
      Constructor<?> constructor = PluginManager.get()
          .loadClass(className)
          .getDeclaredConstructor(PinotConfiguration.class, String.class, QueryExecutor.class, LongAccumulator.class);
      constructor.setAccessible(true);
      return (QueryScheduler) constructor.newInstance(schedulerConfig, instanceId, queryExecutor, latestQueryTime);
    } catch (Exception e) {
