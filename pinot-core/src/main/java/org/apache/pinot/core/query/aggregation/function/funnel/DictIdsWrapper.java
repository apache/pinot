...
final class DictIdsWrapper {
  ...
  // Stride-based composite mapping (non-null only for multi-key when product of dict sizes fits in int)
  private final int[] _strides;

  // HashMap-based composite mapping (non-null only for multi-key when stride overflows int)
