...
abstract class BaseDistinctCountSmartSketchAggregationFunction
  ...
  protected final void aggregateIntoSet(int length, AggregationResultHolder aggregationResultHolder,
    ...
    if (valueSet.size() > getThreshold()) {
      aggregationResultHolder.setValue(convertSetToSketch(valueSet, storedType));
    }
  }
  ...
  @Override
  public final Object extractAggregationResult(AggregationResultHolder aggregationResultHolder) {
    ...
    switch (valueType) {
      ...
      default:
        throw new IllegalStateException(
            "Illegal data type for DISTINCT_COUNT_SMART aggregation function: " + valueType);
    }
  }
