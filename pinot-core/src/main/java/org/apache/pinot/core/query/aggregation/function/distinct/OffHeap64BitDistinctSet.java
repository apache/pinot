...
/**
 * Off-heap distinct set for 64-bit values.
 *
 * NOTE: We store and return hash mixed values instead of original values.
 */
@NotThreadSafe
public class OffHeap64BitDistinctSet extends BaseOffHeapDistinctSet {
  private int _capacity;
  private int _mask;
  private int _maxFill;

  public OffHeap64BitDistinctSet(int expectedValues) {
    _capacity = arraySize(expectedValues);
    _mask = _capacity - 1;
    _maxFill = maxFill(_capacity);
    _address = allocateMemory((long) _capacity << 3);
  }
  ...
  private void rehash() {
    ...
    _mask = _capacity - 1;
    _maxFill = maxFill(_capacity);
    long newAddress = allocateMemory((long) _capacity << 3);
  ...
  public void merge(BaseOffHeapDistinctSet another) {
    ...
    OffHeap64BitDistinctSet another64BitSet = (OffHeap64BitDistinctSet) another;
  ...
  public static OffHeap64BitDistinctSet deserialize(ByteBuffer buffer) {
    int size = buffer.getInt();
    OffHeap64BitDistinctSet set = new OffHeap64BitDistinctSet(size);
    if (size > 0) {
      int startIndex = 0;
