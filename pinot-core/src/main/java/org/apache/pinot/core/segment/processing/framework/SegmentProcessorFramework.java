...
public class SegmentProcessorFramework {
  ...
  private Map<String, GenericRowFileManager> doMap(SegmentMapper mapper)
    ...
    try {
      ...
      // There was an error in the mapper phase, clean up all the files generated by the mapper.
      if (partitionToFileManagerMap != null && !partitionToFileManagerMap.isEmpty()) {
        for (GenericRowFileManager fileManager : partitionToFileManagerMap.values()) {
          fileManager.cleanUp();
        }
      }
      throw e;
