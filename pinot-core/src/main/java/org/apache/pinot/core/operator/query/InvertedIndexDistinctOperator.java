 ...
 * </ul>
 *
 * <p>Enabled via the {@code useInvertedIndexDistinct} query option. The cost ratio can be tuned
...
public class InvertedIndexDistinctOperator extends BaseOperator<DistinctResultsBlock> {
   ...
   * {@code dictionaryCardinality * costRatio <= filteredDocCount}.
   *
  ...
  private int estimateFilteredDocCount() {
    ...
    // Conservative fallback: assume all docs match (biases toward inverted index)
    return _indexSegment.getSegmentMetadata().getTotalDocs();
  }
  ...
  private boolean addValueToDistinctTable(DistinctTable distinctTable, int dictId,
    ...
    switch (_dictionary.getValueType()) {
      ...
      case BYTES:
        return addToTable((BytesDistinctTable) distinctTable, _dictionary.getByteArrayValue(dictId),
            orderByExpression);
      default:
        throw new IllegalStateException("Unsupported data type: " + _dictionary.getValueType());
    }
  ...
  public String toExplainString() {
    String explainName = _usedInvertedIndexPath ? EXPLAIN_NAME : EXPLAIN_NAME_SCAN_FALLBACK;
