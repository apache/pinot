...
public class AggregationGroupByOrderByPlanNode implements PlanNode {
  ...
  public AggregationGroupByOrderByOperator run() {
    ...
    if (_transformPlanNode != null) {
      ...
      return new AggregationGroupByOrderByOperator(_indexSegment, _functionContexts, _groupBy, _orderBy,
          _maxInitialResultHolderCapacity, _numGroupsLimit, _transformPlanNode.run(), numTotalRawDocs, false);
    } else {
      // Use star-tree
      return new AggregationGroupByOrderByOperator(_indexSegment, _functionContexts, _groupBy, _orderBy,
          _maxInitialResultHolderCapacity, _numGroupsLimit, _starTreeTransformPlanNode.run(), numTotalRawDocs, true);
    }
  }
