...
public class TableResizer {
  ...
  private IntermediateRecord[] getTopRecordsHeap(RadixPartitionedHashMap recordsMap, int size,
    ...
    Iterator<IntermediateRecord> mapEntryIterator = recordsMap.iterator();

    // Initialize a heap with the first 'size' map entries
    IntermediateRecord[] heap = new IntermediateRecord[size];
    for (int i = 0; i < size; i++) {
      IntermediateRecord entry = mapEntryIterator.next();
      if (entry._values == null) {
        entry._values = getIntermediateRecordValues(entry);
      }
      heap[i] = entry;
    }
    ...
    while (mapEntryIterator.hasNext()) {
      IntermediateRecord intermediateRecord = mapEntryIterator.next();
      intermediateRecord._values = getIntermediateRecordValues(intermediateRecord);
  ...
  private Comparable[] getIntermediateRecordValues(IntermediateRecord entry) {
    Comparable[] orderByValues = new Comparable[_numOrderByExpressions];
    for (int i = 0; i < _numOrderByExpressions; i++) {
      orderByValues[i] = _orderByValueExtractors[i].extract(entry._record);
    }
  ...
  List<Record> getSortedTopRecords(RadixPartitionedHashMap recordsMap, int size) {
  ...
  private Collection<Record> getUnsortedTopRecords(RadixPartitionedHashMap recordsMap, int size) {
    int numRecords = recordsMap.size();
    if (numRecords <= size) {
