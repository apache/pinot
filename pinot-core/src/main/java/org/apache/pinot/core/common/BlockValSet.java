...
/// The `BlockValSet` contains a block of values for a column (Projection layer) or a transform expression
/// (Transform layer).
public interface BlockValSet {
  ...
  /// Returns `true` if the value set is for a single-value column, `false` otherwise.
  boolean isSingleValue();

  /// Returns the dictionary for the column if one exists, or `null` otherwise. The dictionary may live on disk
  /// (segment-backed columns) or be built on the fly (transform functions). It may be present even when
  /// [#isDictionaryEncoded()] returns `false` — a column declared as `EncodingType.RAW` with an
  /// explicit `dictionaryIndex` carries a dictionary on disk but a RAW forward index, and a column with a
  /// disabled forward index has no way to read dict IDs at all. Callers that select between a dictionary-id read
  /// path ([#getDictionaryIdsSV()] / [#getDictionaryIdsMV()]) and a value read path MUST gate on
  /// [#isDictionaryEncoded()], not `getDictionary() != null`.
  @Nullable
  Dictionary getDictionary();

  /// Returns `true` if the dict-id read path ([#getDictionaryIdsSV()] / [#getDictionaryIdsMV()])
  /// is callable on this value set.
  ///
  /// The default implementation falls back to `getDictionary() != null`, which is correct for value sets
  ...
  /// `EncodingType.RAW` alongside an explicit `dictionaryIndex` (dictionary present, but
  /// `readDictIds` throws), or where the forward index is disabled outright (no forward index to read).
