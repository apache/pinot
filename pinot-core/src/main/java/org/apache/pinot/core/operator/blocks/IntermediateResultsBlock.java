...
public class IntermediateResultsBlock implements Block {
  ...
  private DataTable getResultDataTable()
    ...
    Object[] columnDefaultNullValues = null;
    RoaringBitmap[] columnNullRoaringBitmap = null;
    if (DataTableBuilder.getCurrentDataTableVersion() >= DataTableBuilder.VERSION_4) {
      columnDefaultNullValues = new Object[numColumns];
      columnNullRoaringBitmap = new RoaringBitmap[numColumns];
    ...
    while (iterator.hasNext()) {
      ...
      for (int colIdx = 0; colIdx < values.length; colIdx++) {
        ...
        if (value == null && columnNullRoaringBitmap != null) {
          value = columnDefaultNullValues[colIdx];
          columnNullRoaringBitmap[colIdx].add(rowId);
        }
        setDataTableColumn(storedColumnDataTypes[colIdx], dataTableBuilder, colIdx, value);
      }
      dataTableBuilder.finishRow();
      rowId++;
    }
    if (columnNullRoaringBitmap != null) {
      for (int colId = 0; colId < numColumns; colId++) {
        dataTableBuilder.setNullRowIds(columnNullRoaringBitmap[colId].toMutableRoaringBitmap());
      }
    }
  ...
  private DataTable getAggregationResultDataTable()
    ...
    Object[] columnDefaultNullValues = null;
    RoaringBitmap[] columnNullBitmap = null;
    for (int i = 0; i < numAggregationFunctions; i++) {
      ...
      columnNullBitmap[i] = new RoaringBitmap();
    }
    ...
    for (int i = 0; i < numAggregationFunctions; i++) {
      ...
      if (columnDataTypes[i] != ColumnDataType.OBJECT) {
        if (value == null) {
          value = columnDefaultNullValues[i];
          columnNullBitmap[i].add(0);
        }
      }
    ...
    if (columnNullBitmap != null) {
      for (int i = 0; i < numAggregationFunctions; i++) {
        if (columnNullBitmap[i].getCardinality() > 0) {
          dataTableBuilder.setNullRowIds(columnNullBitmap[i].toMutableRoaringBitmap());
        }
      }
    }
    DataTable dataTable = dataTableBuilder.build();
