  ...
  private static String computePayloadHash(String tableName, List<String> columns,
    ...
    try {
      ...
      for (List<Object> row : rows) {
        for (Object val : row) {
          // Encode type prefix so that NULL vs "null", 1 vs "1", etc. produce different hashes
          String typeTag = (val == null) ? "NULL" : val.getClass().getSimpleName();
          digest.update(typeTag.getBytes(StandardCharsets.UTF_8));
          digest.update((byte) 0);
