...
public class BrokerReduceService {
  ...
  public BrokerResponseNative reduceOnDataTable(BrokerRequest brokerRequest,
    ...
    if (brokerRequest.getSelections() != null) {
      // Selection query
      SelectionResultSetter selectionResultSetter =
          new SelectionResultSetter(tableName, brokerRequest, cachedDataSchema, dataTableMap, brokerResponseNative,
              brokerMetrics);
      selectionResultSetter.setSelectionResults();
    } else {
      // Aggregation query
      if (!brokerRequest.isSetGroupBy()) {
        // Aggregation only
        AggregationResultSetter aggregationResultSetter =
            new AggregationResultSetter(tableName, brokerRequest, cachedDataSchema, dataTableMap, brokerResponseNative,
                brokerMetrics);
        aggregationResultSetter.setAggregationResults();
      } else {
        // Aggregation group by
        GroupByResultSetter groupByResultSetter =
            new GroupByResultSetter(tableName, brokerRequest, cachedDataSchema, dataTableMap, brokerResponseNative,
                brokerMetrics);
        groupByResultSetter.setAggregationGroupByResults();
      }
    }
