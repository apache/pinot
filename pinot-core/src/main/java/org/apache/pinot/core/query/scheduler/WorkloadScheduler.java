...
public class WorkloadScheduler extends QueryScheduler {
  ...
  public static final String MAX_SECONDARY_QUERIES = "wlm.maxSecondaryRunnerThreads";
  ...
  public ListenableFuture<byte[]> submit(ServerQueryRequest queryRequest) {
    ...
    if (!_workloadBudgetManager.canAdmitQuery(workloadName)) {
      LOGGER.warn("Workload budget exceeded for query {} table {}, workloadName: {}", queryRequest.getRequestId(),
          queryRequest.getTableNameWithType(), workloadName);
