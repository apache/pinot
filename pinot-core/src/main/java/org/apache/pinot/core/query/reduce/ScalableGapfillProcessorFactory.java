...
public final class ScalableGapfillProcessorFactory {
  private ScalableGapfillProcessorFactory() {
  }

  /**
   * Constructs the right result reducer based on the given query context.
   */
  public static ScalableGapfillProcessor getScalableGapfillProcessor(
    ...
    if (queryContext.getSelectExpressions().size() == 2
        ...
        && queryContext.getSelectExpressions().get(1).getFunction().getFunctionName().equalsIgnoreCase("count")) {
      return new ScalableGapfillProcessorForCount(queryContext, gapfillType);
    }
    ...
    for (ExpressionContext expressionContext: queryContext.getSelectExpressions()) {
      if (expressionContext.getType() == ExpressionContext.Type.FUNCTION
          && (expressionContext.getFunction().getFunctionName().equalsIgnoreCase("sum")
      || expressionContext.getFunction().getFunctionName().equalsIgnoreCase("avg"))) {
        sumOrAvg++;
    ...
    if (groupBy == 1 && sumOrAvg + groupBy == queryContext.getSelectExpressions().size()) {
      return new ScalableGapfillProcessorForSumAvg(queryContext, gapfillType);
    } else {
      return null;
    }
  }
