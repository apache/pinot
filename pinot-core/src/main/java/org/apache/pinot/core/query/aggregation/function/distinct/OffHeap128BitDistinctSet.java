...
/**
 * Off-heap distinct set for 128-bit values.
 *
 * NOTE: We store and return hash mixed values instead of original values.
 */
@NotThreadSafe
public class OffHeap128BitDistinctSet extends BaseOffHeapDistinctSet {
  private int _capacity;
  private int _mask;
  private int _maxFill;

  public OffHeap128BitDistinctSet(int expectedValues) {
    _capacity = arraySize(expectedValues);
    _mask = _capacity - 1;
    _maxFill = maxFill(_capacity);
    _address = allocateMemory((long) _capacity << 4);
  }
  ...
  private void rehash() {
    ...
    _mask = _capacity - 1;
    _maxFill = maxFill(_capacity);
    long newAddress = allocateMemory((long) _capacity << 4);
  ...
  @Override
  public void merge(BaseOffHeapDistinctSet another) {
    if (another.isEmpty()) {
      return;
    }
    OffHeap128BitDistinctSet another128BitSet = (OffHeap128BitDistinctSet) another;
  ...
  public static OffHeap128BitDistinctSet deserialize(ByteBuffer buffer) {
    int size = buffer.getInt();
    OffHeap128BitDistinctSet set = new OffHeap128BitDistinctSet(size);
    if (size > 0) {
      int startIndex = 0;
