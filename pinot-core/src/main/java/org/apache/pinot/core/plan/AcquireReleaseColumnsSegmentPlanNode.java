...
/// NOTE: This is only used if `pinot.server.query.executor.enable.prefetch` is true
/// This PlanNode differs from the other PlanNodes in the following way:
/// This PlanNode does not invoke a `run` on the childOperator in its run method.
/// Instead, it passes the childPlanNode as is, to the [AcquireReleaseColumnsSegmentOperator],
...
public class AcquireReleaseColumnsSegmentPlanNode implements PlanNode {
  ...
  /// but instead just creates a [AcquireReleaseColumnsSegmentOperator] and passes the plan to it
  @Override
