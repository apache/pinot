...
/**
 * Off-heap distinct set for 32-bit values.
 *
 * NOTE: We store and return hash mixed values instead of original values.
 */
@NotThreadSafe
public class OffHeap32BitDistinctSet extends BaseOffHeapDistinctSet {
  private int _capacity;
  private int _mask;
  private int _maxFill;

  public OffHeap32BitDistinctSet(int expectedValues) {
    _capacity = arraySize(expectedValues);
    _mask = _capacity - 1;
    _maxFill = maxFill(_capacity);
    _address = allocateMemory((long) _capacity << 2);
  }
  ...
  private void rehash() {
    ...
    _mask = _capacity - 1;
    _maxFill = maxFill(_capacity);
    long newAddress = allocateMemory((long) _capacity << 2);
  ...
  public void merge(BaseOffHeapDistinctSet another) {
    ...
    OffHeap32BitDistinctSet another32BitSet = (OffHeap32BitDistinctSet) another;
  ...
  public static OffHeap32BitDistinctSet deserialize(ByteBuffer buffer) {
    int size = buffer.getInt();
    OffHeap32BitDistinctSet set = new OffHeap32BitDistinctSet(size);
    if (size > 0) {
      int startIndex = 0;
