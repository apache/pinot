...
public class BuildRingBufferConsumer implements EventHandler<GenericRow> {
  ...
  public void onEvent(GenericRow row, long sequence, boolean endOfBatch) {
    try {
      ...
      if (row.getValue(GenericRow.MULTIPLE_RECORDS_KEY) != null) {
        totalRecordReadTime += (System.currentTimeMillis() - recordReadStartTime);
  ...
  private void indexRow(GenericRow row) {
    ...
    if (transformedRow != null && IngestionUtils.shouldIngestRow(transformedRow)) {
      ...
      long indexStopTime = System.currentTimeMillis();
      totalIndexTime += (indexStopTime - recordReadStopTime);
    }
  }

  public long getTotalRecordReadTime() {
    return this.totalRecordReadTime;
  }
  ...
  public long getTotalStatsCollectorTime() {
    return this.totalStatsCollectorTime;
  }
}
