    ...
    if (blockValSet.getValueType() == DataType.BYTES && blockValSet.isSingleValue()) {
      ...
      forEachNotNull(length, blockValSet, (from, to) -> {
        for (int i = from; i < to; i++) {
          ...
          FloatSet set = ObjectSerDeUtils.FLOAT_SET_SER_DE.deserialize(starTreeSetPayload(bytesValues[i],
              ElementType.FLOAT));
          FloatIterator iterator = set.iterator();
          while (iterator.hasNext()) {
            setGroupByResult(groupByResultHolder, groupKey, iterator.nextFloat());
          }
        }
      });
      return;
