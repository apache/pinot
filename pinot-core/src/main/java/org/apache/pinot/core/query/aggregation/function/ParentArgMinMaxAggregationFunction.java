...
public class ParentArgMinMaxAggregationFunction extends ParentAggregationFunction<ArgMinMaxObject, ArgMinMaxObject> {
  ...
  private void initializeWithNewDataBlocks(Map<ExpressionContext, BlockValSet> blockValSetMap) {
    if (blockValSetMap == null) {
      if (_schemaInitiated.get()) {
        return;
      }
      _schemaInitiated.set(true);
    ...
    List<ArgMinMaxWrapperValSet> argMinMaxWrapperMeasuringColumnSets = _argMinMaxWrapperMeasuringColumnSets.get();
    List<ArgMinMaxWrapperValSet> argMinMaxWrapperProjectionColumnSets = _argMinMaxWrapperProjectionColumnSets.get();
    if (_schemaInitiated.get()) {
      for (int i = 0; i < _numMeasuringColumns; i++) {
        argMinMaxWrapperMeasuringColumnSets.get(i).setNewBlock(blockValSetMap.get(_measuringColumns.get(i)));
      }
      for (int i = 0; i < _numProjectionColumns; i++) {
        argMinMaxWrapperProjectionColumnSets.get(i).setNewBlock(blockValSetMap.get(_projectionColumns.get(i)));
      }
      return;
    }
    ...
    for (int i = 0; i < _numMeasuringColumns; i++) {
      ...
      switch (blockValSet.getValueType().getStoredType()) {
        ...
        case FLOAT:
          argMinMaxWrapperMeasuringColumnSets.add(
              new ArgMinMaxWrapperValSet(true, DataSchema.ColumnDataType.FLOAT, blockValSet));
          measuringColTypes[i] = DataSchema.ColumnDataType.FLOAT;
          break;
        case DOUBLE:
          argMinMaxWrapperMeasuringColumnSets.add(
              new ArgMinMaxWrapperValSet(true, DataSchema.ColumnDataType.DOUBLE, blockValSet));
        ...
        case BIG_DECIMAL:
          argMinMaxWrapperMeasuringColumnSets.add(
              new ArgMinMaxWrapperValSet(true, DataSchema.ColumnDataType.BIG_DECIMAL, blockValSet));
    ...
    for (int i = 0; i < _projectionColumns.size(); i++) {
      ...
      if (blockValSet.isSingleValue()) {
        switch (blockValSet.getValueType()) {
          ...
          case BYTES:
            argMinMaxWrapperProjectionColumnSets.add(
                new ArgMinMaxWrapperValSet(true, DataSchema.ColumnDataType.BYTES, blockValSet));
        ...
        switch (blockValSet.getValueType()) {
          ...
          case FLOAT:
            argMinMaxWrapperProjectionColumnSets.add(
                new ArgMinMaxWrapperValSet(false, DataSchema.ColumnDataType.FLOAT_ARRAY, blockValSet));
            projectionColTypes[i] = DataSchema.ColumnDataType.FLOAT_ARRAY;
            break;
          case DOUBLE:
            argMinMaxWrapperProjectionColumnSets.add(
                new ArgMinMaxWrapperValSet(false, DataSchema.ColumnDataType.DOUBLE_ARRAY, blockValSet));
            projectionColTypes[i] = DataSchema.ColumnDataType.DOUBLE_ARRAY;
            break;
          case STRING:
            argMinMaxWrapperProjectionColumnSets.add(
                new ArgMinMaxWrapperValSet(false, DataSchema.ColumnDataType.STRING_ARRAY, blockValSet));
            projectionColTypes[i] = DataSchema.ColumnDataType.STRING_ARRAY;
            break;
          case BYTES:
            argMinMaxWrapperProjectionColumnSets.add(
                new ArgMinMaxWrapperValSet(false, DataSchema.ColumnDataType.BYTES_ARRAY, blockValSet));
