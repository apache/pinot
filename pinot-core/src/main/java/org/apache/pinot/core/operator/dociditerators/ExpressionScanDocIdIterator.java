...
/**
 * The {@code ExpressionScanDocIdIterator} is the base class of scan-based iterator for ExpressionFilterDocIdSet that
 * can handle filters on the expressions. It leverages the projection operator to batch processing the records block
 * by block.
 */
public abstract class ExpressionScanDocIdIterator implements ScanBasedDocIdIterator {
  protected final TransformFunction _transformFunction;
  protected final Map<String, DataSource> _dataSourceMap;
  protected final int _endDocId;

  protected final int[] _docIdBuffer = new int[DocIdSetPlanNode.MAX_DOC_PER_CALL];
  ...
  public ExpressionScanDocIdIterator(TransformFunction transformFunction, Map<String, DataSource> dataSourceMap,
      int numDocs) {
  ...
  abstract protected void processProjectionBlock(ProjectionBlock projectionBlock, BitmapDataProvider matchingDocIds);
