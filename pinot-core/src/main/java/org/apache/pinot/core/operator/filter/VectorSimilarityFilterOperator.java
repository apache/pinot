...
/// - **Required candidate scope:** When the query restricts which documents are visible in a segment, that set is
///      applied before candidate selection rather than intersected afterwards; see [VectorCandidateScope]. This
///      operator requires a filter-aware reader in that case -- the planner picks [ExactVectorScanFilterOperator]
///      when the reader cannot do filtered search.
///
...
public class VectorSimilarityFilterOperator extends BaseFilterOperator {
  ...
  /// restriction. Applied before top-K selection; see [VectorCandidateScope].
  @Nullable
  ...
  ///
  /// @param candidateScope documents this predicate may consider, applied before top-K selection. The reader must
  ///        support filtered search when a scope is supplied; [org.apache.pinot.core.plan.FilterPlanNode] selects
  ///        [ExactVectorScanFilterOperator] instead when it does not.
  public VectorSimilarityFilterOperator(VectorIndexReader vectorIndexReader, VectorSimilarityPredicate predicate,
      int numDocs, VectorSearchParams searchParams, @Nullable ForwardIndexReader<?> forwardIndexReader,
      @Nullable VectorIndexConfig vectorIndexConfig, boolean hasMetadataFilter,
    ...
    _requiredDocIds = candidateScope != null ? candidateScope.getRequiredDocIds() : null;
  ...
  protected void explainAttributes(ExplainAttributeBuilder attributeBuilder) {
    ...
    if (hasRequiredCandidateScope()) {
      ...
      attributeBuilder.putLongIdempotent("requiredDocIdFilterCardinality", _requiredDocIds.getCardinality());
