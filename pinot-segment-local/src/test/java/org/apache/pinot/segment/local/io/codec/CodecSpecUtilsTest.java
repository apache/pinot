...
public class CodecSpecUtilsTest {
  ...
  public void testCompressionOnlySpecsMapToLegacyChunkCompressionType() {
    ...
    assertEquals(CodecSpecUtils.toLegacyChunkCompressionType("ZSTANDARD"), ChunkCompressionType.ZSTANDARD);
  }

  @Test
  public void testNonLegacyCompatibleSpecsReturnNull() {
    assertNull(CodecSpecUtils.toLegacyChunkCompressionType("ZSTD(8)"));
    assertNull(CodecSpecUtils.toLegacyChunkCompressionType("DELTA"));
    assertNull(CodecSpecUtils.toLegacyChunkCompressionType("DELTA,LZ4"));
    assertNull(CodecSpecUtils.toLegacyChunkCompressionType("LZ4,SNAPPY"));
  }

  @Test
  public void testHasTransform() {
    assertFalse(CodecSpecUtils.hasTransform("LZ4"));
    assertFalse(CodecSpecUtils.hasTransform("LZ4,ZSTD(3)"));
    assertTrue(CodecSpecUtils.hasTransform("DELTA,LZ4"));
    assertTrue(CodecSpecUtils.hasTransform("T64"));
  }
}
