...
public class DefaultColumnHandlerTest {
  ...
  @Test
  public void testComputeDefaultColumnActionMapWithReclaimDeletedColumns()
      throws Exception {
    TableConfig reclaimTableConfig =
        new TableConfigBuilder(TableType.OFFLINE).setTableName(RAW_TABLE_NAME).build();
    reclaimTableConfig.getIndexingConfig().setReclaimDeletedColumnsOnReload(true);

    try (SegmentDirectory segmentDirectory = new SegmentLocalFSDirectory(INDEX_DIR, ReadMode.mmap);
        SegmentDirectory.Writer writer = segmentDirectory.createWriter()) {
      // Remove a non-auto-generated (originally ingested) single-value STRING dimension from the schema.
      _schema.removeField("column2");

      // With reclaim disabled (default), the ingested column is NOT physically removed.
      BaseDefaultColumnHandler defaultHandler =
          new V3DefaultColumnHandler(INDEX_DIR, segmentDirectory.getSegmentMetadata(),
              new IndexLoadingConfig(TABLE_CONFIG, _schema), writer);
      assertEquals(defaultHandler.computeDefaultColumnActionMap(), Map.of());

      // With reclaim enabled, the ingested column gets a REMOVE action so its index files are reclaimed on reload.
      BaseDefaultColumnHandler reclaimHandler =
          new V3DefaultColumnHandler(INDEX_DIR, segmentDirectory.getSegmentMetadata(),
              new IndexLoadingConfig(reclaimTableConfig, _schema), writer);
      assertEquals(reclaimHandler.computeDefaultColumnActionMap(),
          Map.of("column2", DefaultColumnAction.REMOVE_DIMENSION));
      assertTrue(reclaimHandler.needUpdateDefaultColumns());
    } finally {
      // Restore the schema to its original state for other tests.
      _schema.addField(new DimensionFieldSpec("column2", DataType.STRING, true));
    }
  }
