...
public class ColumnarSegmentBuildingTest {
  ...
  public void testCompatibleDataTypeChanges() throws Exception {
    // Create original segment with original data types
    Schema originalSchema = new Schema.SchemaBuilder()
        .addSingleValueDimension("intColumn", FieldSpec.DataType.INT)
        .addSingleValueDimension("longColumn", FieldSpec.DataType.LONG)
        .addSingleValueDimension("anotherIntColumn", FieldSpec.DataType.INT)
        .addSingleValueDimension("thirdIntColumn", FieldSpec.DataType.INT)
        .build();

    // Create test data
    List<GenericRow> testData = new ArrayList<>();
    for (int i = 0; i < 100; i++) {
      GenericRow row = new GenericRow();
      row.putValue("intColumn", i);
      row.putValue("longColumn", (long) i * 1000);
      row.putValue("anotherIntColumn", i * 10);
      row.putValue("thirdIntColumn", i + 100);
      testData.add(row);
    }

    // Create original segment
    File originalSegmentDir = createSegmentFromData("compatibleDataTypeTest", originalSchema, testData);

    // Create target schema with compatible data type changes
    // INT -> LONG, LONG -> STRING, INT -> FLOAT, INT -> STRING
    Schema targetSchema = new Schema.SchemaBuilder()
        .addSingleValueDimension("intColumn", FieldSpec.DataType.LONG)      // INT -> LONG
        .addSingleValueDimension("longColumn", FieldSpec.DataType.STRING)   // LONG -> STRING
        .addSingleValueDimension("anotherIntColumn", FieldSpec.DataType.FLOAT) // INT -> FLOAT
        .addSingleValueDimension("thirdIntColumn", FieldSpec.DataType.STRING)  // INT -> STRING
        .build();

    // Create columnar segment with data type changes
    File columnarSegmentDir = createColumnarSegmentWithDataTypeChanges("compatibleDataTypeTestColumnar",
        originalSegmentDir, targetSchema);

    // Verify the columnar segment has the correct data types and values
    verifySegmentWithDataTypeChanges(columnarSegmentDir, targetSchema, testData);

    // Clean up
    FileUtils.deleteDirectory(originalSegmentDir);
    FileUtils.deleteDirectory(columnarSegmentDir);
  }

  private File createSegmentFromData(String segmentName, Schema schema, List<GenericRow> testData) throws Exception {
    File outputDir = new File(_tempDir, segmentName + "_original");
    ...
    // Create segment using row-major approach
    SegmentIndexCreationDriverImpl driver = new SegmentIndexCreationDriverImpl();
    TestRecordReader recordReader = new TestRecordReader(testData);
  ...
  private File createColumnarSegmentWithDataTypeChanges(String segmentName, File originalSegmentDir,
      Schema targetSchema) throws Exception {
    // Load the original segment
    IndexSegment originalSegment = ImmutableSegmentLoader.load(originalSegmentDir, ReadMode.mmap);

    try {
      // Create segment generator config with target schema
      TableConfig tableConfig = new TableConfigBuilder(TableType.OFFLINE).setTableName("testTable").build();
      SegmentGeneratorConfig config = new SegmentGeneratorConfig(tableConfig, targetSchema);
      config.setOutDir(_tempDir.getAbsolutePath());
      config.setSegmentName(segmentName);

      // Create column reader factory
      PinotSegmentColumnReaderFactory factory = new PinotSegmentColumnReaderFactory(originalSegment);

      // Create and build columnar segment
      SegmentIndexCreationDriverImpl driver = new SegmentIndexCreationDriverImpl();
      driver.init(config, factory);
      driver.build();

      return new File(_tempDir, segmentName);
    } finally {
      originalSegment.destroy();
    }
  }

  private void verifySegmentWithDataTypeChanges(File segmentDir, Schema targetSchema,
      List<GenericRow> originalData) throws Exception {
    IndexSegment segment = ImmutableSegmentLoader.load(segmentDir, ReadMode.mmap);

    try {
      // Verify schema matches target schema
      for (FieldSpec fieldSpec : targetSchema.getAllFieldSpecs()) {
        String columnName = fieldSpec.getName();
        ColumnMetadata columnMetadata = segment.getSegmentMetadata().getColumnMetadataFor(columnName);
        Assert.assertNotNull(columnMetadata, "Column should exist: " + columnName);
        Assert.assertEquals(columnMetadata.getDataType(), fieldSpec.getDataType(),
            "Data type should match for column: " + columnName);
      }

      // Verify data integrity with type conversions using segment record reading
      GenericRow actualRow = new GenericRow();
      for (int docId = 0; docId < originalData.size(); docId++) {
        GenericRow originalRow = originalData.get(docId);
        segment.getRecord(docId, actualRow);

        // Verify INT -> LONG conversion
        long expectedLongValue = (Integer) originalRow.getValue("intColumn");
        long actualLongValue = (Long) actualRow.getValue("intColumn");
        Assert.assertEquals(actualLongValue, expectedLongValue, "INT -> LONG conversion failed for docId: " + docId);

        // Verify LONG -> STRING conversion
        String expectedStringValue = String.valueOf(originalRow.getValue("longColumn"));
        String actualStringValue = (String) actualRow.getValue("longColumn");
        Assert.assertEquals(actualStringValue, expectedStringValue,
            "LONG -> STRING conversion failed for docId: " + docId);

        // Verify INT -> FLOAT conversion
        float expectedFloatValue = ((Integer) originalRow.getValue("anotherIntColumn")).floatValue();
        float actualFloatValue = (Float) actualRow.getValue("anotherIntColumn");
        Assert.assertEquals(actualFloatValue, expectedFloatValue, 0.001f,
            "INT -> FLOAT conversion failed for docId: " + docId);

        // Verify INT -> STRING conversion
        String expectedStringValue2 = String.valueOf(originalRow.getValue("thirdIntColumn"));
        String actualStringValue2 = (String) actualRow.getValue("thirdIntColumn");
        Assert.assertEquals(actualStringValue2, expectedStringValue2,
            "INT -> STRING conversion failed for docId: " + docId);
      }
    } finally {
      segment.destroy();
    }
