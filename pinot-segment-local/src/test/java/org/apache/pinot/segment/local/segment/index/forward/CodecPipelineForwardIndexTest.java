...
public class CodecPipelineForwardIndexTest implements PinotBuffersAfterMethodCheckRule {
  ...
  /// Chain framework smoke test — the validator and executor must accept and run any number
  /// of transform stages followed by an optional compression stage. All v1 built-in transforms
  /// (DELTA, DELTADELTA, T64, GORILLA) embed a per-frame `[flag][count]` header in their output
  /// and are therefore **terminal**: they are not chainable with another *transform*, only with a
  /// final compression stage that operates on arbitrary bytes. This test exercises the longest
  /// chain that is round-trip-correct with the v1 built-ins: one transform followed by one
  /// compression.
  ///
