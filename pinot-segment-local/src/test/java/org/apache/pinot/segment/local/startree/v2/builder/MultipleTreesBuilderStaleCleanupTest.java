...
public class MultipleTreesBuilderStaleCleanupTest {
  ...
  public void staleIndexFileFromKilledBuildIsCleanedUp()
    ...
    FileUtils.writeStringToFile(staleIndex, "stale bytes from a prior killed build",
  ...
  public void staleSeparatorTempDirFromKilledIncrementalIsCleanedUp()
    ...
    FileUtils.writeStringToFile(new File(staleSeparatorDir, "leftover.bin"), "old",
        java.nio.charset.StandardCharsets.UTF_8);
  ...
  private void buildBaseSegment()
    ...
    List<GenericRow> rows = Arrays.asList(makeRow("A", 1L), makeRow("B", 2L), makeRow("C", 3L));
  ...
  private List<StarTreeV2BuilderConfig> createBuilderConfigs(File segmentDir)
    ...
    try {
      return StarTreeBuilderUtils.generateBuilderConfigs(Arrays.asList(starTreeConfig), false,
