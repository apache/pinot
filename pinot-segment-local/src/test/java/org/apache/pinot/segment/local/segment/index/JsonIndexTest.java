...
public class JsonIndexTest implements PinotBuffersAfterMethodCheckRule {
  ...
  /**
   * The realtime parse-once path must build an identical index: indexing each document via {@code addParsed(Map)}
   * (the cache path) must yield the same matching docIds as {@code add(String)} (the re-parse path) for every filter.
   */
  @Test
   ...
   * PinotDataType does) and that node is fed to the index via {@code addParsed}. It must yield the same matching docIds
