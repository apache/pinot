...
public interface TableDataManager {
  ...
  /// See [#addSegment(ImmutableSegment, SegmentZKMetadata)] for details.
  @VisibleForTesting
  ...
  ///
  /// Segment ZK metadata might not be available when replacing a CONSUMING segment with the locally sealed one or
  /// invoked from tests.
  ///
  ...
  /// Offloads a segment from table like the method [#offloadSegment(String)], but this method doesn't take
  ...
  /// Deletes a segment from this table — offloads it if currently loaded, then removes its on-disk data (the
  /// per-segment directory and any tier-specific artefacts).
  ...
  /// by using [#isSegmentDeletedRecently(String)].
  ///
  ...
  /// @return Segment data manager with the given name, or `null` if no segment matches the name
  @Nullable
  ...
  /// and cached.
  ///
  /// @return List of [SegmentErrorInfo]
  ...
  /// Returns the cached latest [TableConfig] and [Schema] pair for the table. The cache is refreshed when
  /// invoking [#fetchIndexLoadingConfig()] or [#updateCachedTableConfigAndSchema(TableConfig, Schema)], and
  ...
  /// [#fetchIndexLoadingConfig()], which is also called incidentally during state transitions and reloads merely
  /// to obtain the index loading config; this callback is invoked only on a genuine config/schema change.
  ///
  /// The default implementation refreshes the cached table config and schema (by fetching them from ZK), which
  /// preserves the historical behavior of the refresh path. Implementations that need to react to a config/schema
  /// change beyond refreshing the cache should override this method and additionally invoke `super`, or the
  ...
  ///
  /// @return List of [StaleSegment] with segment names and reason why it is stale
