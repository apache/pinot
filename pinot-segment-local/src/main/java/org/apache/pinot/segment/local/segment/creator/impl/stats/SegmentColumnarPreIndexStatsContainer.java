 ...
 * reads column values using PinotSegmentColumnReader. For new columns, uses default values.
  ...
  private void collectStatsFromExistingColumn(String columnName, AbstractColumnStatisticsCollector statsCollector) {
    ...
    try (PinotSegmentColumnReader columnReader = new PinotSegmentColumnReader(_sourceSegment, columnName)) {
      for (int docId = 0; docId < _totalDocCount; docId++) {
        ...
        // Convert value to target data type if necessary
        Object convertedValue = convertValueToTargetDataType(value, targetDataType, columnName);
        statsCollector.collect(convertedValue);
      }
    } catch (Exception e) {
