...
public class VectorIndexUtils {
  ...
  /// Deliberately excludes the IVF `*.combined.index` extensions — those are the transient
  /// consolidated form that the converter is supposed to pack into `columns.psf` via the
  /// standard copy path, not sibling-copy. Callers that need to know about the combined form too
  /// should use [#hasCombinedFormVectorIndex] alongside this.
  ...
  /// `.vector.ivfpq.combined.index`). The combined form is written by an IVF creator run
  /// with `storeInSegmentFile=true` and is meant to be packed into `columns.psf` by
  ...
  /// Returns the `columns.psf` typed-entry buffer holding the column's consolidated vector
  /// index, or `null` when no such entry has been packed into `columns.psf` yet.
  ///
  /// **Contract per store implementation.** On a V3 [SingleFileIndexDirectory], unlike
  /// [SegmentDirectory.Reader#hasIndexFor], this does NOT report a legacy on-disk sidecar (an
  ...
  /// [SingleFileIndexDirectory#INDEX_NOT_FOUND_MESSAGE_PREFIX]) maps to `null`. On a
  /// V1/V2 [FilePerIndexDirectory] there is no `columns.psf`: a still-present legacy
  /// HNSW Lucene *directory* fails to map (message ending with
  /// [FilePerIndexDirectory#NOT_A_REGULAR_FILE_MESSAGE_SUFFIX]) and also maps to `null`
  ...
  /// segment version or on-disk artifacts first, as the `VectorIndexHandler` (v3 gate) and
  /// the HNSW reader factory (legacy-directory-first) call sites do.
  ///
  /// Any other `RuntimeException` (e.g. a corruption marker) is rethrown rather than masked
  /// as "absent", so the migration / crash-recovery paths do not proceed on a broken segment.
  /// Genuine [IOException]s also propagate.
  ///
  /// Callers use this to tell "a prior absorb already committed bytes" (crash recovery) apart from
  /// "first absorb of an existing sidecar", and to select the `columns.psf` read path only
  ...
  /// Sniffs the vector backend that produced a consolidated `columns.psf` payload from its
  /// leading magic bytes, or `null` when the column has no consolidated entry or the payload
  /// starts with an unknown magic. Complements [#detectVectorIndexBackend], which only sees
  ...
  ///
  /// `IVF_ON_DISK` shares the IVF_FLAT file format, so its payloads report
  /// [VectorBackendType#IVF_FLAT] — mirroring what [#detectVectorIndexBackend] reports
  ...
  /// Maps a configured backend to the storage format it reads/writes. `IVF_ON_DISK` shares
  ...
  /// a valid `IVF_ON_DISK` column would report perpetual drift (the payload always sniffs as
  /// `IVF_FLAT`) and be destroyed and rebuilt on every segment load.
  ...
  /// `LUCENE_V2` magic string, IVF payloads start with their 4-byte big-endian format magic
  /// (`IVFF` / `IVPQ`). Returns `null` for anything else. Bytes are assembled
  ...
  ///
  /// @param combined when `true`, returns the combined-form extension used when
  ///                 `storeInSegmentFile=true` (consumed by the V2→V3 converter, then
  ///                 removed). When `false`, returns the legacy file extension that
  ///                 remains alongside `columns.psf`.
  ...
  ///                 the same `LUCENE_V2` layout the text index uses; the legacy form is
