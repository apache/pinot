...
public class MutableJsonIndexImpl implements MutableJsonIndex {
  ...
  private RoaringBitmap getMatchingFlattenedDocIds(Predicate predicate) {
    ...
    Pair<String, RoaringBitmap> pair = getKeyAndFlattenedDocId(key);
    key = pair.getLeft();
  ...
  public Map<String, RoaringBitmap> getMatchingFlattenedDocsMap(String jsonPathKey) {
    ...
    _readLock.lock();
    try {
      Pair<String, RoaringBitmap> result = getKeyAndFlattenedDocId(jsonPathKey);
   ...
   */
  private Pair<String, RoaringBitmap> getKeyAndFlattenedDocId(String key) {
