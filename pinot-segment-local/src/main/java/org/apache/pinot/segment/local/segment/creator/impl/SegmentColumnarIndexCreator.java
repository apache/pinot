...
public class SegmentColumnarIndexCreator implements SegmentCreator {
  ...
  public static void addColumnMetadataInfo(PropertiesConfiguration properties, String column,
    ...
    if (dataType.equals(DataType.STRING)) {
      // for String, we should set the maxLength value always.
      // if maxLengthExceedStrategy is NO_ACTION, then we should set that as well so that String value doesn't get
      // trimmed by default
      // we don't need to set anything for JSON and BYTES, as default action is NO_ACTION so whatever data is
      // originally there for these dataType columns will stay as is.
      properties.setProperty(getKeyFor(column, SCHEMA_MAX_LENGTH), fieldSpec.getMaxLength());
      if (maxLengthExceedStrategy != null && maxLengthExceedStrategy.equals(
