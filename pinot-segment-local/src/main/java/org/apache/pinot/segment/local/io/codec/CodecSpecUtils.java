...
/// Utilities for classifying codec DSL specs into legacy-compatible chunk compression versus full
/// codec-pipeline transforms. Alongside [CodecPipelineExecutor], this is the only public entry
/// point into the otherwise package-private codec runtime: index factories use it to decide
/// whether a `codecSpec` can be served by the existing raw forward-index formats or needs the
/// codec-pipeline (V7) format.
///
...
public final class CodecSpecUtils {
  ...
  /// Returns `true` when the spec contains at least one transform stage (e.g. `DELTA`, `T64`).
  /// Transform stages always require the codec-pipeline format.
  public static boolean hasTransform(String codecSpec) {
    return hasTransform(CodecSpecParser.parse(codecSpec), CodecRegistry.DEFAULT);
  }

  /// Returns `true` when the parsed pipeline contains at least one transform stage.
  static boolean hasTransform(CodecPipeline pipeline, CodecRegistry registry) {
  ...
  public static ChunkCompressionType toLegacyChunkCompressionType(CodecPipeline pipeline) {
    ...
    // CodecInvocation.name() is normalized to upper case by the parser.
