...
public final class VarByteChunkSVForwardIndexReader extends BaseChunkForwardIndexReader {
  ...
  public String getMapEntryValueAsString(int docId, ChunkReaderContext context, PreparedMapKey key) {
    return MapUtils.deserializeMapEntryValueAsString(getBytes(docId, context), key);
  }

  @Override
  public void getMapEntryValuesAsString(int docId, ChunkReaderContext context, PreparedMapKey[] keys,
      String[] values) {
    MapUtils.deserializeMapEntryValuesAsString(ByteBuffer.wrap(getBytes(docId, context)), keys, values);
  }
