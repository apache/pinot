...
public class StatelessRealtimeSegmentWriter implements Closeable {
  ...
  public StatelessRealtimeSegmentWriter(SegmentZKMetadata segmentZKMetadata, IndexLoadingConfig indexLoadingConfig,
    ...
    // Initialize mutable segment with configurations. StatelessRealtimeSegmentWriter is a one-shot tool, so the
    // fallback path relies on the error log instead of emitting a server metric.
