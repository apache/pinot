...
public class SegmentIndexCreationDriverImpl implements SegmentIndexCreationDriver {
   ...
   * <p>This method is optimized for immutable-to-immutable segment conversion where data is not re-sorted, not re-transformed (skipping row transformations)
   * and stats are collected(skipping row transformations) by reading in columnar fashion. The segments are also built in a columnar fashion
   * similar to {@link #buildByColumn(IndexSegment)}.
