...
/// Utility for reading an HNSW vector index from a combined buffer ([PinotDataBuffer]).
///
/// The combined buffer uses the same `LUCENE_V2` layout as the text index (written by
/// `HnswVectorIndexCombined`). This class delegates header and metadata parsing to
/// [LuceneTextIndexBufferReader], then constructs a [PinotBufferLuceneDirectory] that
/// Lucene's `DirectoryReader` can open without any filesystem access.
///
/// Two entry-points are provided:
///   - [#createLuceneDirectory] — a read-only [Directory] over the Lucene index
///     files embedded in the buffer.
///   - [#extractDocIdMappingBuffer] — a sub-buffer view covering just the docId mapping
///     bytes, or `null` if the mapping was not packed.
public final class HnswVectorIndexBufferReader {
  ...
  /// Creates a Lucene [Directory] that reads all Lucene index files from a combined HNSW
  ...
  /// excluded from the directory view so that `DirectoryReader` does not see an unknown file.
  ///
  ...
  /// @return a read-only [Directory] backed by the buffer
  ...
  ///
  /// Returns `null` when the mapping was not included (e.g. the index was built from an
  ...
  ///
  /// The returned sub-buffer is a [`view`][PinotDataBuffer#view] of the original buffer and
  /// shares its lifetime. The caller must not close it independently.
  ///
  /// The view is forced to [ByteOrder#LITTLE_ENDIAN] regardless of the combined buffer's
  /// order. The docId mapping is packed verbatim from the file-backed sidecar, which
  /// `DocIdTranslator` always writes/reads little-endian; the enclosing `columns.psf`
  ...
  /// @return little-endian sub-buffer for the mapping bytes, or `null` if not present
