...
/// Normalizes a single column value read from a `ColumnReader` into the canonical form the
...
/// the raw [FieldSpec#getDefaultNullValue()] rather than the row-major time-range-validated /
...
public final class ColumnarValueNormalizer {
  ...
  /// @param destDataType the column's destination [PinotDataType], i.e.
  ///        `PinotDataType.getPinotDataTypeForIngestion(fieldSpec)` — pass it in pre-computed so
