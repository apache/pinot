      ...
      for (Map.Entry<String, Object> mapValueEntry : mapValue.entrySet()) {
        ...
        if (keyStats instanceof BigDecimalColumnPreIndexStatsCollector) {
          try {
            ...
            LOGGER.warn("Could not parse map key '{}' value '{}' as BIG_DECIMAL; promoting key stats to STRING collector",
                key, value);
        ...
        if (keyStats instanceof IntColumnPreIndexStatsCollector) {
          try {
            ...
            LOGGER.warn(
                "Could not parse map key '{}' value '{}' as INT; promoting key stats to STRING collector for mixed-type values",
                key, value);
            keyStats = promoteNumericKeyStatsToStringCollector(key, value);
          }
          continue;
        ...
        if (keyStats instanceof LongColumnPreIndexStatsCollector) {
          try {
            ...
            LOGGER.warn(
                "Could not parse map key '{}' value '{}' as LONG; promoting key stats to STRING collector for mixed-type values",
                key, value);
            keyStats = promoteNumericKeyStatsToStringCollector(key, value);
          }
          continue;
        ...
        if (keyStats instanceof FloatColumnPreIndexStatsCollector) {
          try {
            ...
            LOGGER.warn(
                "Could not parse map key '{}' value '{}' as FLOAT; promoting key stats to STRING collector for mixed-type values",
                key, value);
            keyStats = promoteNumericKeyStatsToStringCollector(key, value);
          }
          continue;
        ...
        if (keyStats instanceof DoubleColumnPreIndexStatsCollector) {
          try {
            ...
            LOGGER.warn(
                "Could not parse map key '{}' value '{}' as DOUBLE; promoting key stats to STRING collector for mixed-type values",
                key, value);
            keyStats = promoteNumericKeyStatsToStringCollector(key, value);
          }
          continue;
