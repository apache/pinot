...
abstract class OffHeapByteVectorValues extends ByteVectorValues
  ...
  protected final byte[] _binaryValue;
  ...
  OffHeapByteVectorValues(int dimension, int size, IndexInput slice, int byteSize) {
    ...
    _slice = slice;
    _byteSize = byteSize;
    _bytebuffer = ByteBuffer.allocate(byteSize);
    _binaryValue = _bytebuffer.array();
  }
  ...
  private void readValue(int targetOrd)
      throws IOException {
    _slice.seek((long) targetOrd * _byteSize);
    _slice.readBytes(_bytebuffer.array(), _bytebuffer.arrayOffset(), _byteSize);
  }

  static OffHeapByteVectorValues load(
      Lucene95HnswVectorsReader.FieldEntry fieldEntry, IndexInput vectorData)
      throws IOException {
    if (fieldEntry._docsWithFieldOffset == -2 || fieldEntry._vectorEncoding != VectorEncoding.BYTE) {
    ...
    int byteSize = fieldEntry._dimension;
