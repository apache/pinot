...
public class IvfPqVectorIndexReader implements VectorIndexReader, NprobeAware {
  ...
  public IvfPqVectorIndexReader(String column, File indexDir, VectorIndexConfig config) {
    ...
    try (DataInputStream in = new DataInputStream(new FileInputStream(indexFile))) {
      ...
      Preconditions.checkState(magic == IvfPqVectorIndexCreator.MAGIC,
          "Invalid IVF_PQ magic: 0x%s, expected 0x%s", Integer.toHexString(magic),
          Integer.toHexString(IvfPqVectorIndexCreator.MAGIC));

      int version = in.readInt();
      Preconditions.checkState(version == IvfPqVectorIndexCreator.FORMAT_VERSION,
          "Unsupported IVF_PQ format version: %s, expected: %s", version, IvfPqVectorIndexCreator.FORMAT_VERSION);
      ...
      _trainSampleSize = in.readInt();
      _trainingSeed = in.readLong();

      _distanceFunction = IvfPqVectorIndexCreator.stableIdToDistanceFunction(distanceFunctionId);

      Preconditions.checkState(_pqM > 0 && _pqM <= _dimension,
          "Invalid pqM=%s for dimension=%s", _pqM, _dimension);
      Preconditions.checkState(_pqNbits > 0 && _pqNbits <= 8,
          "Invalid pqNbits=%s in IVF_PQ index", _pqNbits);

      _subvectorLengths = VectorQuantizationUtils.computeSubvectorLengths(_dimension, _pqM);
      ...
      for (int c = 0; c < _nlist; c++) {
        ...
        for (int d = 0; d < _dimension; d++) {
          ...
          centroidNormSquare += _centroids[c][d] * _centroids[c][d];
        }
        _centroidNormSquares[c] = centroidNormSquare;
      }

      _codebooks = new float[_pqM][][];
      _codebookNormSquares = new float[_pqM][];
      _listDocIds = new int[_nlist][];
      _listCodes = new byte[_nlist][][];
      if (_numVectors > 0 && _nlist > 0) {
        for (int m = 0; m < _pqM; m++) {
          int subvectorLength = in.readInt();
          int codebookSize = in.readInt();
          Preconditions.checkState(subvectorLength == _subvectorLengths[m],
              "Unexpected subvector length for subquantizer %s: got %s, expected %s", m, subvectorLength,
              _subvectorLengths[m]);
          Preconditions.checkState(codebookSize == (1 << _pqNbits),
              "Unexpected codebook size for subquantizer %s: got %s, expected %s", m, codebookSize, 1 << _pqNbits);
          _codebooks[m] = new float[codebookSize][subvectorLength];
          _codebookNormSquares[m] = new float[codebookSize];
          for (int code = 0; code < codebookSize; code++) {
            float codeNormSquare = 0.0f;
            for (int d = 0; d < subvectorLength; d++) {
              _codebooks[m][code][d] = in.readFloat();
              codeNormSquare += _codebooks[m][code][d] * _codebooks[m][code][d];
            }
        ...
        for (int c = 0; c < _nlist; c++) {
          ...
          _listCodes[c] = new byte[listSize][_pqM];
          for (int i = 0; i < listSize; i++) {
            _listDocIds[c][i] = in.readInt();
          }
          for (int i = 0; i < listSize; i++) {
            in.readFully(_listCodes[c][i]);
          }
        }
      } else {
        for (int m = 0; m < _pqM; m++) {
          _codebooks[m] = new float[0][0];
          _codebookNormSquares[m] = new float[0];
        }
        for (int c = 0; c < _nlist; c++) {
          _listDocIds[c] = new int[0];
          _listCodes[c] = new byte[0][_pqM];
        }
      }
  ...
  public MutableRoaringBitmap getDocIds(float[] searchQuery, int topK) {
    ...
    for (int probe : probeCentroids) {
      ...
      if (_distanceFunction == VectorIndexConfig.VectorDistanceFunction.EUCLIDEAN
        ...
        float[][] tables =
            VectorQuantizationUtils.buildL2DistanceTables(queryResidual, _codebooks, _subvectorLengths);
