...
///
/// For multiple records with the same comparison value (default to timestamp), the manager will preserve the latest
...
///
/// There will be short term inconsistency when updating the upsert metadata, but should be consistent after the
/// operation is done:
///
...
public interface PartitionUpsertMetadataManager extends Closeable {
  ...
  /// TODO: Revisit this and see if we can use the same IndexLoadingConfig for all segments. Tier info might be
