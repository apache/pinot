...
public class ZoneAwareSegmentAssignmentStrategy implements SegmentAssignmentStrategy {
  ...
  public void init(HelixManager helixManager, TableConfig tableConfig) {
    ...
    LOGGER.info("Initialized ZoneAwareSegmentAssignmentStrategy for table: {}", tableConfig.getTableName());
  }

  @Override
  public List<String> assignSegment(String segmentName, List<String> instances, Map<String, Map<String, String>> instanceStates,
      Map<String, InstanceConfig> instanceConfigMap, int numReplicas) {
    // Group instances by availability zone
    Map<String, List<InstanceInfo>> instancesByZone = groupInstancesByZone(instances, instanceConfigMap, instanceStates);
    ...
    LOGGER.info("Assigning segment {} with {} replicas across {} availability zones", segmentName, numReplicas, availableZones);
    
    // Assign replicas to zones, prioritizing distribution across zones
    List<String> selectedInstances = new ArrayList<>(numReplicas);
    
    // Case: We have at least as many zones as replicas (ideal scenario)
    if (availableZones >= numReplicas) {
      ...
      int remainingReplicas = numReplicas - availableZones;
      int zoneIndex = 0;
  ...
  private Map<String, List<InstanceInfo>> groupInstancesByZone(List<String> instances, 
    ...
    for (String instanceName : instances) {
      ...
      int segmentCount = 0;
      Map<String, String> instanceStateMap = instanceStates.get(instanceName);
      if (instanceStateMap != null) {
        segmentCount = instanceStateMap.size();
      }
