...
public class PredicateParseResult {
  ...
  public String toString() {
    return "PredicateParseResult{" + "dims" + _candidateDims + ", " + _iteratorEvalPriorityEnum + ", "
        + _recommendationPriorityEnum + ", nESI=" + String.format("%.3f", _nESI) + ", selected="
        + String.format("%.3f", _percentSelected) + ", nESIWithIdx=" + String.format("%.3f", _nESIWithIdx) + '}';
  }
  ...
  public static PredicateParseResult EMPTY_PREDICATE_PARSE_RESULT() {
    return PredicateParseResultBuilder.aPredicateParseResult().setCandidateDims(FixedLenBitset.IMMUTABLE_EMPTY_SET)
        ...
        .setRecommendationPriorityEnum(
            QueryInvertedSortedIndexRecommender.RecommendationPriorityEnum.NON_CANDIDATE_SCAN)
        .setnESI(NESI_ZERO).setPercentSelected(PERCENT_SELECT_ZERO).setnESIWithIdx(NESI_ZERO).build();
  }
}
