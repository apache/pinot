...
public class PinotLLCRealtimeSegmentManager {
  ...
  public void setUpNewTable(TableConfig tableConfig, IdealState idealState) {
    ...
    for (PartitionGroupMetadata partitionGroupMetadata : newPartitionGroupMetadataList) {
      String segmentName = setupNewPartitionGroup(tableConfig, streamConfig, partitionGroupMetadata, currentTimeMs,
  ...
  private void commitSegmentMetadataInternal(String realtimeTableName,
    ...
    Set<Integer> newPartitionGroupSet = newPartitionGroupMetadataList.stream()
        .map(PartitionGroupMetadata::getPartitionGroupId).collect(Collectors.toSet());
    int numPartitionGroups = newPartitionGroupMetadataList.size();
  ...
  private boolean isPeerURL(String segmentLocation) {
    return segmentLocation != null
        && segmentLocation.toLowerCase().startsWith(CommonConstants.Segment.PEER_SEGMENT_DOWNLOAD_SCHEME);
  }
  ...
  IdealState ensureAllPartitionsConsuming(TableConfig tableConfig, PartitionLevelStreamConfig streamConfig,
          ...
          if (latestSegmentZKMetadata.getStatus() == Status.DONE) {
            ...
            if (newPartitionGroupSet.contains(partitionGroupId)) {
              ...
              CommittingSegmentDescriptor committingSegmentDescriptor = new CommittingSegmentDescriptor(
                  latestSegmentName, (offsetFactory.create(latestSegmentZKMetadata.getEndOffset()).toString()), 0);
    ...
    for (PartitionGroupMetadata partitionGroupMetadata : newPartitionGroupMetadataList) {
      ...
      if (!latestSegmentZKMetadataMap.containsKey(partitionGroupId)) {
        String newSegmentName = setupNewPartitionGroup(tableConfig, streamConfig, partitionGroupMetadata, currentTimeMs,
  ...
  private StreamPartitionMsgOffset getPartitionGroupSmallestOffset(StreamConfig streamConfig, int partitionGroupId) {
    ...
    streamConfigMapWithSmallestOffsetCriteria.put(
        StreamConfigProperties.constructStreamProperty(streamConfig.getType(),
