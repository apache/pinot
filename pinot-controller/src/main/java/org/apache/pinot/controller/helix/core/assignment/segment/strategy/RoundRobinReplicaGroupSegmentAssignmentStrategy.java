...
/// It can be used in the same scenarios as [ReplicaGroupSegmentAssignmentStrategy], except the new segment will
...
///
/// When the number of instances in each replica group increases, this is useful when we don't care about the
/// existing segments as they might be removed later by retention, or there are significantly unbalanced demands on
/// newer segments. This strategy works better than [ReplicaGroupSegmentAssignmentStrategy] since that strategy
/// may require a bootstrap rebalance in this case.
///
/// The round-robin counter is maintained per table and partition in memory and does not sync across controller
