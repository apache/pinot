...
public class PinotInstanceAssignmentRestletResource {
  ...
  public Map<String, InstancePartitions> assignInstances(
    ...
    if (!dryRun) {
      ...
      // Add the new set of assigned instances to the replica group information maintained in the ideal state.
  ...
  public Map<String, InstancePartitions> setInstancePartitions(
    ...
    if (instancePartitionsType != null) {
      // Update ideal state RG information with new set of instances
      _resourceManager.combineInstancePartitionsInIdealState(
          TableNameBuilder.forType(tableType).tableNameWithType(tableName), List.of(instancePartitions));
