  ...
  public Map<String, List<SegmentSizeInfo>> getSizeDetailsFromServers(BiMap<String, String> serverEndPoints,
    ...
    for (int i = 0; i < serverUrls.size(); i++) {
      ...
      try {
        ...
        LOGGER.warn("Interrupted exception while reading segment size for table: {}. Host endpoint: {}", table, url, e);
      } catch (ExecutionException e) {
        if (Throwables.getRootCause(e) instanceof SocketTimeoutException) {
          LOGGER.warn("Server request to read table size was timed out for table: {}. Host endpoint: {}", table, url, e);
        } else if (Throwables.getRootCause(e) instanceof ConnectTimeoutException) {
          LOGGER.warn("Server request to read table size timed out waiting for connection. table: {}. Host endpoint: {}", table, url, e);
        } else if (Throwables.getRootCause(e) instanceof ConnectionPoolTimeoutException) {
          LOGGER.warn("Server request to read table size timed out on getting a connection from pool, table: {}. Host endpoint: {}", table, url, e);
        } else {
          LOGGER.warn("Execution exception while reading segment sizes for table: {}. Host endpoint: {}", table, url, e);
        }
      } catch(Exception e) {
        LOGGER.warn("Error while reading segment sizes for table: {}. Host endpoint: {}", table, url);
      } finally {
        if (getMethod != null) {
