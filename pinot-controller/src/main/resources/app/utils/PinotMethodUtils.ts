...
const normalizeSqlResponse = (responseData: SQLResult) => {
  const queryResponse = getAsObject(responseData);
  ...
  return {
    ...
    queryStats: {
      ...
      records: [
        [
          queryResponse.timeUsedMs,
          queryResponse.numDocsScanned,
          queryResponse.totalDocs,
          queryResponse.numServersQueried,
          queryResponse.numServersResponded,
          queryResponse.numSegmentsQueried,
          queryResponse.numSegmentsProcessed,
          queryResponse.numSegmentsMatched,
          queryResponse.numConsumingSegmentsQueried,
          queryResponse.numEntriesScannedInFilter,
          queryResponse.numEntriesScannedPostFilter,
          queryResponse.numGroupsLimitReached,
          queryResponse.numGroupsWarningLimitReached,
          queryResponse.partialResponse ? queryResponse.partialResponse : '-',
          queryResponse.minConsumingFreshnessTimeMs,
...
const getQueryResults = (params) => {
  return getQueryResult(params).then(({ data }) => normalizeSqlResponse(data));
};

const getQueryLogResults = (params) => {
  return getQueryLogResult(params).then(({ data }) => normalizeSqlResponse(data));
};
