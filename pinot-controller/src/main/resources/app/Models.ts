...
/**
 * Central type/enum module for Pinot Controller UI.
 *
 * Previously declared as an ambient module in interfaces/types.d.ts so that
 * webpack (via resolve.modules) could resolve the bare 'Models' specifier.
 * Vite uses an explicit alias in vite.config.ts instead, so this is now a
 * real TypeScript module that can be imported normally.
 *
 * NOTE: All `const enum` declarations have been converted to regular `enum`
 * because esbuild (used by Vite) does not support cross-file const enum
 * inlining (see https://esbuild.github.io/content-types/#typescript-caveats).
 */

// ---------------------------------------------------------------------------
// Runtime enums (were const enum – values now live at runtime, not inlined)
// ---------------------------------------------------------------------------
/* START GENAI */
...
export enum DISPLAY_SEGMENT_STATUS {
  BAD = 'BAD',
  GOOD = 'GOOD',
  UPDATING = 'UPDATING',
}
...
export enum InstanceStatus {
  ...
  QUERIES_DISABLED = 'QUERIES_DISABLED',
}
...
export type RebalanceTableSegmentJob = {
  jobId: string;
  messageCount: number;
  submissionTimeMs: number;
  jobType: string;
