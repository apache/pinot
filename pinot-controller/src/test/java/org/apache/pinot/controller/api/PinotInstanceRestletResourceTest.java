...
public class PinotInstanceRestletResourceTest extends ControllerTest {
  ...
  public void testMinionStatusEndpoint()
    ...
    String allMinionsUrl = _urlBuilder.forMinionStatus(null, 0);
    ...
    String drainedMinionsUrl = _urlBuilder.forMinionStatus("DRAINED", 0);
    ...
    String onlineMinionsUrl = _urlBuilder.forMinionStatus("ONLINE", 0);
    ...
    // Test 5: Get with limit
    String limitedUrl = _urlBuilder.forMinionStatus(null, 2);
    String limitedResponse = sendGetRequest(limitedUrl);
    JsonNode limitedJson = JsonUtils.stringToJsonNode(limitedResponse);

    assertEquals(limitedJson.get("currentMinionCount").asInt(), 2);
    assertEquals(limitedJson.get("minionStatus").size(), 2);

    // Test 6: Get DRAINED with limit
    String drainedLimitedUrl = _urlBuilder.forMinionStatus("DRAINED", 1);
    String drainedLimitedResponse = sendGetRequest(drainedLimitedUrl);
    JsonNode drainedLimitedJson = JsonUtils.stringToJsonNode(drainedLimitedResponse);

    assertEquals(drainedLimitedJson.get("currentMinionCount").asInt(), 1);
    assertEquals(drainedLimitedJson.get("minionStatus").size(), 1);
    assertEquals(drainedLimitedJson.get("minionStatus").get(0).get("status").asText(), "DRAINED");
  ...
  public void testMinionStatusEndpointCaseInsensitive()
    ...
    String upperCaseUrl = _urlBuilder.forMinionStatus("DRAINED", 0);
    String lowerCaseUrl = _urlBuilder.forMinionStatus("drained", 0);
