...
public class PinotTableReloadServiceTest {
  ...
  public void testGetServerToSegments() {
    ...
    Map<String, List<String>> serverToSegmentsMap = _service.getServerToSegments(tableName, null, null);
    assertEquals(serverToSegmentsMap, fullServerToSegmentsMap);

    // Get all segments on svr02.
    serverToSegmentsMap = _service.getServerToSegments(tableName, null, "svr02");
    assertEquals(serverToSegmentsMap, Map.of("svr02", List.of("seg02", "seg03")));

    // Get all servers with seg01.
    serverToSegmentsMap = _service.getServerToSegments(tableName, "seg01", null);
    ...
    serverToSegmentsMap = _service.getServerToSegments(tableName, "seg01", "svr01");
    assertEquals(serverToSegmentsMap, Map.of("svr01", List.of("seg01")));
    serverToSegmentsMap = _service.getServerToSegments(tableName, "anySegment", "anyServer");
    assertEquals(serverToSegmentsMap, Map.of("anyServer", List.of("anySegment")));
    serverToSegmentsMap = _service.getServerToSegments(tableName, "seg01|seg02", "svr02");
