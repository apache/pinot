...
public class PinotTaskManagerStatelessTest extends ControllerTest {
  ...
  @Test
  public void testPinotTaskManagerSchedulerWithStoppedTaskQueue()
      throws Exception {
    Map<String, Object> properties = getDefaultControllerConfiguration();
    properties.put(ControllerConf.ControllerPeriodicTasksConf.PINOT_TASK_MANAGER_SCHEDULER_ENABLED, true);
    startController(properties);
    ...
    // Schedule tasks for table
    List<String> taskIDs =
        taskManager.scheduleTaskForTable(segmentGenerationAndPushTask, tableConfig.getTableName(), null);
    assert taskIDs != null;
    assertEquals(taskIDs.size(), 0);
