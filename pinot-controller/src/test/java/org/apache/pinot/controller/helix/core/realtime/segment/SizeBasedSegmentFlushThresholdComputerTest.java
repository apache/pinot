...
public class SizeBasedSegmentFlushThresholdComputerTest {
  ...
  public void testCommitTimeCompactionWithHighCompactionRatio() {
    ...
    // Should use pre-commit rows (100,000) for calculation, not post-commit (20,000)
  ...
  public void testCommitTimeCompactionWithMediumCompactionRatio() {
    ...
    // Should use pre-commit rows (60,000) for calculation
    // (60,000 / 250,000) * 400,000 = 96,000
    assertEquals(threshold, 96_000);
    assertEquals(computer.getSegmentRowsToSizeRatio(), 0.24); // 60,000 / 250,000
  }
  ...
  public void testCommitTimeCompactionWithLowCompactionRatio() {
    ...
    // Should use pre-commit rows (45,000) for calculation
    // (45,000 / 180,000) * 350,000 = 87,500
    assertEquals(threshold, 87_500);
    assertEquals(computer.getSegmentRowsToSizeRatio(), 0.25); // 45,000 / 180,000
  }
  ...
  public void testCommitTimeCompactionWithTimeThresholdHit() {
    ...
    // Should use pre-commit rows (80,000) * 1.1 = 88,000 since time threshold not reached
    assertEquals(threshold, 88_000);
  }
  ...
  public void testRatioCalculationProgressionWithCommitTimeCompaction() {
    ...
    // Should use pre-commit rows (60,000) for ratio calculation
    // (0.1 * (60,000 / 200,000)) + (0.9 * 0.2) = (0.1 * 0.3) + (0.9 * 0.2) = 0.03 + 0.18 = 0.21
    assertEquals(secondRatio, 0.21, 0.001);
  }
  ...
  public void testMixedScenariosWithAndWithoutCompaction() {
    ...
    computer.onSegmentCommit(compactedDescriptor, compactedMetadata);
    assertEquals(computer.getSegmentRowsToSizeRatio(), 0.4); // 80,000 / 200,000
    ...
    // Should use post-commit rows (30,000) for traditional segment
    // (0.1 * (30,000 / 200,000)) + (0.9 * 0.4) = (0.1 * 0.15) + (0.9 * 0.4) = 0.015 + 0.36 = 0.375
    assertEquals(computer.getSegmentRowsToSizeRatio(), 0.375, 0.001);

    // Next threshold calculation should work correctly
    int threshold = computer.computeThreshold(streamConfig, "newSegmentName");
    // (300,000 * 0.375) = 112,500
    assertEquals(threshold, 112_500);
  }
