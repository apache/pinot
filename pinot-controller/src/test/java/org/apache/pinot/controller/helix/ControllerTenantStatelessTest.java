...
public class ControllerTenantStatelessTest extends ControllerTest {
  ...
  public void testBrokerTenant()
    ...
    JsonNode response = JsonUtils.stringToJsonNode(
        sendGetRequest(getAdminUrlBuilder().forTenantGet()));
    ...
    for (int i = 1; i <= NUM_BROKER_TAGS; i++) {
      ...
      response = JsonUtils.stringToJsonNode(
          sendGetRequest(getAdminUrlBuilder().forBrokerTenantGet(brokerTag)));
      ...
      response = JsonUtils.stringToJsonNode(
          sendGetRequest(getAdminUrlBuilder().forTenantGet(brokerTag)));
      Assert.assertEquals(response.get("BrokerInstances").size(), NUM_BROKERS_PER_TAG);
    ...
    for (int i = 1; i <= NUM_BROKER_TAGS; i++) {
      String brokerTenant = BROKER_TAG_PREFIX + i;
      sendDeleteRequest(getAdminUrlBuilder().forBrokerTenantDelete(brokerTenant));
      Assert.assertEquals(_helixAdmin
              .getInstancesInClusterWithTag(getHelixClusterName(), TagNameUtils.getBrokerTagForTenant(brokerTenant))
              .size(),
          0);
  ...
  public void testEmptyServerTenant() {
    try {
      sendGetRequest(getAdminUrlBuilder().forServerTenantGet("doesn't_exist"));
      Assert.fail();
    } catch (Exception e) {
    }
  }
  ...
  public void testServerTenant()
    ...
    for (int i = 1; i <= NUM_SERVER_TAGS; i++) {
      ...
      response = JsonUtils.stringToJsonNode(
          sendGetRequest(getAdminUrlBuilder().forServerTenantGet(serverTag)));
      ...
      response = JsonUtils.stringToJsonNode(
          sendGetRequest(getAdminUrlBuilder().forTenantGet(serverTag)));
    ...
    for (int i = 1; i < NUM_SERVER_TAGS; i++) {
      String serverTenant = SERVER_TAG_PREFIX + i;
      sendDeleteRequest(getAdminUrlBuilder().forServerTenantDelete(serverTenant));
      Assert.assertEquals(_helixAdmin
          .getInstancesInClusterWithTag(getHelixClusterName(), TagNameUtils.getOfflineTagForTenant(serverTenant))
          .size(), 0);
      Assert.assertEquals(_helixAdmin
