...
|---|---|---|
| [`/lint`](lint/SKILL.md) | Run all pre-commit quality checks (formatting, license, style, compiler warnings via `-Xlint:all`) on only the modules touched by the diff. | 30–120s warm, up to 5min cold |
...
- Compiler warnings are filtered to changed files only — pre-existing warnings in unchanged code are not reported.
