...
public class SegmentOnlineOfflineStateModelFactory extends StateModelFactory<StateModel> {
  ...
  public class SegmentOnlineOfflineStateModel extends StateModel {
    ...
    private void onBecomeOnlineFromOfflineForOfflineSegment(Message message,
        NotificationContext context) {
      // TODO: Need to revisit this part to see if it's possible to add offline segment by just
      // giving
      ...
      try {
        ...
        if (segmentMetadataFromServer == null) {
          ...
          if (new File(localSegmentDir).exists()) {
            ...
            try {
              ...
              LOGGER.error(
                  "Failed to load segment from local, will try to reload it from controller!", e);
              FileUtils.deleteQuietly(new File(localSegmentDir));
              segmentMetadataFromServer = null;
            }
          }
        ...
        if (isNewSegmentMetadata(segmentMetadataFromServer, segmentMetadataForCheck)) {
          if (segmentMetadataFromServer == null) {
            LOGGER
                .info("Loading new segment from controller - " + segmentMetadataForCheck.getName());
          } else {
          ...
          for (retryCount = 0; retryCount < SEGMENT_LOAD_MAX_RETRY_COUNT; ++retryCount) {
            ...
            try {
              ...
              LOGGER.warn("Caught exception while loading segment " + segmentId + ", attempt "
                  + (retryCount + 1) + " of " + SEGMENT_LOAD_MAX_RETRY_COUNT, e);
              ...
              if (retryCount < SEGMENT_LOAD_MAX_RETRY_COUNT) {
                ...
                long waitTime =
                    (long) ((SEGMENT_LOAD_MIN_RETRY_DELAY_MILLIS + attemptDurationMillis)
                        * retryDurationMultiplier);
          ...
          if (SEGMENT_LOAD_MAX_RETRY_COUNT <= retryCount) {
            String msg =
                "Failed to load segment " + segmentId + " after " + retryCount + " retries";
            LOGGER.error(msg);
    ...
    private String downloadSegmentToLocal(String uri, String tableName, String segmentId)
      ...
      if (uri.startsWith("hdfs:")) {
        ...
        try {
          tempSegmentFile = new File(INSTANCE_DATA_MANAGER.getSegmentFileDirectory() + "/"
              + tableName + "/temp_" + segmentId + "_" + System.currentTimeMillis());
          if (uri.startsWith("http:") || uri.startsWith("https:")) {
            tempFile =
                new File(INSTANCE_DATA_MANAGER.getSegmentFileDirectory(), segmentId + ".tar.gz");
          ...
          final File segmentDir = new File(
              new File(INSTANCE_DATA_MANAGER.getSegmentDataDirectory(), tableName), segmentId);
          Thread.sleep(1000);
          if (segmentDir.exists()) {
            LOGGER.info(
                "Deleting the directory and recreating it again- " + segmentDir.getAbsolutePath());
          ...
          Thread.sleep(1000);
          LOGGER
              .info("Was able to succesfully rename the dir to match the segmentId - " + segmentId);
    ...
    private String getSegmentLocalDirectory(String tableName, String segmentId) {
      final String segmentDir =
          INSTANCE_DATA_MANAGER.getSegmentDataDirectory() + "/" + tableName + "/" + segmentId;
      return segmentDir;
    }
