...
public interface PinotFS extends Closeable, Serializable {
  ...
  /// Deletes the files at the locations provided. If the segmentUri is a directory, it will delete the entire
  /// directory.
  ...
  /// Moves the file or directory from the src to dst. Does not keep the original file. If the dst has parent
  /// directories that haven't been created, this method will create all the necessary parent directories. Note: In
  /// Pinot we recommend the full paths of both src and dst be specified. For example, if a file /a/b/c is moved to a
  /// file /x/y/z, in the case of overwrite, the directory /a/b still exists, but will not contain the file 'c'.
  /// Instead, /x/y/z will contain the contents of 'c'. If src is a directory /a/b which contains two files /a/b/c and
  /// /a/b/d, and the dst is /x/y, the result would be that the directory /a/b under /a gets removed and dst directory
  /// contains two files which is /x/y/c and /x/y/d. If src is a directory /a/b needs to be moved under another
  /// directory /x/y, please specify the dst to /x/y/b.
  ...
  /// Lists files with metadata, applying a path filter and stopping after `maxResults` matches.
