...
public final class IngestionConfigUtils {
  ...
  /// Builds a column-to-explicit-transform-function map once so callers do not rescan the ingestion config per column.
  /// Ingestion [TransformConfig]s win; schema-level [FieldSpec#getTransformFunction] is the legacy fallback for
  /// columns not listed in the ingestion config. Evaluators implicitly synthesized from field specs are intentionally
  /// omitted because they have no configured expression that can be persisted as replayable provenance.
  public static Map<String, String> getTransformFunctionByColumn(@Nullable TableConfig tableConfig,
    ...
    if (schema != null) {
      for (FieldSpec fieldSpec : schema.getAllFieldSpecs()) {
        ...
        if (StringUtils.isNotEmpty(transformFunction)) {
