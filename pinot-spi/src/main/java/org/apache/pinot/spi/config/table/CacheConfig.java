...
public class CacheConfig extends BaseJsonConfig {
  ...
  @JsonPropertyDescription("The type of the cache (e.g. 'InMemory')")
  ...
  public CacheConfig(@JsonProperty(value = "enabled") boolean enabled,
    ...
    if (enabled) {
      Preconditions.checkArgument(type != null && !type.isEmpty(),
          "Cache type must be specified when cache is enabled");
      Preconditions.checkArgument(maxSize > 0, "Cache maxSize must be positive when cache is enabled");
      Preconditions.checkArgument(ttl > 0, "Cache ttl must be positive when cache is enabled");
    }
    _enabled = enabled;
    _type = type;
    _maxSize = maxSize;
