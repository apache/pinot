...
/// Registry for [TableConfigValidator] implementations.
/// Supports multiple validators invoked in registration order; the first rejection short-circuits with an error.
/// When no validators are registered, [#validate] is a no-op.
///
/// **Lifecycle:** `BaseControllerStarter#stop()` invokes [#reset()] automatically, so
/// subclasses do **not** need to call it themselves. Validators registered against controller-owned state
/// (e.g. `HelixAdmin`, `HelixManager`, ZK client references) become stale once the controller
/// stops; the automatic `reset()` in `stop()` ensures they are removed before any subsequent
/// `start()` registers replacements. This matters for in-process restart scenarios such as integration
/// tests that reuse a single JVM across multiple controller lifecycles — without it, the next config-mutation
/// request would fail when a stale validator dereferences its torn-down dependencies.
public class TableConfigValidatorRegistry {
  ...
  /// @return `true` if a validator was removed, `false` otherwise
  ...
  /// Helix clients).
  ///
  /// In production this is rarely needed since service shutdown coincides with JVM shutdown. It is essential
