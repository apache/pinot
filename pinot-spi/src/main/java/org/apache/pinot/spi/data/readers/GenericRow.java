...
public class GenericRow implements Serializable {
  ...
  /// Transient, per-row cache of already-parsed JSON values (column -> `Map`/`List`/`JsonNode`).
  ...
  /// instead of re-parsing the serialized string. See [_parsedJsonCache].
  ...
  /// Returns the cached already-parsed JSON value for the given field, or `null` if none was cached.
  @Nullable
  ...
  /// example after a sanitization transformer trims an over-length JSON string). See [_parsedJsonCache].
