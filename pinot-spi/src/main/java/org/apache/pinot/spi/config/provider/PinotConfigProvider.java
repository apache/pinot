...
public interface PinotConfigProvider {
  ...
  /**
   * Registers the {@link LogicalTableChangeListener} and notifies it whenever any changes (addition, update, removal)
   * to any of the logical tables are detected. If the listener is successfully registered,
   * {@link LogicalTableChangeListener#onChange(List)} will be invoked with the current logical tables.
   *
   * @return {@code true} if the listener is successfully registered, {@code false} if the listener is already
   *         registered.
   */
