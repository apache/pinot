...
public class QueryThreadContext implements AutoCloseable {
  ...
  /// Returns the correlation ID of the query associated to the current thread. It should never return `null` in
  /// production code where [QueryThreadContext] has been set up, but could be `null` in tests when [QueryThreadContext]
  /// is not set up.
  @Nullable
  public static String getCid() {
    ...
    // NOTE: In production code, threadContext should never be null. It might be null in tests when QueryThreadContext
    //       is not set up.
    return threadContext != null ? threadContext.getExecutionContext().getCid() : null;
  }
