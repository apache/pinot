...
public final class CompressionCodecSpecParser {
  ...
  public static CompressionCodecSpec parse(String rawSpec) {
    ...
    if (openParenIndex < 0) {
      return CompressionCodecSpec.of(parseCodecName(trimmedSpec, rawSpec));
    }
    ...
    Integer level = parseLevel(levelToken, rawSpec);
    return CompressionCodecSpec.of(parseCodecName(codecToken, rawSpec),
        Collections.singletonMap(CompressionCodecCapabilities.LEVEL_PARAM, Integer.toString(level)));
  }

  private static String parseCodecName(String codecToken, String rawSpec) {
    if (!CODEC_NAME_PATTERN.matcher(codecToken).matches()) {
      throwInvalidSpec(rawSpec);
    }
    return CompressionCodecCapabilities.canonicalizeCodecName(codecToken);
  }

  private static Integer parseLevel(String levelToken, String rawSpec) {
    try {
      return Integer.parseInt(levelToken);
