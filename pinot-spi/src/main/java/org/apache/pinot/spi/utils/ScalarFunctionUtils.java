...
public final class ScalarFunctionUtils {
  ...
  private static final String SCALAR_FUNCTION_PACKAGE_REGEX = ".*\\.function\\..*";
  ...
  public static List<String> getScalarFunctionNames(ScalarFunction scalarFunction, String defaultName) {
    ...
    if (names.length == 0) {
      return List.of(ScalarFunctionUtils.canonicalize(defaultName));
    }

    Set<String> canonicalNames = new LinkedHashSet<>();
    for (String name : names) {
      canonicalNames.add(ScalarFunctionUtils.canonicalize(name));
    }
