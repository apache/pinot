...
oldCommonJar=$(find "${oldTargetDir}/pinot-common/target" -maxdepth 1 -type f -name 'pinot-common-*.jar' \
  ! -name '*-sources.jar' ! -name '*-tests.jar' ! -name '*-javadoc.jar' -print -quit)
if [ -z "${oldCommonJar}" ]; then
  echo "Failed to locate the built old pinot-common artifact under ${oldTargetDir}/pinot-common/target"
  exit 1
fi
if ! oldColumnDataType=$(javap -classpath "${oldCommonJar}" \
    'org.apache.pinot.common.proto.Expressions$ColumnDataType' 2>&1); then
  echo "Failed to inspect ColumnDataType in built old artifact ${oldCommonJar}: ${oldColumnDataType}"
  exit 1
fi
if echo "${oldColumnDataType}" | grep -Eq 'public static final .*ColumnDataType VARIANT;'; then
...
if [ -f "$testSuiteDir/old-broker-new-servers.yaml" ]; then
  ...
  # This phase contains only queries against the data generated by the preceding upgrade phase, so intentionally
  # reuse genNum instead of generating another table/segment set while the old broker is active.
