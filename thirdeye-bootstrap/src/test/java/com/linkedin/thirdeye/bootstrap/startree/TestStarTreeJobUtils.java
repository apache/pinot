...
public class TestStarTreeJobUtils
{
  ...
  public void testFindBestMatch() throws Exception
  {
    ...
    for (String dimension : dimensions)
    {
      ...
      forwardIndex.get(dimension).put(dimension + "X", StarTreeConstants.FIRST_VALUE); // will not be in combos
      for (int i = 0; i < 3; i++)
      {
    ...
    int[] oneOther = new int[] {
            StarTreeConstants.OTHER_VALUE,
            forwardIndex.get("B").get("B2"),
            forwardIndex.get("C").get("C0")
    };
    int[] twoOther = new int[] {
            StarTreeConstants.OTHER_VALUE,
            forwardIndex.get("B").get("B0"),
            StarTreeConstants.OTHER_VALUE
    };

    // No match, so should be all other
    int[] res = StarTreeJobUtils.findBestMatch(
            new DimensionKey(new String[] { "AX", "BX", "CX" }), dimensions, combinations, forwardIndex);
    Assert.assertEquals(res, allOther);

    // Should be one other
    res = StarTreeJobUtils.findBestMatch(
            new DimensionKey(new String[] { "AX", "B2", "C0" }), dimensions, combinations, forwardIndex);
    Assert.assertEquals(res, oneOther);

    // Should be two other
    res = StarTreeJobUtils.findBestMatch(
            new DimensionKey(new String[] { "AX", "B0", "C1" }), dimensions, combinations, forwardIndex);
