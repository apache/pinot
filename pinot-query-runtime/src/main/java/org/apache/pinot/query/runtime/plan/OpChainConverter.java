...
/// Extension point for alternative plan-to-[OpChain] converters. The active implementation is chosen by
/// [OpChainConverterDispatcher]: highest [#priority()] wins unless an override is set (for product
...
public interface OpChainConverter {
  ...
  /// unless [OpChainConverterDispatcher#setActiveConverterIdOverride] is set. Ties break lexicographically by
  /// [#converterId()] (ascending).
  ///
  ...
  /// @param tracker A tracker that must be called every time a PlanNode is converted into an operator, so the caller
  ///                can keep track of this mapping.
  ...
  ///
  /// This is used, for example, when a pipeline breaker detects an error and needs to propagate it to downstream
  /// stages.
  ///
  /// The default implementation uses a Java [ErrorOperator] + [MailboxSendOperator] pipeline.
