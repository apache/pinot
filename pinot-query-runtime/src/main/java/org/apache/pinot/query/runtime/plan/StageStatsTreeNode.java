...
/// [org.apache.pinot.common.proto.Worker.StageStatsNode]. Mirrors the proto shape but stores a deserialized
/// [StatMap] so that merging across multiple opchain reports (different workers of the same stage) is just a
/// recursive walk + [StatMap#merge(StatMap)] per node.
///
/// Used by the stream-mode stats reporting path. Mutable: [#merge(StageStatsTreeNode)] sums the other tree's
/// stat maps into this one in place.
public class StageStatsTreeNode {

  /// Thrown by [StageStatsTreeNode#merge(StageStatsTreeNode)] when the two trees disagree on shape (operator type
  /// at any position differs, or children-count differs). The broker catches this, logs, and marks the stage as
  /// `mergeFailed` in the per-stage coverage structure.
  ...
  /// position, same arity). On mismatch this method throws [ShapeMismatchException] and leaves this tree in an
  ...
  /// Flattens this tree into a [MultiStageQueryStats.StageStats.Closed] via inorder traversal (leftmost-leaf
  /// first), mirroring the order in which operators emit their stats today
  /// ([MultiStageQueryStats.StageStats.Open#addLastOperator] call order). The result is compatible with the
  /// legacy [QueryDispatcher.QueryResult] contract that exposes per-stage stats as a flat
  /// `List<StageStats.Closed>`.
  ///
  /// The flattening is lossy in the same way the legacy format is: tree shape is not preserved. Callers that need
  /// tree shape should use this [StageStatsTreeNode] directly.
  ...
  /// Returns a deep copy of this subtree: every node's [StatMap] is cloned via the [StatMap] copy
  /// constructor and children are copied recursively. The `type` and (immutable) `planNodeIds` are shared.
  ///
  /// Used to hand a caller a snapshot of the accumulator that is isolated from subsequent in-place
  /// [#merge(StageStatsTreeNode)] mutations of the live tree — otherwise a late merge on one thread would race
  /// with a reader (e.g. `StatMap.asJson()`) on another, since [StatMap] is not safe for concurrent
  /// read/merge.
