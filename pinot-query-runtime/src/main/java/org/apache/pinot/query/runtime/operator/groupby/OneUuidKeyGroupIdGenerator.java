...
/// (`byte[]`, `ByteArray`, `String` or `UuidKey`) to [UuidKey] — two primitive longs — so map probing avoids byte-array
/// hashing/equality on the hot path. Not thread-safe; each instance is owned by a single operator thread, matching the
/// other [GroupIdGenerator] implementations.
