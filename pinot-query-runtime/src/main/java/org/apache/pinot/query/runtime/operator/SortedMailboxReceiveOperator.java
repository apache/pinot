 ...
 *       between {@link #getNextBlock()} calls. Senders are deliberately allowed to backpressure — that is where the
 *       memory advantage comes from — but no sender is left parked indefinitely: every {@link #refill} takes at most
 *       one element from each sibling stream, and a stalled merge drains ready siblings fully into a per-stream
 *       backlog. Under sustained key skew that backlog trades some of the memory advantage for liveness.</li>
 * </ul>
 * The k-way merge is enabled only when the {@code streamingSortedMailboxReceive} query option is
 * {@code true} <b>and</b> {@link MailboxReceiveNode#isSortedOnSender()} is true. All other combinations fall back to
 * the accumulate-then-sort path. Which path was taken is reported in the query response {@code stageStats} as the
 * {@code kWayMergeUsed} stat (see {@link BaseMailboxReceiveOperator.StatKey#K_WAY_MERGE_USED}).
 *
...
public class SortedMailboxReceiveOperator extends BaseMailboxReceiveOperator {
   ...
   * {@code SortOperator} skips its priority queue when its input is a {@code SortedMailboxReceiveOperator}. If a sender
  ...
  /**
   * Polls every active stream other than {@code state} at most once, non-blocking, pruning any that become exhausted.
   * See {@link #refill} for why this runs unconditionally rather than only when the merge stalls.
   */
   ...
   * so {@link #head()} is valid until {@link #advance()} returns {@code false}.
   */
  private static final class Cursor {
    ...
    /**
     * Advances past the current row. Returns {@code true} if a new head is available in this block.
     */
    boolean advance() {
      _idx++;
      if (_idx < _rows.size()) {
