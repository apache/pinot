...
public abstract class BaseMailboxReceiveOperator extends MultiStageOperator {
  ...
  public enum StatKey implements StatMap.Key {
    ...
    /// Per-upstream-sender wall-clock elapsed time (consumer construction -> EOS arrival), encoded as a
    /// semicolon-separated list of `hostname|mailboxPort=elapsedMs` pairs.
    ///
    /// Populated by every [BaseMailboxReceiveOperator]. Elapsed time is anchored to consumer construction (after
    /// pipeline breakers), avoiding cross-host clock skew. When multiple workers on the same stage contribute stats
    /// for the same sender, the merge function takes the maximum elapsed time so the slowest observation is preserved
    /// for the same sending server.
    ///
    /// On the broker side, [QueryDispatcher#extractMaxTimingsPerInstance] reads this stat and calls
    /// [ServerRoutingStatsManager#recordStatsUponResponseArrival]. Senders with no observed timing are omitted from
    /// the encoding.
