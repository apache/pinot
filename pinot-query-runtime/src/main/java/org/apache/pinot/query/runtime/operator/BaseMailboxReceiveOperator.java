...
public abstract class BaseMailboxReceiveOperator extends MultiStageOperator {
  ...
  public enum StatKey implements StatMap.Key {
     ...
     * <p>
     * Only {@code SortedMailboxReceiveOperator} ever sets this; the unsorted receive operator always leaves it at
     * {@code false}. It is recorded at construction time (not when the first row is merged), so it reports the path
     * the operator was configured to take even when the result is empty.
     * <p>
     * Reporting is presence-based: {@link StatMap} drops boolean keys whose value is {@code false}, so this renders
     * into the response {@code stageStats} as {@code kWayMergeUsed: true} on the merge path and is simply absent on
     * the accumulate-then-sort path (and on any server predating this stat).
     * <p>
     * NOTE: this key is intentionally the last constant in this enum. {@link StatMap} serializes keys by ordinal, so
