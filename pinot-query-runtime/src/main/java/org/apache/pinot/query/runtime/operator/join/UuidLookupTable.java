...
/// Lookup table optimized for Pinot's logical UUID type — stores entries keyed by [UuidKey] (two primitive longs) so
/// the hot path avoids `ByteArray` wrapping/equals.
///
/// **Contract:** [#addRow] normalizes the supplied key implicitly via [#normalizeKey]; [#containsKey] and [#lookup]
/// do NOT — callers must pass an already-normalized key (the join operators do this via
/// `_rightTable.normalizeKey(...)`). Passing a raw `byte[]`, `String`, or `ByteArray` to [#containsKey] / [#lookup]
/// will silently miss because the table is keyed on `UuidKey`, whose equality is by primitive longs.
