...
public class MailboxSendOperator extends MultiStageOperator {
  ...
  /// 2. Then, a single outer spool broadcast exchange is created to fan out the data to all the inner exchanges.
  ///    It copies blocks that carry mutable cells (aggregation intermediate results) so that no two receiver stages
  ///    share them.
  ///
  ...
  private static BlockExchange getBlockExchange(OpChainExecutionContext ctx, MailboxSendNode node,
    ...
    return BlockExchange.getSpoolExchange(perStageSendingMailboxes, mainSplitter, statsIndexChooser);
  }
