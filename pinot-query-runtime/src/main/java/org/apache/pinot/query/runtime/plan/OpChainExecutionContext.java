...
/// The `OpChainExecutionContext` class contains the information derived from the PlanRequestContext.
...
public class OpChainExecutionContext {
  ...
  /// [org.apache.pinot.query.runtime.plan.PlanNodeToOpChain] during opchain construction. Cardinality is
  ...
  /// (`MultiStageStatsTreeEncoder`) to attach plan-node identifiers to each operator's stats.
  ///
  ...
  /// structure, so the ids match without being serialized on the wire. Used by `MultiStageStatsTreeEncoder` to
  /// populate `StageStatsNode.plan_node_ids`.
  ///
  ...
  /// Whether stats for this opchain are reported out-of-band via the `SubmitWithStream` bidi RPC (stream mode),
  /// rather than piggybacked on the mailbox EOS (legacy mode). Set server-side by the `SubmitWithStream` handler
  /// via [CommonConstants.MultiStageQueryRunner#KEY_OF_STATS_REPORTING_MODE]. This is the only mode that consumes
  /// the per-operator plan-node ids, so the id pre-walk in `PlanNodeToOpChain` is gated on it — gating on
  /// [#isSendStats()] instead would both waste work on every legacy query (the default) and, conversely, skip
  /// the walk in stream mode (where `sendStats` is forced off), leaving the ids unpopulated.
  ...
  /// opchain is constructed; subsequent calls overwrite. `planNodes` is captured by reference; callers should not
  ...
  /// [org.apache.pinot.query.runtime.plan.PlanNodeToOpChain] pre-walk.
  ...
  /// Returns the stage-scoped id assigned to the given PlanNode, or `-1` if no id was recorded for it.
  ...
  /// register synthetic stats operators via [#recordPlanNodesForOperator].
