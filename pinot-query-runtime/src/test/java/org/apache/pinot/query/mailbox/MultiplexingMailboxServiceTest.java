...
public class MultiplexingMailboxServiceTest {
  private static final StringMailboxIdentifier localMailboxId = new StringMailboxIdentifier("localJobId", "localhost",
      0, "localhost", 0);
  private static final StringMailboxIdentifier nonLocalMailboxId = new StringMailboxIdentifier("localJobId",
  ...
  public void testHappyPath() {
    ...
    Assert.assertTrue(multiplexService.getSendingMailbox(localMailboxId) instanceof InMemorySendingMailbox);
    Assert.assertTrue(multiplexService.getSendingMailbox(nonLocalMailboxId) instanceof GrpcSendingMailbox);

    Assert.assertTrue(multiplexService.getReceivingMailbox(localMailboxId) instanceof InMemoryReceivingMailbox);
    Assert.assertTrue(multiplexService.getReceivingMailbox(nonLocalMailboxId) instanceof GrpcReceivingMailbox);
