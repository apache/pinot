...
public class QueriesMetadataTest extends QueryRunnerTestBase {
  private static final ObjectMapper MAPPER = new ObjectMapper();
  private static final Pattern TABLE_NAME_REPLACE_PATTERN = Pattern.compile("\\{([\\w\\d]+)\\}");
  private static final String QUERY_TEST_RESOURCE_FOLDER = "metadata_queries";
  ...
  public void setUp()
    ...
    _queryEnvironment =
        QueryEnvironmentTestBase.getQueryEnvironment(_reducerGrpcPort, server1.getPort(), server2.getPort(),
            factory1.buildSchemaMap(), tableToSegmentMap1, tableToSegmentMap2);
    server1.start();
    server2.start();
  ...
  public void testQueryTestCasesWithH2(String testCaseName, String sql, String expect, int numSegments)
      throws Exception {
    ...
    for (Map.Entry<String, QueryTestCase> testCaseEntry : testCaseMap.entrySet()) {
      ...
      for (QueryTestCase.Query queryCase : queryCases) {
        ...
        if (queryCase._outputs == null) {
          ...
          Object[] testEntry =
              new Object[]{testCaseName, sql, queryCase._expectedException, segmentCount};
  ...
  private static Map<String, QueryTestCase> getTestCases()
    ...
    ClassLoader classLoader = QueriesMetadataTest.class.getClassLoader();
    // Get all test files.
