...
/// Unit tests for [ServerPlanRequestUtils].
public class ServerPlanRequestUtilsTest {
  ...
  ///
  /// Before the fix, UUID ByteArray values were passed as raw byte[] literals into the dynamic filter. The server-side
  /// predicate evaluator expected a canonical UUID string, so the filter never matched and UUID join queries silently
  /// returned no rows.
  ///
