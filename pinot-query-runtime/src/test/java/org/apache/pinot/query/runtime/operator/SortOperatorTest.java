...
public class SortOperatorTest {
  ...
  /// End-to-end fast-path test: feed a real [SortedMailboxReceiveOperator] running in k-way MERGE mode (block
  /// size = 2, so it emits MULTIPLE bounded data blocks) into a [SortOperator] with a LIMIT. Because the input is
  /// a `SortedMailboxReceiveOperator`, the SortOperator skips re-sorting and just slices to the limit. The final
