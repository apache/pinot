...
public class SortedMailboxReceiveOperatorTest {
  ...
  /**
   * Early termination in production ({@code SortOperator} hitting its LIMIT) arrives <em>mid-merge</em>: the heap is
   * live, mailboxes still hold undelivered blocks, and no handle has reached EOS. That is the state {@code drainToEos}
   * was rewritten for — it must round-robin every remaining handle to EOS rather than head-of-line block on one while
   * a sibling's sender sits on a full mailbox. Terminating before the first {@code nextBlock()} (as the other early
  ...
  /**
   * The stat travels to the broker through {@link StatMap#serialize}/{@link StatMap#deserialize}, which encode keys by
  ...
  /**
   * Drives a single-mailbox receive to completion and asserts the {@code K_WAY_MERGE_USED} stat, both on the stat map
   * and in the JSON that is rendered into the query response {@code stageStats}. Both paths must return the same rows,
