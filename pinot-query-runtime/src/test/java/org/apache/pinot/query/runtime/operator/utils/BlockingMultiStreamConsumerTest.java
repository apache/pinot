...
/// Unit tests for the per-stream pull mode of [BlockingMultiStreamConsumer] (the
/// [BlockingMultiStreamConsumer.StreamHandle] / [BlockingMultiStreamConsumer#streamHandles()] /
/// `StreamHandle.readBlocking()` API plus the single-mode guard). The round-robin path is covered separately by
/// `MailboxReceiveOperatorTest`.
public class BlockingMultiStreamConsumerTest {
  ...
  /// Serializes a [MultiStageQueryStats] with a fixed leaf-side operator shape, so the consumer can merge it on
  /// success EOS. Two EOS blocks with the same shape merge cleanly (see
  ...
  /// [BlockingMultiStreamConsumer.StreamHandle#poll()] is the non-blocking primitive the k-way merge's cooperative
  /// drain relies on: it must never park, must surface data/EOS exactly like `readBlocking()` does, and must not
  /// re-poll an already-exhausted stream (mirrors the mailbox-release comment on the `Handle` implementation).
  @Test
  ...
  /// A hand-written [AsyncStream] whose [#poll()] drains a scripted queue (empty queue => not ready yet) and
  /// which exposes a way to fire the captured new-data callback, so tests can drive the per-stream blocking loop
  /// without any real mailbox infrastructure.
