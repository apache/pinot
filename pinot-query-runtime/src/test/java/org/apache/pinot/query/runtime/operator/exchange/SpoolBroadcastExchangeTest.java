...
public class SpoolBroadcastExchangeTest {
  ...
  public void setUp() {
    ...
    // The spool destinations are per-stage exchanges wrapped as local sending mailboxes
  ...
  @Test
  public void shouldCopyBlocksWithMutableCellsForAllDestinationsButOne() {
    // Given:
    ...
    // Then: the first destination receives the original block, the second one receives a copy that shares no mutable
  ...
  @Test
  public void shouldShareBlocksWithoutMutableCells() {
  ...
  public void shouldFailToCopyBlocksWithObjectColumnsWithoutAggFunctions() {
    ...
    assertThrows(IllegalStateException.class, block::copy);
  }

  @Test
  public void shouldShareBlocksWithMutableCellsWithSingleDestination() {
    RowHeapDataBlock block = funnelBlock(stepEvents(new FunnelStepEvent(1000L, 0)));
  ...
  private static void route(MseBlock.Data block, SendingMailbox... destinations) {
    ...
    new SpoolBroadcastExchange(destinationList, BlockSplitter.NO_OP, BlockExchange.RANDOM_INDEX_CHOOSER)
        .route(destinationList, block);
  }
