...
public class SystemTableRegistryTest {
  ...
  public void testRegisterGetAndClose()
    ...
    SystemTableDataProvider provider = new TestProvider("SYSTEM.FOO", closeCount);
    SystemTableRegistry.register(provider);

    assertNotNull(SystemTableRegistry.get("system.foo"));
    assertSame(SystemTableRegistry.get("SYSTEM.FOO"), provider);
    assertEquals(SystemTableRegistry.getProviders().size(), 1);
    assertEquals(closeCount.get(), 0);

    SystemTableRegistry.close();
    assertEquals(closeCount.get(), 1);
    assertFalse(SystemTableRegistry.isRegistered("system.foo"));
  }

  private static final class TestProvider implements SystemTableDataProvider {
