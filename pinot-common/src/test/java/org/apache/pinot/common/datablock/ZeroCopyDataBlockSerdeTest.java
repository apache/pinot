  ...
  public Object[][] blocks() {
    ...
    return new Object[][]{
        ...
        {"eos with empty stat", new MetadataBlock(Collections.singletonList(PinotDataBuffer.empty()))},
        {
            ...
            new MetadataBlock(List.of(PinotDataBuffer.empty(), PinotDataBuffer.empty()))
        },
        {"eos with one not empty stat", new MetadataBlock(List.of(PinotByteBuffer.wrap(bytes1)))},
        {
            "eos with two not empty stat",
            new MetadataBlock(List.of(PinotByteBuffer.wrap(bytes1), PinotByteBuffer.wrap(bytes2)))
        },
        {
            "error with stats", MetadataBlock.newErrorWithStats(12, 21, "fakeId",
            Map.of(123, "error"),
            List.of(PinotByteBuffer.wrap(bytes1)))
        }
    };
  }
