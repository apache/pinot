...
public class QueryWorkloadConfigUtilsTest {
  ...
  public Object[][] fromZNRecordDataProvider() throws JsonProcessingException {
    ...
    NodeConfig leafNodeConfig = new NodeConfig(NodeConfig.Type.LEAF_NODE, validEnforcementProfile, leafPropagationScheme);

    // Non-leaf node
    PropagationScheme nonLeafPropagationScheme = new PropagationScheme("TENANT", List.of("value3", "value4"));
    NodeConfig nonLeafNodeConfig = new NodeConfig(NodeConfig.Type.NON_LEAF_NODE, validEnforcementProfile, nonLeafPropagationScheme);
    ...
    // Null propagation scheme for both nodes
    NodeConfig nodeConfigWithoutPropagationScheme = new NodeConfig(NodeConfig.Type.LEAF_NODE, validEnforcementProfile, null);
    List<NodeConfig> nodeConfigsWithoutPropagation = List.of(nodeConfigWithoutPropagationScheme, nodeConfigWithoutPropagationScheme);
    ...
    // Null propagation scheme in leaf node only
    NodeConfig leafNodeConfigWithoutPropagation = new NodeConfig(NodeConfig.Type.LEAF_NODE, validEnforcementProfile, null);
    List<NodeConfig> nodeConfigsWithNullLeaf = List.of(leafNodeConfigWithoutPropagation, nonLeafNodeConfig);
    ZNRecord znRecordWithNullLeaf = new ZNRecord("workloadId");
    znRecordWithNullLeaf.setSimpleField(QueryWorkloadConfig.QUERY_WORKLOAD_NAME, "workloadId");
    znRecordWithNullLeaf.setSimpleField(QueryWorkloadConfig.NODE_CONFIGS, JsonUtils.objectToString(nodeConfigsWithNullLeaf));
    QueryWorkloadConfig expectedQueryWorkloadConfigWithNullLeaf = new QueryWorkloadConfig("workloadId", nodeConfigsWithNullLeaf);
    data.add(new Object[] { znRecordWithNullLeaf, expectedQueryWorkloadConfigWithNullLeaf, false });
  ...
  public Object[][] updateZNRecordDataProvider() throws JsonProcessingException {
    ...
    List<NodeConfig> nodeConfigsWithoutPropagation = List.of(nodeConfigWithoutPropagation, nodeConfigWithoutPropagation);
