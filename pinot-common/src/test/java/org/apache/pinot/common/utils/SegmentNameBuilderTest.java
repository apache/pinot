...
//      SegmentNameBuilder.Realtime.buildHighLevelConsumerSegmentName("__", "a", "b");
  ...
  public void LLCSegNameTest() {
    ...
    assertEquals(segName1.getSegmentName(), segmentName);
    assertEquals(segName1.getPartitionId(), partitionId);
    assertEquals(segName1.getCreationTime(), creationTime);
    assertEquals(segName1.getSequenceNumber(), sequenceNumber);
    assertEquals(segName1.getTableName(), tableName);

    LLCSegmentName segName2 = new LLCSegmentName(segmentName);
    assertEquals(segName2.getSegmentName(), segmentName);
    assertEquals(segName2.getPartitionId(), partitionId);
    assertEquals(segName2.getCreationTime(), creationTime);
    assertEquals(segName2.getSequenceNumber(), sequenceNumber);
    assertEquals(segName2.getTableName(), tableName);

    assertEquals(segName1, segName2);
