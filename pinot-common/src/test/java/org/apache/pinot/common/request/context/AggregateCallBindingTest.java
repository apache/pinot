...
public class AggregateCallBindingTest {
  ...
  @Test
  public void testLogicalTypesAndLiteralOptionsReachReturnTypeInference() {
    List<ExpressionContext> value = List.of(ExpressionContext.forIdentifier("value"));
    assertEquals(AggregationFunctionTypeResolver.inferReturnType("same", ReturnTypes.ARG0, value,
        List.of(ColumnDataType.TIMESTAMP)), ColumnDataType.TIMESTAMP);
    assertEquals(AggregationFunctionTypeResolver.inferReturnType("same", ReturnTypes.ARG0, value,
        List.of(ColumnDataType.BOOLEAN)), ColumnDataType.BOOLEAN);

    SqlReturnTypeInference byOption = binding -> binding.getTypeFactory().createSqlType(
        "BIGINT".equals(binding.getOperandLiteralValue(1, String.class)) ? SqlTypeName.BIGINT : SqlTypeName.INTEGER);
    List<ExpressionContext> withOption =
        List.of(ExpressionContext.forIdentifier("value"), ExpressionContext.forLiteral(DataType.STRING, "BIGINT"));
    assertEquals(AggregationFunctionTypeResolver.inferReturnType("pick", byOption, withOption,
        List.of(ColumnDataType.INT, ColumnDataType.STRING)), ColumnDataType.LONG);
  }

  @Test(dataProvider = "protocols")
  public void testDeeplyNestedFunctionsRemainDecodable(TProtocolFactory factory) throws Exception {
    // Servers decode requests with a default TDeserializer. Function must stay free of the generated recursion-depth
    // limit (64) that newer Thrift compilers add, or deeply nested expressions such as c0 + c1 + ... + c100 fail.
    Expression expression = RequestUtils.getIdentifierExpression("c0");
    for (int i = 1; i <= 100; i++) {
      expression =
          RequestUtils.getFunctionExpression("plus", List.of(expression, RequestUtils.getIdentifierExpression("c" + i)));
    }
    Expression restored = new Expression();
    new TDeserializer(factory).deserialize(restored, new TSerializer(factory).serialize(expression));
    assertEquals(restored, expression);
  }
}
