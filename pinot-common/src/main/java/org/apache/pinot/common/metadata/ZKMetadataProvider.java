...
public class ZKMetadataProvider {
  ...
  private static final String PROPERTYSTORE_QUERY_WORKLOAD_CONFIGS_PREFIX = "/CONFIGS/QUERY_WORKLOAD_CONFIGS";
  ...
  public static boolean setQueryWorkloadConfig(ZkHelixPropertyStore<ZNRecord> propertyStore,
    ...
    String path = constructPropertyStorePathForWorkloadConfig(queryWorkloadConfig.getWorkloadName());
    boolean isNewConfig = !propertyStore.exists(path, AccessOption.PERSISTENT);
    ZNRecord znRecord = isNewConfig ? new ZNRecord(queryWorkloadConfig.getWorkloadName())
