...
public class QueryWorkloadConfigUtils {
  ...
  private static void validateCostSplits(List<CostSplit> costSplits, String prefix, List<String> errors) {
    ...
    // Validate reasonable total costs (optional business rule validation)
    validateTotalCosts(totalCpuCost, totalMemoryCost, prefix, errors);
  }
  ...
  /**
   * Validates that total costs are within reasonable bounds.
   *
   * @param totalCpuCost the total CPU cost across all cost splits
   * @param totalMemoryCost the total memory cost across all cost splits
   * @param prefix the prefix for error messages
   * @param errors the list to add validation errors to
   */
  private static void validateTotalCosts(long totalCpuCost, long totalMemoryCost, String prefix, List<String> errors) {
    // Validate reasonable CPU cost bounds (e.g., not exceeding 1 min per query)
    long maxReasonableCpuNs = 60L * 1_000_000_000L; // 1 hour in nanoseconds
    if (totalCpuCost > maxReasonableCpuNs) {
      errors.add(prefix + " total CPU cost (" + totalCpuCost + "ns) exceeds reasonable limit (" +
                maxReasonableCpuNs + "ns = 1 hour)");
    }
    //TODO: Add memory cost validation
  }
