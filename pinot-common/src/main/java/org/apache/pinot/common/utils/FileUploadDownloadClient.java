...
public class FileUploadDownloadClient implements AutoCloseable {
    ...
    public static URI getRevertReplaceSegmentsURI(URI controllerURI, String rawTableName, String tableType,
        ...
        return getURI(controllerURI.getScheme(), controllerURI.getHost(), controllerURI.getPort(),
            ...
            TYPE_DELIMITER + tableType + SEGMENT_LINEAGE_ENTRY_ID_PARAMETER + segmentLineageEntryId
                + FORCE_REVERT_PARAMETER + forceRevert);
    }
    ...
    private static ClassicHttpRequest getAddSchemaRequest(URI uri, String schemaName, File schemaFile,
        @Nullable List<Header> headers, @Nullable List<NameValuePair> parameters) {
        return getUploadFileRequest(HttpPost.METHOD_NAME, uri, getContentBody(schemaName, schemaFile), headers,
            parameters);
    }
    ...
    private static ClassicHttpRequest getUploadSegmentMetadataFilesRequest(URI uri, Map<String, File> metadataFiles,
        ...
        MultipartEntityBuilder multipartEntityBuilder =
            MultipartEntityBuilder.create().setMode(HttpMultipartMode.LEGACY);
    ...
    public SimpleHttpResponse uploadSegmentMetadata(URI uri, String segmentName, File segmentMetadataFile,
        ...
        throws IOException, HttpErrorStatusException {
        return HttpClient.wrapAndThrowHttpException(_httpClient.sendRequest(
            getUploadSegmentMetadataRequest(uri, segmentName, segmentMetadataFile, headers, parameters),
            socketTimeoutMs));
    }
    ...
    public SimpleHttpResponse uploadSegment(URI uri, String segmentName, File segmentFile,
        ...
        throws IOException, HttpErrorStatusException {
        return HttpClient.wrapAndThrowHttpException(
            _httpClient.sendRequest(getUploadSegmentRequest(uri, segmentName, segmentFile, headers, parameters),
                socketTimeoutMs));
    }
    ...
    public SimpleHttpResponse uploadSegment(URI uri, String segmentName, InputStream inputStream,
        ...
        return HttpClient.wrapAndThrowHttpException(
            _httpClient.sendRequest(getUploadSegmentRequest(uri, segmentName, inputStream, headers, parameters),
                HttpClient.DEFAULT_SOCKET_TIMEOUT_MS));
    }
                ...
                try {
                    ...
                    if (e.getStatusCode() < 500) {
                        if (e.getStatusCode() == Response.Status.NOT_FOUND.getStatusCode()) {
                            LOGGER.error("Segments not found for table {} when sending request uri: {}", rawTableName,
                                uri);
                        }
                    }
                    return false;
    ...
    public String uploadToSegmentStore(String uri)
        ...
        ClassicRequestBuilder requestBuilder =
            ClassicRequestBuilder.post(new URI(uri)).setVersion(HttpVersion.HTTP_1_1);
        // sendRequest checks the response status code
        SimpleHttpResponse response = HttpClient.wrapAndThrowHttpException(
            _httpClient.sendRequest(requestBuilder.build(), HttpClient.DEFAULT_SOCKET_TIMEOUT_MS));
        String downloadUrl = response.getResponse();
        if (downloadUrl.isEmpty()) {
            throw new HttpErrorStatusException(String.format(
                "Returned segment download url is empty after requesting servers to upload by the path: %s", uri),
    ...
    public SimpleHttpResponse endReplaceSegments(URI uri, int socketTimeoutMs,
        ...
        throws IOException, HttpErrorStatusException {
        String jsonBody =
            (endReplaceSegmentsRequest == null) ? null : JsonUtils.objectToString(endReplaceSegmentsRequest);
        return HttpClient.wrapAndThrowHttpException(
    ...
    public SimpleHttpResponse sendSegmentCompletionProtocolRequest(URI uri, int socketTimeoutMs)
        throws IOException, HttpErrorStatusException {
        return sendSegmentCompletionProtocolRequest(uri, Collections.emptyList(), Collections.emptyList(),
            socketTimeoutMs);
    }
