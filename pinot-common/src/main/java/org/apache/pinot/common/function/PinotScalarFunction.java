...
/// Provides finer control to the scalar functions annotated with [org.apache.pinot.spi.annotations.ScalarFunction]
///
/// See more details in [FunctionRegistry].
public interface PinotScalarFunction {

  /// Returns the name of the function.
  String getName();

  /// Returns the set of names of the function, including the primary name returned by [#getName()].
  ///
  /// The value of this function may be used to register the function in the OperatorTable, although the names included
  /// an optional [org.apache.pinot.spi.annotations.ScalarFunction]} annotation higher priority.
  ...
  ///
  /// See for example [FunctionRegistry.ArgumentCountBasedScalarFunction] which overrides this method to also
  ...
  /// class. For example, this should not be a call to [java.util.Objects#toIdentityString(Object)], given it will
  /// include the hash code of the object id, which will be different for each instance of the object. Instead, it
  /// should
  /// depend on the class.
  ...
  /// Returns the corresponding [PinotSqlFunction] to be registered into the OperatorTable, or `null` if it
  ...
  /// Returns the [FunctionInfo] for the given argument types, or `null` if there is no matching.
  @Nullable
  default FunctionInfo getFunctionInfo(ColumnDataType[] argumentTypes) {
    return getFunctionInfo(argumentTypes.length);
  }

  /// Returns the [FunctionInfo] for the given number of arguments, or `null` if there is no matching.
  @Nullable
