...
public final class OomProtectionUtils {
  ...
  /**
   * Validate GC/IHOP settings for OOM protection. The OOM query killer assumes G1GC with a predictable (non-adaptive)
   * IHOP value that stays comfortably below the kill thresholds so GC has a chance to reclaim memory before Pinot
   * starts terminating queries.
   *
  ...
  public static boolean enforceIhopGcOrDisableOom(PinotConfiguration config, List<String> jvmInputArgs) {
    ...
    boolean adaptiveIhopDisabled = hasDisableAdaptiveIhopFlag(jvmInputArgs);
    if (!adaptiveIhopDisabled) {
    ...
    // Require IHOP to be explicitly set so GC will trigger before OOM protections
    Double ihopRatio = null;
    if (jvmInputArgs != null) {
      for (String arg : jvmInputArgs) {
        if (arg != null && arg.startsWith("-XX:InitiatingHeapOccupancyPercent=")) {
          ...
          if (idx > 0 && idx + 1 < arg.length()) {
            try {
              ...
              LOGGER.warn(
                  "Failed to parse InitiatingHeapOccupancyPercent from '{}'; disabling OOM protection.", arg, e);
              config.setProperty(
    ...
    if (ihopRatio == null) {
      LOGGER.warn(
          "G1 IHOP value not explicitly set via -XX:InitiatingHeapOccupancyPercent. Disabling OOM protection to avoid "
              + "query kills without GC backpressure.");
      config.setProperty(CommonConstants.Accounting.FULLY_QUALIFIED_CONFIG_OF_OOM_PROTECTION_KILLING_QUERY, false);
      return true;
    }
