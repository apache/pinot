...
public class CalciteSqlParser {
  ...
  public static SqlNodeAndOptions compileToSqlNodeAndOptions(String sql)
    ...
    sql = rewriteArrayJoinBaseTables(sql);
  ...
  private static String rewriteArrayJoinBaseTables(String sql) {
    Matcher matcher = Pattern.compile("(from\\s+)([^\\s\\(\\)]+)(\\s+array\\s+join)", Pattern.CASE_INSENSITIVE)
        .matcher(sql);
    ...
    while (matcher.find()) {
      ...
      if (!tableRef.startsWith("(")) {
        matcher.appendReplacement(buffer, matcher.group(1) + "(" + tableRef + ")" + matcher.group(3));
        replaced = true;
      }
    }
    if (replaced) {
