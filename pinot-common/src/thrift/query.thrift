...
//    do not reuse the id.
  ...
  // Rolling upgrade: a pre-grouping-sets server silently ignores this field and runs a plain GROUP BY (dropping
  // the subtotal/grand-total rows and the $groupingId column the broker reducer expects), so upgrade servers
  // before brokers.
