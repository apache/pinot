...
public class TestStarTreeRecordImpl
{
  ...
  public void testAliasOther() throws Exception
  {
    ...
    Assert.assertEquals(aliased.getDimensionKey().getDimensionValue(config.getDimensions(), "A"), StarTreeConstants.OTHER);
  }
  ...
  public void testUpdateDimensionValues() throws Exception
  {
    ...
    Assert.assertEquals(builder.getDimensionKey().getDimensionValue(config.getDimensions(), "A"), "A0");

    // Update with another, should go to star
    builder.updateDimensionKey(getDimensionKey("A1", "B0", "C0"));
    Assert.assertEquals(builder.getDimensionKey().getDimensionValue(config.getDimensions(), "A"), StarTreeConstants.STAR);
  }
