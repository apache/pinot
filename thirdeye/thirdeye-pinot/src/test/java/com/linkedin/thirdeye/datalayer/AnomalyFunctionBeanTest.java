...
public class AnomalyFunctionBeanTest extends AbstractBaseTest {
  ...
  public void testAll() {
    ...
    List<AnomalyFunctionBean> retrievedByPredicate =
        genericPojoDao.get(predicate, AnomalyFunctionBean.class);
    ...
    List<AnomalyFunctionBean> retrievedAfterUpdateByPredicate =
        genericPojoDao.get(predicate, AnomalyFunctionBean.class);
    Assert.assertEquals(1, retrievedByPredicate.size());
    Assert.assertEquals("testCollection-new",
        retrievedAfterUpdateByPredicate.get(0).getCollection());
