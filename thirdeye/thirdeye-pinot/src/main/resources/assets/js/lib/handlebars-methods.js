$(document).ready(function() {
   ...
   //takes a string returns a HEX color code
    Handlebars.registerHelper('colorByName', function(name) {
        // Assign a line color to each metric on the time-series by turn the
		// name into hexadecimal colorcode
        if(name == "?"){
            name = "other"
        }else if(name == ""){
            name = "unknown"
        }
        return "#" + ( parseInt(name + name + name, 36) + 16777216 * 4 ).toString(16).substring(0, 6)
    });
    
  //takes a string returns a HEX color code
    Handlebars.registerHelper('colorById', function( id, numIds ) {
