...
public class PinotThirdEyeSummaryClient implements OLAPDataBaseClient {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (args.length == 0) {
      ...
      formatter.printHelp(PinotThirdEyeSummaryClient.class.getSimpleName(), options);
    } else {
  ...
  private static Options buildOptions() {
    ...
    Option dataset =
        OptionBuilder.withArgName("name").hasArg().withDescription("dataset name").isRequired().create("dataset");
    options.addOption(dataset);

    Option metricName =
        OptionBuilder.withArgName("name").hasArg().withDescription("metric name").isRequired().create("metric");
    options.addOption(metricName);

    Option dimensions =
        OptionBuilder.withArgName("list").hasArg().withDescription("dimension names that are separated by comma")
            .create("dimensions");
    options.addOption(dimensions);

    Option filters =
        OptionBuilder.withArgName("json").hasArg().withDescription("filter to apply on the data cube").create("filters");
    options.addOption(filters);


    Option currentStart =
        OptionBuilder.withArgName("millis").hasArg().withDescription("current start time incluseive").isRequired()
            .create("currentStart");
    options.addOption(currentStart);

    Option currentEnd = OptionBuilder.withArgName("millis").hasArg().withDescription("current end time exclusive").isRequired()
        .create("currentEnd");
    options.addOption(currentEnd);

    Option baselineStart =
        OptionBuilder.withArgName("millis").hasArg().withDescription("baseline start time inclusive").isRequired()
            .create("baselineStart");
    options.addOption(baselineStart);

    Option baselineEnd = OptionBuilder.withArgName("millis").hasArg().withDescription("baseline end time exclusive").isRequired()
        .create("baselineEnd");
    options.addOption(baselineEnd);

    Option size = OptionBuilder.withArgName("number").hasArg().withDescription("size of summary").create("size");
    options.addOption(size);

    Option topDimension = OptionBuilder.withArgName("number").hasArg().withDescription("number of top dimensions")
        .create("topDimensions");
    options.addOption(topDimension);

    Option hierarchies =
        OptionBuilder.withArgName("json").hasArg().withDescription("dimension hierarchies").create("hierarchies");
    options.addOption(hierarchies);

    Option oneSideError =
        OptionBuilder.withArgName("boolean").withDescription("enable one side error summary")
            .create("oneSideError");
    options.addOption(oneSideError);

    Option manualOrder =
        OptionBuilder.withArgName("boolean").withDescription("use manual dimension order")
            .create("manual");
    options.addOption(manualOrder);

    Option dateTimeZone =
        OptionBuilder.withArgName("id").hasArg().withDescription("time zone id in Joda library").create("timeZone");
