...
public class ScanRemoveAnomalyTest {
  ...
  public void ScanRemoveAnomalyTest() throws IOException {
    // SequentialMonitoringInputOne("timeseries.csv", "output1.csv", 720, 72, 200,
    // ScanStatistics.Pattern.UP,
    // ScanStatistics.Pattern.DOWN, 1.0, 1.0);
    // SequentialMonitoringInputTwo("caseStudyOne.csv", "output2.csv", 720, 72, 24, 200,
    // ScanStatistics.Pattern.DOWN,
    // ScanStatistics.Pattern.UP, 1.0, 1.0);
    // SequentialMonitoringSingleSideInputOne("RcaseOne.csv", "RseasonalOutputOne.csv",
    // "RoutputOne.csv", 648, 72, 24, 500,
    // ScanStatistics.Pattern.DOWN, 1.0);
    SequentialMonitoringSingleSideInputTwo("debug1_deseasonal.csv", "RoutputTwo.csv", 648, 72, 24,
        500, ScanStatistics.Pattern.DOWN, 1.0);
    SequentialMonitoringSingleSideInputTwo("debug2_deseasonal.csv", "RoutputThree.csv", 648, 72, 24,
        500, ScanStatistics.Pattern.UP, 1.0);
    // NoHistoryAnomaliesRemoved("timeseries.csv", 720, 200);

  }

  public void SequentialMonitoringSingleSideInputTwo(String deSeasonalInputFilename,
      String outputFilename, int training_size, int monitor_size, int scan_size, int numSims,
      ScanStatistics.Pattern target_pattern, double target_level) throws IOException {
    int[] columns = {
        1
    };
    ...
    SequentialMonitoringCompareWithR(series, inputTimestamps, outputFilename, training_size,
        monitor_size, scan_size, numSims, target_pattern, target_level);
  }

  public void SequentialMonitoringSingleSideInputOne(String filename, String seasonalOutputFilename,
      String outputFilename, int training_size, int monitor_size, int scan_size, int numSims,
      ScanStatistics.Pattern target_pattern, double target_level) throws IOException {
    int[] columns = {
        3
    ...
    // double[][] data = STLTest.removeSeasonality(timestamps, series, 168, 1, 15, 0.75, 0.10,
    // true);
  ...
  public void SequentialMonitoringInputOne(String filename, String outputFilename,
      int training_size, int monitor_size, int scan_size, int numSims,
      ScanStatistics.Pattern target_pattern, ScanStatistics.Pattern complementary_pattern,
      double target_level, double complementary_level) throws IOException {
    int[] columns = {
        2, 3
    };
    double[] deseasonal_series = GetData(filename, columns);
    SequentialMonitoring(deseasonal_series, outputFilename, training_size, monitor_size, scan_size,
        numSims, target_pattern, complementary_pattern, target_level, complementary_level);
  }
  ...
  private void SequentialMonitoringCompareWithR(double[] series, String[] inputTimestamps,
      String outputFilename, int training_size, int monitor_size, int scan_size, int numSims,
      ScanStatistics.Pattern target_pattern, double target_level) throws IOException {
    ...
    for (int monitor_idx = monitor_start; monitor_idx <= monitor_end; monitor_idx += scan_size) {
       ...
       * tmp_idx = tmp_idx- 168;
       * }
       * System.out.println("train"+inputTimestamps[ii]+","+ inputTimestamps[tmp_idx] +","
       * +data_seasonal[tmp_idx]+
       * "," + series[ii]);
       ...
       * tmp_idx = tmp_idx- 168;
       * }
       * System.out.println("monitor"+inputTimestamps[ii]+","+ inputTimestamps[tmp_idx] +","
       * +data_seasonal[tmp_idx]+
       * "," + series[ii]);
      ...
      long[] trainingTimestamps =
          Arrays.copyOfRange(timestamps, current_train_start, current_train_end);
      ScanStatistics scanStatistics = new ScanStatistics(numSims, 1, 100000, 0.05, target_pattern,
          1, false, up_level, down_level, notequal_level);
      ...
      double[] trainingDataWithOutTargetAnomalies = ScanStatisticsAnomalyDetectionFunction
          .removeAnomalies(trainingTimestamps, train, currentAnomalousTimestamps);

      Range<Integer> target_anomaly =
          scanStatistics.getInterval(trainingDataWithOutTargetAnomalies, monitor);
      ...
      if (target_anomaly != null) {
        ...
        Range<Integer> anomalyOffset =
            Range.closedOpen(target_anomaly.lowerEndpoint() + current_monitor_start,
                target_anomaly.upperEndpoint() + current_monitor_start);
        output_list.add(
            anomalyOffset.lowerEndpoint() + "," + inputTimestamps[anomalyOffset.lowerEndpoint()]
                + "," + (anomalyOffset.upperEndpoint() - 1) + ","
                + inputTimestamps[anomalyOffset.upperEndpoint() - 1] + "," + current_monitor_start
                + "," + inputTimestamps[current_monitor_start] + "," + (current_monitor_end - 1)
                + "," + inputTimestamps[current_monitor_end - 1]);
        System.out.println(inputTimestamps[anomalyOffset.lowerEndpoint()] + ","
  ...
  private void SequentialMonitoringReplay(double[] series, String[] inputTimestamps,
      String outputFilename, int training_size, int monitor_size, int scan_size, int numSims,
      ScanStatistics.Pattern target_pattern, double target_level) throws IOException {
    int numData = series.length;
    double up_level = 1.0;
    ...
    for (int monitor_idx = monitor_start; monitor_idx <= monitor_end; monitor_idx += scan_size) {
      ...
      double[] training_before_deseason_series =
          Arrays.copyOfRange(series, monitor_idx - training_size, monitor_idx);
      ...
      double[] trainingDataWithOutTargetAnomalies = ScanStatisticsAnomalyDetectionFunction
          .removeAnomalies(trainingTimestamps, train, targetAnomalousTimestamps);
      ...
      if (target_anomaly != null) {
        ...
        Range<Integer> anomalyOffset =
            Range.closedOpen(target_anomaly.lowerEndpoint() + monitor_idx,
                target_anomaly.upperEndpoint() + monitor_idx);
        output_list.add(anomalyOffset.lowerEndpoint() + ","
            + inputTimestamps[anomalyOffset.lowerEndpoint()] + "," + anomalyOffset.upperEndpoint()
            + "," + inputTimestamps[anomalyOffset.upperEndpoint()] + "," + monitor_idx + ","
            + inputTimestamps[monitor_idx] + "," + (monitor_idx + monitor_size - 1) + ","
  ...
  private void SequentialMonitoring(double[] deseasonal_series, String outputFilename,
      int training_size, int monitor_size, int scan_size, int numSims,
      ScanStatistics.Pattern target_pattern, ScanStatistics.Pattern complementary_pattern,
      double target_level, double complementary_level) throws IOException {
    ...
    for (int monitor_idx = monitor_start; monitor_idx <= monitor_end; monitor_idx += monitor_size) {
      System.out.println(monitor_idx);
      double[] train =
          Arrays.copyOfRange(deseasonal_series, monitor_idx - training_size, monitor_idx);
      double[] monitor =
          Arrays.copyOfRange(deseasonal_series, monitor_idx, monitor_idx + monitor_size);
      long[] trainingTimestamps =
          Arrays.copyOfRange(timestamps, monitor_idx - training_size, monitor_idx);
      ScanStatistics scanStatistics = new ScanStatistics(numSims, 1, 100000, 0.05, target_pattern,
          1, false, up_level, down_level, notequal_level);

      ScanStatistics complementaryScanStatistics = new ScanStatistics(numSims, 1, 100000, 0.05,
          complementary_pattern, 1, false, up_level, down_level, notequal_level);

      double[] trainingDataWithOutAggregateAnomalies = ScanStatisticsAnomalyDetectionFunction
          .removeAnomalies(trainingTimestamps, train, aggregateAnomalousTimestamps);

      double[] trainingDataWithOutComplementaryAnomalies = ScanStatisticsAnomalyDetectionFunction
          .removeAnomalies(trainingTimestamps, train, complementaryAnomalousTimestamps);

      Range<Integer> target_anomaly =
          scanStatistics.getInterval(trainingDataWithOutAggregateAnomalies, monitor);

      Range<Integer> complementary_anomaly = complementaryScanStatistics
          .getInterval(trainingDataWithOutComplementaryAnomalies, monitor);
      ...
      if (target_anomaly != null) {
        ...
        System.out
            .println(anomalyOffset.lowerEndpoint() + "," + anomalyOffset.upperEndpoint() + ",");
      }
      if (complementary_anomaly != null) {
        for (int i = complementary_anomaly.lowerEndpoint(); i < complementary_anomaly
            .upperEndpoint(); i++) {
          aggregateAnomalousTimestamps.add((long) (i + monitor_idx));
  ...
  private void NoHistoryAnomaliesRemoved(String filename, int split, int numSims)
      throws IOException {
    int[] columns = {
        2, 3
    };
    ...
    ScanStatistics scanStatistics =
        new ScanStatistics(numSims, 1, 100000, 0.05, ScanStatistics.Pattern.DOWN, 1, false);

    Range<Integer> anomaly = scanStatistics.getInterval(train, monitor);
    if (anomaly != null) {
      Range<Integer> anomalyOffset =
          Range.closedOpen(anomaly.lowerEndpoint() + split, anomaly.upperEndpoint() + split);
    ...
    scanStatistics =
        new ScanStatistics(numSims, 1, 100000, 0.05, ScanStatistics.Pattern.UP, 1, false);
    ...
    scanStatistics =
        new ScanStatistics(numSims, 1, 100000, 0.05, ScanStatistics.Pattern.NOTEQUAL, 1, false);
