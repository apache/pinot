...
public class StateSpaceModel {
  ...
  public StateSpaceModel(DoubleMatrix state_transition_matrix, DoubleMatrix observation_matrix,
      DoubleMatrix transition_covariance_matrix, DoubleMatrix observation_covariance_matrix,
      DoubleMatrix initial_state_mean, DoubleMatrix initial_state_covariance,
    ...
    if (!state_transition_matrix.isSquare()
      ...
      throw new MatrixDimensionMismatchException(state_transition_matrix.getRows(),
          state_transition_matrix.getColumns(), dimension_of_states_, dimension_of_states_);
    }
    ...
    if (!transition_covariance_matrix.isSquare()
      ...
      throw new MatrixDimensionMismatchException(transition_covariance_matrix.getRows(),
          transition_covariance_matrix.getColumns(), dimension_of_states_, dimension_of_states_);
    }
    ...
    if (!observation_covariance_matrix.isSquare()
      ...
      throw new MatrixDimensionMismatchException(observation_covariance_matrix.getRows(),
          observation_covariance_matrix.getColumns(), dimension_of_observations_,
    ...
    if (initial_state_mean.getRows() != dimension_of_states_
      ...
      throw new MatrixDimensionMismatchException(initial_state_mean.getRows(),
          initial_state_mean.getColumns(), dimension_of_states_, 1);
    }
    ...
    if (!initial_state_covariance.isSquare()
      ...
      throw new MatrixDimensionMismatchException(initial_state_covariance.getRows(),
          initial_state_covariance.getColumns(), dimension_of_states_, dimension_of_states_);
    }
  ...
  public void calculatePrediction(int steps_ahead) {
    if (steps_ahead > 0) {
      ...
      for (int ii = 0; ii < steps_ahead; ii++) {
        ...
        DoubleMatrix R = state_transition_matrix_ref_.mmul(previous_state_covariance)
        ...
        predicted_variances_[ii] = observation_matrix_ref_.mmul(R)
            .mmul(observation_matrix_ref_.transpose()).addi(observation_covariance_matrix_ref_);
  ...
  private void updateFilteredStates() {
    ...
    for (int ii = 0; ii < number_of_observations_; ii++) {
      ...
      predicted_observation_covariances_[ii] = observation_matrix_ref_.mmul(R)
          .mmul(observation_matrix_T).addi(observation_covariance_matrix_ref_);
      ...
      if (observations_ref_[ii] == null) {
        ...
        DoubleMatrix inv_predicted_observation_covariances =
            Solve.pinv(predicted_observation_covariances_[ii]);
        ...
        estimated_state_means_[ii] = R_obs_mat_T_inv_pred_obs_cov
            .mmul(observations_ref_[ii].sub(predicted_observation_means_[ii])).addi(a);
        ...
        estimated_covariances_[ii] = observation_matrix_ref_.mmul(estimated_state_covariances_[ii])
            .mmul(observation_matrix_T).addi(observation_covariance_matrix_ref_);
      }
    }
  }
