...
public class DropWizardApplicationRunner {
  ...
  public static <T extends Configuration> DropWizardServer<T> createServer(T config,
    ...
    File tmpConfigFile = new File(System.getProperty("java.io.tmpdir"),
    ...
    ConfigurationFactory<T> configurationFactory =
        bootstrap.getConfigurationFactoryFactory().create((Class<T>) config.getClass(),
            bootstrap.getValidatorFactory().getValidator(), bootstrap.getObjectMapper(), "dw");
    final T builtConfig = configurationFactory.build(bootstrap.getConfigurationSourceProvider(),
        tmpConfigFile.getAbsolutePath());

    // Configure logging
    builtConfig.getLoggingFactory().configure(bootstrap.getMetricRegistry(),
    ...
    return new DropWizardServer(builtConfig, bootstrap, application, environment, server,
        environment.metrics());
  }
  ...
  public static class DropWizardServer<T extends Configuration> {
    ...
    DropWizardServer(T builtConfig, Bootstrap<T> bootstrap, Application<T> application,
        Environment environment, Server jettyServer, MetricRegistry metricRegistry) {
