...
public class VolumeHeatMap extends SimpleHeatMap {
  ...
  @Override
  protected List<HeatMapCell> generateHeatMap(Map<String, Number> baseline,
    ...
    Map<String, Number> logBaseline = new HashMap<String, Number>(
        baseline.size());
    ...
    for (Map.Entry<String, Number> entry : logBaseline.entrySet()) {
      double score = (entry.getValue().doubleValue() - logBaselineStats
          .getAverage()) / Math.sqrt(logBaselineStats.getVariance());
    ...
    for (Map.Entry<String, Number> entry : current.entrySet()) {
      ...
      double ratio = (currentMetricValue.doubleValue() - baselineMetricValue
          .doubleValue()) / baselineStats.getSum();
      ...
      HeatMapCell cell = new HeatMapCell(entry.getKey(), currentMetricValue,
          baselineMetricValue, label.toString(), ratio, alphas.get(entry
              .getKey()), COLOR);
  ...
  private static double normalCdf(double x) {
    ...
    double p = d
        * t
        * (.3193815 + t
            * (-.3565638 + t * (1.781478 + t * (-1.821256 + t * 1.330274))));
    if (x > 0) {
      p = 1 - p;
    }
    return p;
  ...
  private static Stats getStats(Map<String, Number> metrics) {
    ...
    for (Map.Entry<String, Number> entry : metrics.entrySet()) {
      if (Double.NEGATIVE_INFINITY != entry.getValue().doubleValue()) {
        ...
        sumSquares += entry.getValue().doubleValue()
            * entry.getValue().doubleValue();
      }
    }

    double average = sum / metrics.size();
    double variance = (sumSquares - (sum * sum) / metrics.size())
        / (metrics.size() - 1);
