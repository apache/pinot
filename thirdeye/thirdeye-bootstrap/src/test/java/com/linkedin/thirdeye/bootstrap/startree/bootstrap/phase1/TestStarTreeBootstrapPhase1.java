...
public class TestStarTreeBootstrapPhase1 {
  ...
  private List<Pair<GenericRecord, NullWritable>> generateTestDataMapper() throws Exception {
    ...
    List<Pair<GenericRecord, NullWritable>> inputRecords =
        new ArrayList<Pair<GenericRecord, NullWritable>>();
    ...
    input.put("hoursSinceEpoch",
        TestStarTreeBootstrapPhase1.TestHelper.generateRandomHoursSinceEpoch());
    input.put("m1", 10);
    input.put("m2", 20);
  ...
  private List<Pair<BytesWritable, List<BytesWritable>>> generateTestDataReducer()
      throws Exception {
    ...
    BootstrapPhaseMapOutputValue outputValue1 =
        new BootstrapPhaseMapOutputValue(key, TestStarTreeBootstrapPhase1.TestHelper
    ...
    BootstrapPhaseMapOutputValue outputValue2 =
        new BootstrapPhaseMapOutputValue(key, TestStarTreeBootstrapPhase1.TestHelper
            .generateMetricTimeSeries(starTreeBootstrapConfig, 20));
    list.add(new BytesWritable(outputValue2.toBytes()));

    inputRecords.add(
        new Pair<BytesWritable, List<BytesWritable>>(new BytesWritable(outputKey.toBytes()), list));
    return inputRecords;
  }
  ...
  private void generateAndSerializeStarTree() throws IOException {
    ...
    MetricTimeSeries timeSeries = TestStarTreeBootstrapPhase1.TestHelper
    ...
    timeSeries = TestStarTreeBootstrapPhase1.TestHelper
        .generateRandomMetricTimeSeries(starTreeBootstrapConfig);
  ...
  public void setUp() throws IOException {
    ...
    config.set(StarTreeBootstrapPhaseOneConstants.STAR_TREE_BOOTSTRAP_CONFIG_PATH.toString(),
        ClassLoader.getSystemResource(CONF_FILE).toString());
    Schema schema = new Schema.Parser().parse(ClassLoader.getSystemResourceAsStream(SCHEMA_FILE));
    ...
    config.set(StarTreeGenerationConstants.STAR_TREE_GEN_OUTPUT_PATH.toString(),
        thirdEyeRoot + File.separator + "startree_generation");
    config.set(StarTreeBootstrapPhaseOneConstants.STAR_TREE_BOOTSTRAP_CONVERTER_CLASS.toString(),
  ...
  public void testStarTreeBootstrapPhase1() throws Exception {
    ...
    BootstrapPhaseMapOutputValue output = BootstrapPhaseMapOutputValue.fromBytes(
        result.get(0).getSecond().getBytes(), TestHelper.getMetricSchema(starTreeBootstrapConfig));
    Assert.assertEquals(TestStarTreeBootstrapPhase1.TestHelper
        .generateMetricTimeSeries(starTreeBootstrapConfig, 30), output.getMetricTimeSeries());
  }
  ...
  static class TestHelper {
    ...
    public static MetricTimeSeries generateMetricTimeSeries(StarTreeBootstrapPhaseOneConfig config,
        Integer value) {
      List<String> names = config.getMetricNames();
      List<MetricType> types = config.getMetricTypes();
      MetricSchema schema = new MetricSchema(names, types);
