...
public class TestRollUpPhase2 {
  ...
  private List<Pair<BytesWritable, BytesWritable>> generateTestMapperData() throws Exception {
    ...
    for (int i = 0; i < INPUT_RECORDS; i++) {
      ...
      inputRecords.add(new Pair<BytesWritable, BytesWritable>(new BytesWritable(serializedKey),
          new BytesWritable(serializedMetrics)));
    }
    return inputRecords;
  }
  ...
  public void setUp() throws Exception {
    ...
    configuration.set(RollupPhaseTwoConstants.ROLLUP_PHASE2_CONFIG_PATH.toString(),
        ClassLoader.getSystemResource(CONF_FILE).toString());
    configuration.set(RollupPhaseTwoConstants.ROLLUP_PHASE2_ANALYSIS_PATH.toString(),
        "dummy analysis path");
  ...
  public void testRollUpPhase2() throws Exception {
    ...
    for (Pair<BytesWritable, BytesWritable> p : result) {
      RollupPhaseTwoReduceOutput output = RollupPhaseTwoReduceOutput
          .fromBytes(p.getSecond().getBytes(), TestHelper.getMetricSchema(rollUpConfig));
