...
if [ $noThirdEyeChange -ne 0 ]; then
  ...
  if [ "$TRAVIS_JDK_VERSION" != 'oraclejdk8' ]; then
    ...
    mvn clean install -B -DskipTests=true -Pbin-dist -Dmaven.javadoc.skip=true -Drevision="${REVISION}" ${KAFKA_BUILD_OPTS} > /tmp/mvn_build_log
    if [ $? -eq 0 ]; then
      exit 0
    else
    ...
    mvn clean install -B -DskipTests=true -Pbin-dist -Dmaven.javadoc.skip=true -Drevision="${REVISION}" ${KAFKA_BUILD_OPTS} || exit $?
  fi
fi
