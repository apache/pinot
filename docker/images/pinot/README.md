...
```SHELL
./docker-build.sh [Docker Tag] [Git Branch] [Pinot Git URL] [Kafka Version] [JDK Version]
```
...
For users on Apple Silicon machines, Docker can build the Pinot image for arm64 directly.

* Example of building an arm64 image:
```SHELL
./docker-build.sh pinot:latest master https://github.com/apache/pinot.git 2.0 17
```
