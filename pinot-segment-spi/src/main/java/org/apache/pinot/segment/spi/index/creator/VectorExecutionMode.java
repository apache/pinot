...
///
/// Each mode describes a specific strategy for combining ANN search, metadata filtering,
/// threshold predicates, and exact refinement. The runtime selects a mode based on backend
/// capabilities, query shape, and filter selectivity, then reports it in explain/debug output.
///
/// Execution modes are ordered from cheapest/most approximate to most expensive/exact:
///
/// 1. [#ANN_TOP_K] - pure ANN, no filter, no rerank
/// 2. [#ANN_TOP_K_WITH_RERANK] - ANN + exact distance rerank
/// 3. [#ANN_THEN_FILTER] - ANN first, then apply metadata filter
/// 4. [#ANN_THEN_FILTER_THEN_RERANK] - ANN, filter, then rerank survivors
/// 5. [#FILTER_THEN_ANN] - apply filter first, then ANN on filtered subset
/// 6. [#ANN_THRESHOLD_SCAN] - ANN candidates + exact threshold refinement
/// 7. [#ANN_THRESHOLD_THEN_FILTER] - ANN candidates + threshold + filter
/// 8. [#EXACT_SCAN] - brute-force scan of all vectors
