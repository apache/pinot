...
public final class CodecSpecParser {
  ...
  ///
  /// This name is permanently reserved — it cannot be registered as a codec name. Runtime registries must likewise
  /// reject this reserved name.
  ...
  public static final int MAX_ARGS_PER_STAGE = 16;
  ...
  public static CodecPipeline parse(String spec) {
    ...
    if (trimmed.length() > MAX_SPEC_LENGTH) {
      throw new IllegalArgumentException(
          "codecSpec length " + trimmed.length() + " exceeds maximum " + MAX_SPEC_LENGTH);
    }
  ...
  private static final class Parser {
    ...
    CodecPipeline parsePipelineOrInvocation() {
      ...
      if (PIPELINE_KEYWORD.equalsIgnoreCase(name) && peek() == '(') {
        ...
        addStage(stages, parseInvocation());
        while (peek() == ',') {
          consume(',');
          addStage(stages, parseInvocation());
        }
      ...
      return new CodecPipeline(List.of(new CodecInvocation(name, args)));
    }

    void addStage(List<CodecInvocation> stages, CodecInvocation invocation) {
    ...
    String parseName() {
      ...
      if (_pos - start > MAX_IDENTIFIER_LENGTH) {
        throw new IllegalArgumentException(
            "Codec name length " + (_pos - start) + " exceeds maximum " + MAX_IDENTIFIER_LENGTH + " in: " + _input);
      }
    ...
    List<String> parseArgs() {
      ...
      args.add(parseArg());
      while (peek() == ',') {
        if (args.size() >= MAX_ARGS_PER_STAGE) {
          throw new IllegalArgumentException(
              "Codec invocation exceeds maximum " + MAX_ARGS_PER_STAGE + " arguments in: " + _input);
        }
        consume(',');
        args.add(parseArg());
      }
    ...
    String parseArg() {
      ...
      if (_pos - start > MAX_ARG_LENGTH) {
        throw new IllegalArgumentException(
            "Codec argument length " + (_pos - start) + " exceeds maximum " + MAX_ARG_LENGTH + " in: " + _input);
      }
