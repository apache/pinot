...
///
/// The planner for a concrete evaluator is responsible for binding scalar functions and constructing the root
/// [ExecutableNode]. This class owns the repeated row/value execution logic so ingestion and partition paths do
/// not maintain separate node evaluators.
///
/// **Thread-safety:** Once constructed, an `ExecutableFunctionEvaluator` instance is safe for
/// concurrent invocation by multiple threads and for re-entrant invocation on a single thread.
/// [FunctionNode] allocates a fresh argument scratch array per `execute` call, so neither
...
public class ExecutableFunctionEvaluator implements org.apache.pinot.spi.function.FunctionEvaluator {
  ...
  ///
  /// Allocates a fresh `Object[argumentNodes.length]` per `execute` call. This is safe under both
