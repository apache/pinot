...
public class ColumnMetadataImpl implements ColumnMetadata {
  ...
  private final Map<String, Long> _persistedIndexSizesInBytes;
  ...
  private ColumnMetadataImpl(FieldSpec fieldSpec, int totalDocs, int cardinality, boolean hasDictionary,
      ...
      @Nullable CompressionMetadata compressionMetadata, Map<String, Long> persistedIndexSizesInBytes) {
    ...
    _persistedIndexSizesInBytes = persistedIndexSizesInBytes;
  }
  ...
  @Override
  public Map<String, Long> getPersistedIndexSizesInBytes() {
    return _persistedIndexSizesInBytes;
  }
  ...
  private static Map<String, Long> readPersistedIndexSizes(PropertiesConfiguration config, String column) {
    ...
    for (String key : CommonsConfigurationUtils.getKeys(config)) {
      ...
      String indexTypeId = key.substring(keyPrefix.length());
      if (indexTypeId.isEmpty()) {
        continue;
      }
      try {
        long size = config.getLong(key);
        if (size < 0) {
          continue;
        }
        if (indexSizes == null) {
        ...
        indexSizes.put(indexTypeId, size);
      } catch (Exception e) {
        // Advisory statistic: a malformed value is dropped rather than failing the segment load. This class has no
        // logger, and unlike `parseCompressionType` there is nothing here worth aborting a load over.
        continue;
      }
    }
  ...
  public static class Builder {
    ...
    private Map<String, Long> _persistedIndexSizesInBytes = Map.of();

    public Builder setPersistedIndexSizesInBytes(Map<String, Long> persistedIndexSizesInBytes) {
      _persistedIndexSizesInBytes = persistedIndexSizesInBytes;
      return this;
    }
    ...
    public ColumnMetadataImpl build() {
      ...
      return new ColumnMetadataImpl(_fieldSpec, _totalDocs, _cardinality, _hasDictionary, _forwardIndexEncoding,
          ...
          CompressionMetadata.create(_uncompressedValueSizeInBytes, _forwardIndexChunkCompressionType,
              _dictionaryUncompressedValueSizeInBytes), _persistedIndexSizesInBytes);
    }
  }
}
