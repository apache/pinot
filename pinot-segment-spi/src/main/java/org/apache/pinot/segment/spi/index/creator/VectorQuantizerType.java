...
///
/// Quantizers compress or transform raw float32 vectors into a more compact encoded form.
/// The quantizer type controls how vectors are stored on disk and how approximate distances
/// are computed during search.
///
/// - [#FLAT] - Identity quantizer; stores raw float32 vectors without compression.
/// - [#SQ8] - Scalar quantization to 8-bit unsigned integers per dimension.
/// - [#SQ4] - Scalar quantization to 4-bit unsigned integers per dimension.
/// - [#PQ] - Product quantization, splitting vectors into sub-vectors and encoding
///      each with a codebook index.
public enum VectorQuantizerType {
  ...
  ///
  /// - `FLAT`: 4 bytes per dimension (float32).
  /// - `SQ8`: 1 byte per dimension.
  /// - `SQ4`: ceil(dimension / 2) bytes (4 bits per dimension, packed).
  /// - `PQ`: not deterministic from dimension alone; returns -1. PQ encoding size
  ///      depends on pqM and pqNbits, which are not available at the enum level.
  ///
  ...
  /// @return the corresponding [VectorQuantizerType]
