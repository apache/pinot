...
/// Implementations of this interface can be used to create indexes in realtime tables and at the same time to read
/// them.
///
/// This interface extends [IndexReader] as a marker interface and implementations are encouraged to extend the
...
///
/// This interface explicitly do not extend [org.apache.pinot.segment.spi.index.IndexCreator] and usually
/// implementations should not implement that interface. As explained in their Javadoc, IndexCreators are designed to
/// add rows in docId order. MutableIndexes, on the other hand, can be modified adding indexes in any order and
/// therefore their [#add(Object, int, int)] methods include an extra parameter that indicates the docId of the
/// value to add.
public interface MutableIndex extends IndexReader {

  /// Adds the given single value cell to the index.
  ///
  /// Unlike [org.apache.pinot.segment.spi.index.IndexCreator#add(Object, int)], rows can be added in no
  ...
  void add(Object value, int dictId, int docId);
  ...
  /// @param docId The document id of the given row. A non-negative value.
  void add(Object[] values, @Nullable int[] dictIds, int docId);
