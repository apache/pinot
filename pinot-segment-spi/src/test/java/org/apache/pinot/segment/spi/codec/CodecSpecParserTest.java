...
public class CodecSpecParserTest {
  ...
  @Test(expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp = ".*Leading sign is not allowed.*")
  public void testRejectsPositiveSign() {
    CodecSpecParser.parse("ZSTD(+3)");
  }

  @Test(expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp = ".*Leading sign is not allowed.*")
  public void testRejectsNegativeSign() {
    CodecSpecParser.parse("ZSTD(-3)");
  }
  ...
  @Test(expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp = ".*codecSpec length.*exceeds maximum.*")
  public void testRejectsOversizedSpec() {
    CodecSpecParser.parse("A".repeat(CodecSpecParser.MAX_SPEC_LENGTH + 1));
  }

  @Test(expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp = ".*exceeds maximum 32 stages.*")
  public void testRejectsTooManyStages() {
    StringBuilder spec = new StringBuilder("CODEC(");
    for (int i = 0; i <= CodecSpecParser.MAX_PIPELINE_STAGES; i++) {
      if (i > 0) {
        spec.append(',');
      }
      spec.append("LZ4");
    }
    CodecSpecParser.parse(spec.append(')').toString());
  }

  @Test(expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp = ".*Codec name length.*exceeds maximum.*")
  public void testRejectsOversizedIdentifier() {
    CodecSpecParser.parse("A".repeat(CodecSpecParser.MAX_IDENTIFIER_LENGTH + 1));
  }

  @Test(expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp = ".*Codec argument length.*exceeds maximum.*")
  public void testRejectsOversizedArgument() {
    CodecSpecParser.parse("ZSTD(" + "1".repeat(CodecSpecParser.MAX_ARG_LENGTH + 1) + ")");
  }

  @Test(expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp = ".*exceeds maximum 16 arguments.*")
  public void testRejectsTooManyArguments() {
    StringBuilder spec = new StringBuilder("FPC(");
    for (int i = 0; i <= CodecSpecParser.MAX_ARGS_PER_STAGE; i++) {
      if (i > 0) {
        spec.append(',');
      }
      spec.append(i);
