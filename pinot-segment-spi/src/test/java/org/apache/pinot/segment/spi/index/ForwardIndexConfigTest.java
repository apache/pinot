...
public class ForwardIndexConfigTest {
  ...
  @Test
  public void withLegacyLz4LengthPrefixedChunkCompressionType() {
    assertEquals(ForwardIndexConfig.getActualCompressionCodec(null, ChunkCompressionType.LZ4_LENGTH_PREFIXED, null),
        FieldConfig.CompressionCodec.LZ4);
    assertEquals(ForwardIndexConfig.getActualCompressionCodecSpec(null, ChunkCompressionType.LZ4_LENGTH_PREFIXED, null),
        CompressionCodecSpec.fromString("LZ4"));
  }
  ...
  @Test
  public void testBuilderSupportsStringCompressionCodecSpec() {
    ForwardIndexConfig config = new ForwardIndexConfig.Builder()
        .withCompressionCodecSpec("zstd(3)")
        .build();

    assertEquals(config.getCompressionCodec(), FieldConfig.CompressionCodec.ZSTANDARD);
    assertEquals(config.getCompressionCodecSpec(), CompressionCodecSpec.fromString("ZSTD(3)"));
    assertEquals(config.getChunkCompressionType(), ChunkCompressionType.ZSTANDARD);
  }
}
