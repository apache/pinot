  ...
  private void assignWorkersToPartitionedLeafFragment(DispatchablePlanMetadata metadata,
    ...
    if (numPartitionsPerWorker == 1) {
      ...
      for (int i = 0; i < numWorkers; i++) {
        ...
        Preconditions.checkState(fullyReplicatedServers != null, "Failed to find any segment for table: %s, worker: %s",
            tableName, i);
        ServerInstance serverInstance =
            pickEnabledServer(fullyReplicatedServers, enabledServerInstanceMap, indexToPick++);
        Preconditions.checkState(serverInstance != null,
            "Failed to find enabled fully replicated server for table: %s, worker: %s", tableName, i);
        workedIdToServerInstanceMap.put(workerId, new QueryServerInstance(serverInstance));
