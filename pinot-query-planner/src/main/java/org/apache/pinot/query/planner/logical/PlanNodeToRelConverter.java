  ...
  private static class ConverterVisitor implements PlanNodeVisitor<Void, Void> {
    ...
    public Void visitUnnest(UnnestNode node, Void context) {
      ...
      try {
        ...
        org.apache.calcite.rel.core.CorrelationId corrId = new org.apache.calcite.rel.core.CorrelationId(0);
        RexNode corrRef = rexBuilder.makeCorrel(left.getRowType(), corrId);
        RexNode arrayExprOverCorr;
        if (node.getArrayExpr() instanceof org.apache.pinot.query.planner.logical.RexExpression.InputRef) {
          int idx = ((org.apache.pinot.query.planner.logical.RexExpression.InputRef) node.getArrayExpr()).getIndex();
        ...
        org.apache.calcite.util.ImmutableBitSet requiredCols =
            org.apache.calcite.util.ImmutableBitSet.range(left.getRowType().getFieldCount());
        LogicalCorrelate correlate =
            LogicalCorrelate.create(left, uncollect, corrId, requiredCols, JoinRelType.INNER);
