...
/// [PinotDataDistribution] at node level, the question is whether we should assign this so that number of streams
...
public class PhysicalExchange extends Exchange implements PRelNode {
  ...
  /// variables and don't allow [RelDistribution] or [RelCollation] to be stored in the trait set. The idea is to avoid
  /// duplicate storage of the same information, and to use traits as constraints which are added before any Exchange
  /// nodes are added to the plan.
  private static final RelTraitSet EMPTY_TRAIT_SET = RelTraitSet.createEmpty();
  private final int _nodeId;
  private final List<PRelNode> _pRelInputs;
  /// See documentation for [PhysicalExchange].
  @Nullable
