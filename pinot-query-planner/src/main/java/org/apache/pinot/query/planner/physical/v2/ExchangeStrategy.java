...
public enum ExchangeStrategy {
  ...
  /// `X, X + F, X + 2*F, ...`. Here F is the sub-partitioning factor. Records are assigned based on a
  ...
  /// **Note:** This is different and better than partitioning exchange because the fanout is F, and not N \* (N\*F).
  ...
  /// the join (no fragment split). Inserted by `LookupJoinRule` after worker/exchange assignment;
  /// handled transparently by `PlanFragmentAndMailboxAssignment.processLookupLocalExchange`.
