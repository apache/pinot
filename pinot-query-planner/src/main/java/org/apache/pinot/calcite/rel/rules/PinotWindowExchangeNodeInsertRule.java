...
public class PinotWindowExchangeNodeInsertRule extends RelOptRule {
  ...
  private Window.Group updateLiteralArgumentsInWindowGroup(Window window) {
    ...
    if (offset != null) {
      ...
      if (literal != null) {
        lowerBound = RexWindowBounds.preceding(literal);
        windowChanged = true;
      }
    }
    ...
    if (offset != null) {
      RexLiteral literal = getLiteral(offset, numInputFields, window.constants, projects);
      if (literal != null) {
        upperBound = RexWindowBounds.following(literal);
        windowChanged = true;
      }
    }
  ...
  private void validateWindowFrames(Window.Group windowGroup) {
    ...
    boolean hasBound = (lowerBound.isPreceding() && !lowerBound.isUnbounded()) || (upperBound.isFollowing()
    ...
    if (!windowGroup.isRows) {
      Preconditions.checkState(!hasBound, "Bounded RANGE window frame is currently not supported");
    }
    ...
    Preconditions.checkState(lowerBound.isPreceding() && lowerBound.isUnbounded(),
        String.format("Lower bound must be UNBOUNDED PRECEDING but it is: %s", lowerBound));
    if (windowGroup.orderKeys.getKeys().isEmpty() && !hasRankBasedAgg) {
