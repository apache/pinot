...
public class PinotRelMdSelectivity implements MetadataHandler<BuiltInMetadata.Selectivity> {
  ...
  private double computeSelectivity(@Nullable RexNode predicate, ScanContext ctx,
    ...
    // Through PinotTable, not the provider directly: PinotTable is per-query and memoizes, so a
    // plan pays for each distinct lookup once instead of once per rule application.
    // One gate, shared with PinotTable.getStatistic(): applying the confidence rule separately
    // here let the numerator and denominator of the same estimate disagree.
    double rowCount = table.getUsableRowCount();
    ...
    if (hasTimePredicate && rowCount > 0) {
      ...
      // No clamping to 0: rewriting an absent lower bound as 0 both drops pre-epoch rows from
      // legitimate estimates and, for a predicate with only a negative upper bound, produces
      // startMs > endMs -- an inverted range the store answers with a confident zero.
      long lo = timeLo;
      ...
      OptionalLong estimated = table.estimateRowsInTimeRange(lo, hiExclusive);
  ...
  private double computeConjunctSelectivity(RexNode conjunct, ScanContext ctx, String tableName,
    ...
    // Same shared predicate as the row-count gate, so the two cannot drift apart.
    if (colStats.getNdvConfidence().isUsableForCosting()) {
  ...
  private TimeBound extractTimeBound(RexNode conjunct, String timeCol, ScanContext ctx) {
    ...
    switch (effectiveKind) {
      case GREATER_THAN:
        return new TimeBound(adjustToInclusive(millis, true, true), true);
      case GREATER_THAN_OR_EQUAL:
        return new TimeBound(millis, true);
      case LESS_THAN:
        return new TimeBound(adjustToInclusive(millis, true, false), false);
  ...
  private SearchTimeBounds extractSearchTimeBounds(RexNode conjunct, String timeCol,
    ...
    // Long.MIN_VALUE/MAX_VALUE mean "no bound on this side" and nothing else. An endpoint that is
    // present but not convertible makes the whole range unusable, so bail out to the heuristic
    // guess rather than substituting a sentinel a caller would read as a real bound.
    long lo = Long.MIN_VALUE;
    if (range.hasLowerBound()) {
      Long converted = toInclusiveMillis(range.lowerEndpoint(), range.lowerBoundType(), true);
      if (converted == null) {
        return null;
      }
      lo = converted;
    ...
    if (range.hasUpperBound()) {
      Long converted = toInclusiveMillis(range.upperEndpoint(), range.upperBoundType(), false);
      if (converted == null) {
        return null;
      }
      hi = converted;
  ...
  /// Converts a range endpoint to an INCLUSIVE epoch-millisecond bound, adjusting OPEN
  /// (exclusive) endpoints by one millisecond in the appropriate direction.
  ///
  /// @param endpoint        the comparable endpoint value
  /// @param boundType       Guava [BoundType]
  /// @param isLowerEndpoint `true` when converting the lower endpoint of the range
  /// @return the inclusive millisecond bound, or `null` if the endpoint is not convertible
  @Nullable
  private static Long toInclusiveMillis(Comparable<?> endpoint, BoundType boundType, boolean isLowerEndpoint) {
    Long millis = toEpochMillis(endpoint);
    if (millis == null) {
      return null;
    }
    return adjustToInclusive(millis, boundType == BoundType.OPEN, isLowerEndpoint);
  }

  /// Converts one endpoint value to epoch milliseconds, or `null` when it is not a form this
  /// handler understands.
  ///
  /// Shared by the comparison path and the SEARCH/Sarg path deliberately: they used to convert
  /// independently and accepted different types, so `ts > TIMESTAMP '...'` consulted statistics
  /// while the very same predicate consulted none once Calcite folded it into a Sarg -- which is
  /// what actually reaches the planner for most range predicates.
  @Nullable
  private static Long toEpochMillis(@Nullable Object value) {
    if (value instanceof BigDecimal) {
      // longValueExact, not longValue: the latter discards high-order bits silently, and Calcite
      // represents every numeric literal as BigDecimal. A literal above 2^63 would wrap onto
      // Long.MIN_VALUE / MAX_VALUE -- the very values this class uses to mean "no bound" -- so an
      // impossible predicate would read as unbounded and report selectivity 1.0 instead of
      // admitting it cannot estimate. FLOOR because a bound must not round toward zero: for
      // `ts > -1000.5` the inclusive bound is -1001, not -1000.
      try {
        return ((BigDecimal) value).setScale(0, RoundingMode.FLOOR).longValueExact();
  ...
  /// Shifts an exclusive bound by one millisecond to make it inclusive, saturating instead of
  /// wrapping: `Long.MAX_VALUE + 1` would wrap to `Long.MIN_VALUE`, which this class reads as
  /// "unbounded below" -- turning a bound into its opposite.
  private static long adjustToInclusive(long millis, boolean open, boolean isLowerEndpoint) {
    if (!open) {
      return millis;
    }
    if (isLowerEndpoint) {
      return millis == Long.MAX_VALUE ? Long.MAX_VALUE : millis + 1;
    }
    return millis == Long.MIN_VALUE ? Long.MIN_VALUE : millis - 1;
  }
  ...
  /// Extracts the millisecond value from a [RexLiteral], or `null` when the literal cannot be
  /// interpreted as epoch milliseconds.
  ///
  /// Returning `null` rather than a sentinel matters: every caller falls back to a heuristic guess
  /// when a bound cannot be converted, and an in-band sentinel would instead be read as a real
  /// bound -- producing a confident wrong estimate rather than an admitted unknown.
  @Nullable
  private static Long extractMillis(RexLiteral literal) {
    switch (literal.getType().getSqlTypeName()) {
      ...
      case INTEGER:
      case DECIMAL:
        return toEpochMillis(literal.getValue());
      case TIMESTAMP:
      case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
        // Calcite stores a TIMESTAMP literal as a TimestampString or Calendar; value2 is epoch-ms.
        Long fromValue = toEpochMillis(literal.getValue());
        return fromValue != null ? fromValue : toEpochMillis(literal.getValue2());
      default:
        return null;
    }
  }
