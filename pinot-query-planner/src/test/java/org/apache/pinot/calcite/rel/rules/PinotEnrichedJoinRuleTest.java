...
public class PinotEnrichedJoinRuleTest {
  ...
  public void testOnMatchProjectJoinCase() {
    ...
    LogicalProject project = LogicalProject.create(originalJoin, Collections.emptyList(), projects, List.of("projectCol1"));
  ...
  @Test
  public void testOnMatchSortJoinCase() {
    // Create a RelOptRuleCall for a filter above join
    RelDataType intType = TYPE_FACTORY.builder()
        .add("col1", SqlTypeName.INTEGER)
        .add("col2", SqlTypeName.INTEGER)
    ...
    LogicalJoin originalJoin = LogicalJoin.create(_input, _input, Collections.emptyList(), joinCondition, Collections.emptySet(),
        JoinRelType.INNER);
    // project one column
    RelCollation collations = RelCollations.of(1);
    ...
    LogicalSort sort = LogicalSort.create(originalJoin, collations, offset, fetch);
    Mockito.when(_call.rel(0)).thenReturn(sort);
    Mockito.when(_call.rel(1)).thenReturn(originalJoin);
    PinotEnrichedJoinRule rule = PinotEnrichedJoinRule.Config.SORT_JOIN.toRule();
    Mockito.when(_call.getRule()).thenReturn(rule);
    rule.onMatch(_call);
    ...
    Mockito.verify(_call, Mockito.times(1)).transformTo(logicalEnrichedNodeCapture.capture());
    PinotLogicalEnrichedJoin enrichedJoin = (PinotLogicalEnrichedJoin) logicalEnrichedNodeCapture.getValue();
    assertEquals(enrichedJoin.getCollation(), collations);
    assertEquals(enrichedJoin.getFetch(), fetch);
  ...
  @Test
  public void testRuleWithPlannerSortJoinCase() {
    HepProgramBuilder hepProgramBuilder = new HepProgramBuilder();
    hepProgramBuilder.addRuleCollection(PinotEnrichedJoinRule.PINOT_ENRICHED_JOIN_RULES);
    HepPlanner planner = new HepPlanner(hepProgramBuilder.build());
    RelOptCluster cluster = RelOptCluster.create(planner, REX_BUILDER);
  ...
  @Test
  public void testRuleWithPlannerSortFilterJoinCase() {
    HepProgramBuilder hepProgramBuilder = new HepProgramBuilder();
    hepProgramBuilder.addRuleCollection(PinotEnrichedJoinRule.PINOT_ENRICHED_JOIN_RULES);
    HepPlanner planner = new HepPlanner(hepProgramBuilder.build());
    RelOptCluster cluster = RelOptCluster.create(planner, REX_BUILDER);
    ...
    RexNode offset = REX_BUILDER.makeLiteral(1, TYPE_FACTORY.createSqlType(SqlTypeName.INTEGER));
    RexNode fetch = REX_BUILDER.makeLiteral(2, TYPE_FACTORY.createSqlType(SqlTypeName.INTEGER));
    LogicalSort sort = LogicalSort.create(originalFilter, collations, offset, fetch);
  ...
  @Test
  public void testRuleWithPlannerSortProjectJoinCase() {
    HepProgramBuilder hepProgramBuilder = new HepProgramBuilder();
    hepProgramBuilder.addRuleCollection(PinotEnrichedJoinRule.PINOT_ENRICHED_JOIN_RULES);
    HepPlanner planner = new HepPlanner(hepProgramBuilder.build());
    RelOptCluster cluster = RelOptCluster.create(planner, REX_BUILDER);
  ...
  @Test
  public void testRuleWithPlannerSortProjectFilterJoinCase() {
    HepProgramBuilder hepProgramBuilder = new HepProgramBuilder();
    hepProgramBuilder.addRuleCollection(PinotEnrichedJoinRule.PINOT_ENRICHED_JOIN_RULES);
    HepPlanner planner = new HepPlanner(hepProgramBuilder.build());
    RelOptCluster cluster = RelOptCluster.create(planner, REX_BUILDER);
