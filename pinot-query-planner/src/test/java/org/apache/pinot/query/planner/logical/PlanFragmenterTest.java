...
/// Tests the `sortedOnSender` marking that [PlanFragmenter] applies when the
/// `sortedSelectionMergeEnabled` query option is on.
///
/// This marking is the only gate between the query option and the k-way merge in
/// `SortedMailboxReceiveOperator`, and a false positive is not a crash: the merge assumes each mailbox stream is
/// sorted, nothing downstream re-sorts, and under `LIMIT`/`OFFSET` the query simply returns the wrong rows.
...
public class PlanFragmenterTest {
  ...
  public Object[][] senderShapes() {
    return new Object[][]{
        ...
        {"filter over scan", (Supplier<PlanNode>) () -> sort(filter(scan(OFFLINE_ONLY_TABLE))), true},
        {"project over filter over scan",
            (Supplier<PlanNode>) () -> sort(project(filter(scan(OFFLINE_ONLY_TABLE)))), true},
        {"filter over project over scan",
            (Supplier<PlanNode>) () -> sort(filter(project(scan(OFFLINE_ONLY_TABLE)))), true},
        {"two filters over scan",
            (Supplier<PlanNode>) () -> sort(filter(filter(scan(OFFLINE_ONLY_TABLE)))), true},
        ...
        {"filter over aggregate below sort",
            (Supplier<PlanNode>) () -> sort(filter(aggregate(scan(OFFLINE_ONLY_TABLE)))), false},
        ...
        {"filter over hybrid table", (Supplier<PlanNode>) () -> sort(filter(scan(HYBRID_TABLE))), false},
  ...
  /// When the exchange itself already declares `sortOnSender`, the flag must stay set no matter what the option or
  ...
  /// A null [TableCache] must fail closed: no table can be proven single-physical, so nothing is marked.
  @Test
