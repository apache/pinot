...
public class PinotTableStatisticTest {
  ...
  /// The statistics store is written continuously by the Helix cluster-change threads that collect
  /// segment metadata. If one PinotTable read it more than once, Calcite could take the scan
  /// cardinality from one snapshot while the selectivity handler divides by another, and the same
  /// query would plan differently from compile to compile. Pin the read count.
  ///
  /// This is the test that was missing when `computeStatistic()` bypassed the memo.
  @Test
  public void testStatisticsAreFetchedAtMostOncePerTable() {
    PinotStatisticsProvider provider = mock(PinotStatisticsProvider.class);
    when(provider.getTableStatistics(TABLE_NAME)).thenReturn(
        TableStatistics.builder().rowCount(1234L, StatConfidence.EXACT).build());

    PinotTable table = new PinotTable(buildMinimalSchema(), false, TABLE_NAME, provider);
    // Both accessors, twice each, and via both entry points the planner actually uses.
    table.getStatistic();
    table.getStatistic();
    table.getTableStatistics();
    table.getUsableRowCount();

    verify(provider, times(1)).getTableStatistics(TABLE_NAME);
  }

  /// A confidence tier added to the append-only enum later must be rejected by default, not
  /// silently trusted. The gate is therefore an allow-list, and both the row-count and NDV paths
  /// share it.
  @Test
  public void testOnlyExactAndEstimatedAreUsableForCosting() {
    assertTrue(StatConfidence.EXACT.isUsableForCosting());
    assertTrue(StatConfidence.ESTIMATED.isUsableForCosting());
    assertFalse(StatConfidence.LOW.isUsableForCosting());
    assertFalse(StatConfidence.UNKNOWN.isUsableForCosting());
  }
}
