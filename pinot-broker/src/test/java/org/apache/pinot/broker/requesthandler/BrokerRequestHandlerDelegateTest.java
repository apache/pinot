...
public class BrokerRequestHandlerDelegateTest {
  ...
  @Test
  public void testPreAdmittedSqlQueryContinuesWhenDrainStartsBeforeDelegateExecution()
      throws Exception {
    BaseSingleStageBrokerRequestHandler singleStageBrokerRequestHandler =
        Mockito.mock(BaseSingleStageBrokerRequestHandler.class);
    BrokerDrainManager drainManager = BrokerDrainManager.localOnly("Broker_localhost_8099", () -> {
    ...
    BrokerDrainManager.QueryPermit transportPermit = drainManager.tryAcquireQuery();
    assertNotNull(transportPermit);
    try {
      BrokerDrainManager.DrainStatus drainStatus = drainManager.drain(0L, false);
      assertFalse(drainStatus.isDrained());

      BrokerResponse response = delegate.handleRequestWithPreAcquiredPermit(
          sqlRequest("SELECT * FROM myTable"), null, null, new DefaultRequestContext(), null);

      assertEquals(response.getExceptionsSize(), 0);
      assertEquals(drainManager.getStatus().getInFlightQueries(), 1);
      verify(singleStageBrokerRequestHandler).handleRequest(any(), any(), any(), any(), any());
    } finally {
      transportPermit.close();
    }
    assertEquals(drainManager.getStatus().getInFlightQueries(), 0);
  }
