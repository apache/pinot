...
public class BrokerRoutingManager implements RoutingManager, ClusterChangeHandler {
  ...
  private final EnabledServerInstanceManager _enabledServerManager = new EnabledServerInstanceManager();
  ...
  private void processInstanceConfigChange() {
    ...
    for (ZNRecord instanceConfigZNRecord : instanceConfigZNRecords) {
      ...
      try {
        if (isEnabledServer(instanceConfigZNRecord)) {
          ...
          if (_enabledServerManager.getServers().put(instanceId, serverInstance) == null) {
    ...
    for (String instance : _enabledServerManager.getServers().keySet()) {
    ...
    for (String newDisabledInstance : newDisabledServers) {
      _enabledServerManager.getServers().remove(newDisabledInstance);
    }
  ...
  public synchronized void includeServerToRouting(String instanceId) {
    ...
    if (!_enabledServerManager.getServers().containsKey(instanceId)) {
      LOGGER.info("Server: {} is not enabled, skipping updating the routing", instanceId);
      return;
    }
  ...
  public synchronized void buildRouting(String tableNameWithType) {
    ...
    instanceSelector.init(_routableServers, _enabledServerManager, idealState, externalView, preSelectedOnlineSegments);
  ...
  private Map<ServerInstance, ServerRouteInfo> getServerInstanceToSegmentsMap(String tableNameWithType,
    ...
    for (Map.Entry<String, String> entry : selectionResult.getSegmentToInstanceMap().entrySet()) {
      ServerInstance serverInstance = _enabledServerManager.getServers().get(entry.getValue());
  ...
  public Map<String, ServerInstance> getEnabledServerInstanceMap() {
    return _enabledServerManager.getServers();
  }
  ...
  public static class EnabledServerInstanceManager {
