...
public class SystemTableBrokerRequestHandler extends BaseBrokerRequestHandler {
  ...
  private static final Set<String> HOP_BY_HOP_HEADERS_TO_SKIP =
      Set.of("connection", "content-length", "transfer-encoding", "host");
  ...
  public void shutDown() {
    ...
    try {
      ...
      LOGGER.debug("Failed to close system table scatter-gather http client: {}", e.toString());
    }
    try {
      _scatterGatherConnMgr.close();
    } catch (Exception e) {
      LOGGER.debug("Failed to close system table scatter-gather connection manager: {}", e.toString());
    }
  ...
  protected Map<ServerRoutingInstance, DataTable> scatterGatherSystemTableDataTables(SystemTableProvider provider,
    ...
    ServerRoutingInstance routingInstance = localRoutingInstance != null ? localRoutingInstance
        : new ServerRoutingInstance(SYSTEM_TABLE_PSEUDO_HOST, SYSTEM_TABLE_PSEUDO_PORT, TableType.OFFLINE);
    try {
  ...
  private DataTable fetchDataTableFromBroker(String url, String requestBody, Map<String, String> requestHeaders,
    ...
    try (CloseableHttpResponse response = _scatterGatherHttpClient.execute(httpPost)) {
      int status = response.getCode();
      if (status != 200) {
        String error = EntityUtils.toString(response.getEntity());
        return exceptionDataTable(QueryErrorCode.BROKER_REQUEST_SEND,
            String.format("Scatter-gather system table request failed for %s (status=%d): %s", url, status, error));
      }
      byte[] bytes = EntityUtils.toByteArray(response.getEntity());
      if (bytes.length == 0) {
        return exceptionDataTable(QueryErrorCode.BROKER_REQUEST_SEND,
            "Scatter-gather system table request returned an empty response for: " + url);
      }
