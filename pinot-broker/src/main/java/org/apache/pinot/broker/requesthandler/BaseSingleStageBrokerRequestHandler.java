...
public abstract class BaseSingleStageBrokerRequestHandler extends BaseBrokerRequestHandler {
  ...
  private static class CompiledPinotQuery {
    private PinotQuery _pinotQuery;
    private PinotQuery _serverPinotQuery;
    private Schema _schema;
    private String _tableName;
    private String _rawTableName;
    private BrokerResponseNative _brokerResponseNative;
  }

  private CompiledPinotQuery compilePinotQuery(long requestId, String query, SqlNodeAndOptions sqlNodeAndOptions,
      JsonNode request, @Nullable RequesterIdentity requesterIdentity, RequestContext requestContext,
      @Nullable HttpHeaders httpHeaders, AccessControl accessControl) {
    // Compile the request into PinotQuery
    CompiledPinotQuery compiledPinotQuery = new CompiledPinotQuery();
    ...
    try {
      ...
      if (ParserUtils.canCompileWithMultiStageEngine(query, database, _tableCache)) {
        compiledPinotQuery._brokerResponseNative = new BrokerResponseNative(
            QueryException.getException(QueryException.SQL_PARSING_ERROR, new Exception(
        ...
        return compiledPinotQuery;
      } else {
        compiledPinotQuery._brokerResponseNative =
            new BrokerResponseNative(QueryException.getException(QueryException.SQL_PARSING_ERROR, e));
        return compiledPinotQuery;
      }
    }

    compiledPinotQuery._pinotQuery = pinotQuery;
    ...
    if (isLiteralOnlyQuery(pinotQuery)) {
      ...
      try {
        if (pinotQuery.isExplain()) {
          ...
          compiledPinotQuery._brokerResponseNative = BrokerResponseNative.BROKER_ONLY_EXPLAIN_PLAN_OUTPUT;
          return compiledPinotQuery;
        }
        compiledPinotQuery._brokerResponseNative = processLiteralOnlyQuery(requestId, pinotQuery, requestContext);
        return compiledPinotQuery;
      } catch (Exception e) {
    ...
    compiledPinotQuery._serverPinotQuery = serverPinotQuery;
    ...
    try {
      ...
      compiledPinotQuery._brokerResponseNative =
          new BrokerResponseNative(QueryException.getException(QueryException.QUERY_EXECUTION_ERROR, e));
      return compiledPinotQuery;
    }
    ...
    compiledPinotQuery._tableName = tableName;
    dataSource.setTableName(compiledPinotQuery._tableName);
    String rawTableName = TableNameBuilder.extractRawTableName(compiledPinotQuery._tableName);
    compiledPinotQuery._rawTableName = rawTableName;
    ...
    try {
      ...
      if (e instanceof BadQueryRequestException) {
        ...
        compiledPinotQuery._brokerResponseNative =
            new BrokerResponseNative(QueryException.getException(QueryException.UNKNOWN_COLUMN_ERROR, e));
        return compiledPinotQuery;
      }
    ...
    compiledPinotQuery._schema = schema;
    if (schema != null) {
  ...
  protected BrokerResponse handleRequest(long requestId, String query, SqlNodeAndOptions sqlNodeAndOptions,
    ...
    try {
      ...
      CompiledPinotQuery compiledPinotQuery =
          compilePinotQuery(requestId, query, sqlNodeAndOptions, request, requesterIdentity, requestContext,
              httpHeaders, accessControl);
      long compilationEndTimeNs = System.nanoTime();

      if (compiledPinotQuery._brokerResponseNative != null) {
        return compiledPinotQuery._brokerResponseNative;
      }
      ...
      BrokerRequest brokerRequest = CalciteSqlCompiler.convertToBrokerRequest(compiledPinotQuery._pinotQuery);
      BrokerRequest serverBrokerRequest =
          compiledPinotQuery._serverPinotQuery == compiledPinotQuery._pinotQuery ? brokerRequest
              : CalciteSqlCompiler.convertToBrokerRequest(compiledPinotQuery._serverPinotQuery);
      AuthorizationResult authorizationResult = accessControl.authorize(requesterIdentity, serverBrokerRequest);
      if (authorizationResult.hasAccess()) {
        authorizationResult =
            accessControl.authorize(httpHeaders, TargetType.TABLE, compiledPinotQuery._tableName, Actions.Table.QUERY);
      }

      _brokerMetrics.addPhaseTiming(compiledPinotQuery._rawTableName, BrokerQueryPhase.AUTHORIZATION,
          System.nanoTime() - compilationEndTimeNs);

      if (!authorizationResult.hasAccess()) {
        _brokerMetrics.addMeteredTableValue(compiledPinotQuery._tableName,
            BrokerMeter.REQUEST_DROPPED_DUE_TO_ACCESS_ERROR, 1);
        LOGGER.info("Access denied for request {}: {}, table: {}, reason :{}", requestId, query,
            compiledPinotQuery._tableName, authorizationResult.getFailureMessage());
      ...
      // Validate QPS quota
      String database = DatabaseUtils.extractDatabaseFromFullyQualifiedTableName(compiledPinotQuery._tableName);
      ...
      try {
        validateRequest(compiledPinotQuery._serverPinotQuery, _queryResponseLimit);
      } catch (Exception e) {
        LOGGER.info("Caught exception while validating request {}: {}, {}", requestId, query, e.getMessage());
        requestContext.setErrorCode(QueryException.QUERY_VALIDATION_ERROR_CODE);
        _brokerMetrics.addMeteredTableValue(compiledPinotQuery._rawTableName, BrokerMeter.QUERY_VALIDATION_EXCEPTIONS,
            1);
      ...
      TableRoutingContext tableRoutingContext = getTablesHitByRequest(requestId, compiledPinotQuery._tableName, query);
      if (tableRoutingContext._exception != null) {
        requestContext.setErrorCode(tableRoutingContext._exception.getErrorCode());
        return new BrokerResponseNative(tableRoutingContext._exception);
      }

      // Handle query rewrite that can be overridden by the table configs
      HandlerContext handlerContext =
          getHandlerContext(tableRoutingContext._offlineTableConfig, tableRoutingContext._realtimeTableConfig);
      if (handlerContext._disableGroovy) {
        rejectGroovyQuery(compiledPinotQuery._serverPinotQuery);
      }
      if (handlerContext._useApproximateFunction) {
        handleApproximateFunctionOverride(compiledPinotQuery._serverPinotQuery);
      }

      // Prepare OFFLINE and REALTIME requests
      BrokerRequest offlineBrokerRequest =
          tableRoutingContext.getOfflineBrokerRequest(compiledPinotQuery._serverPinotQuery, compiledPinotQuery._schema,
              _tableCache, _queryOptimizer);
      BrokerRequest realtimeBrokerRequest =
          tableRoutingContext.getRealtimeBrokerRequest(compiledPinotQuery._serverPinotQuery, compiledPinotQuery._schema,
              _tableCache, _queryOptimizer);
      tableRoutingContext.setFanoutTypeAndTenants(requestContext, _tableCache);

      if (offlineBrokerRequest == null && realtimeBrokerRequest == null) {
        return getEmptyBrokerOnlyResponse(compiledPinotQuery._pinotQuery, requestContext, compiledPinotQuery._tableName,
            requesterIdentity);
      }
      ...
      boolean offlineTableDisabled = tableRoutingContext.canRouteOffline() ? false
          : _routingManager.isTableDisabled(tableRoutingContext.getOfflineTableName());

      boolean realtimeTableDisabled = tableRoutingContext.canRouteRealtime() ? false
          : _routingManager.isTableDisabled(tableRoutingContext.getRealtimeTableName());
      ...
      if (realtimeBrokerRequest != null && !realtimeTableDisabled && (!compiledPinotQuery._pinotQuery.isExplain()
          || offlineBrokerRequest != null)) {
      ...
      if (offlineTableDisabled || realtimeTableDisabled) {
        ...
        if ((tableRoutingContext.canRouteHybrid() && (offlineTableDisabled && realtimeTableDisabled))
            || (!tableRoutingContext.canRouteOffline() && realtimeTableDisabled)
            || (!tableRoutingContext.canRouteRealtime() && offlineTableDisabled)) {
          requestContext.setErrorCode(QueryException.TABLE_IS_DISABLED_ERROR_CODE);
          return BrokerResponseNative.TABLE_IS_DISABLED;
        } else if (tableRoutingContext.canRouteHybrid() && realtimeTableDisabled) {
          errorMessage = "Realtime table is disabled in hybrid table";
        } else if (tableRoutingContext.canRouteHybrid() && offlineTableDisabled) {
      ...
      if (numUnavailableSegments > 0) {
        ...
        String realtimeRoutingPolicy =
            realtimeBrokerRequest != null ? getRoutingPolicy(tableRoutingContext._realtimeTableConfig) : null;
        String offlineRoutingPolicy =
            offlineBrokerRequest != null ? getRoutingPolicy(tableRoutingContext._offlineTableConfig) : null;
        ...
        _brokerMetrics.addMeteredTableValue(compiledPinotQuery._rawTableName,
      ...
      if (offlineBrokerRequest == null && realtimeBrokerRequest == null) {
        if (!exceptions.isEmpty()) {
          ...
          _brokerMetrics.addMeteredTableValue(compiledPinotQuery._rawTableName, BrokerMeter.NO_SERVER_FOUND_EXCEPTIONS,
              1);
          return new BrokerResponseNative(exceptions);
        } else {
          // When all segments have been pruned, we can just return an empty response.
          return getEmptyBrokerOnlyResponse(compiledPinotQuery._pinotQuery, requestContext,
              compiledPinotQuery._tableName, requesterIdentity);
        }
      }
      long routingEndTimeNs = System.nanoTime();
      _brokerMetrics.addPhaseTiming(compiledPinotQuery._rawTableName, BrokerQueryPhase.QUERY_ROUTING,
      ...
      try {
        if (offlineBrokerRequest != null) {
          remainingTimeMs = setQueryTimeout(tableRoutingContext.getOfflineTableName(),
              offlineBrokerRequest.getPinotQuery().getQueryOptions(), timeSpentMs);
        }
        if (realtimeBrokerRequest != null) {
          remainingTimeMs = Math.max(remainingTimeMs, setQueryTimeout(tableRoutingContext.getRealtimeTableName(),
      ...
      if (offlineBrokerRequest != null) {
        ...
        setMaxServerResponseSizeBytes(numQueriesIssued, queryOptions, tableRoutingContext._offlineTableConfig);
      ...
      if (realtimeBrokerRequest != null) {
        ...
        setMaxServerResponseSizeBytes(numQueriesIssued, queryOptions, tableRoutingContext._realtimeTableConfig);
        // Set the query option to directly return final result for single server query unless it is explicitly disabled
        if (numQueriesIssued == 1) {
          // Set the same flag in the original server request to be used in the reduce phase for hybrid table
          if (queryOptions.putIfAbsent(QueryOptionKey.SERVER_RETURN_FINAL_RESULT, "true") == null
      ...
      _brokerMetrics.addPhaseTiming(compiledPinotQuery._rawTableName, BrokerQueryPhase.QUERY_EXECUTION,
      ...
      if (QueryOptionsUtils.shouldDropResults(compiledPinotQuery._pinotQuery.getQueryOptions())) {
        brokerResponse.setResultTable(null);
      }
      _brokerMetrics.addTimedTableValue(compiledPinotQuery._rawTableName, BrokerTimer.QUERY_TOTAL_TIME_MS, totalTimeMs,
          TimeUnit.MILLISECONDS);

      // Log query and stats
      _queryLogger.log(new QueryLogger.QueryLogParams(requestContext, compiledPinotQuery._tableName, brokerResponse,
  ...
  private static class TableRoutingContext {
    private TableConfig _realtimeTableConfig;
    private TableConfig _offlineTableConfig;
    private TimeBoundaryInfo _timeBoundaryInfo;
    private ProcessingException _exception;

    @Nullable
    public String getOfflineTableName() {
      return _offlineTableConfig != null ? _offlineTableConfig.getTableName() : null;
    }

    @Nullable
    public String getRealtimeTableName() {
      return _realtimeTableConfig != null ? _realtimeTableConfig.getTableName() : null;
    }

    public boolean canRouteRealtime() {
      return _realtimeTableConfig != null;
    }

    public boolean canRouteOffline() {
      return _offlineTableConfig != null;
    }

    public boolean canRouteHybrid() {
      return canRouteRealtime() && canRouteOffline();
    }

    @Nullable
    private BrokerRequest getBrokerRequest(@Nullable TableConfig tableConfig, PinotQuery serverPinotQuery,
        Schema schema, TableCache tableCache, QueryOptimizer queryOptimizer) {
      if (tableConfig == null) {
        return null;
      }
      String tableName = tableConfig.getTableName();
      PinotQuery pinotQuery = serverPinotQuery.deepCopy();
      pinotQuery.getDataSource().setTableName(tableName);

      if (canRouteHybrid()) {
        attachTimeBoundary(pinotQuery, _timeBoundaryInfo, TableNameBuilder.isOfflineTableResource(tableName));
      }

      handleExpressionOverride(pinotQuery, tableCache.getExpressionOverrideMap(tableName));
      handleTimestampIndexOverride(pinotQuery, tableCache, tableConfig);
      queryOptimizer.optimize(pinotQuery, tableConfig, schema);

      if (isFilterAlwaysFalse(pinotQuery)) {
        return null;
      }

      if (isFilterAlwaysTrue(pinotQuery)) {
        pinotQuery.setFilterExpression(null);
      }
    ...
    @Nullable
    public BrokerRequest getOfflineBrokerRequest(PinotQuery serverPinotQuery, Schema schema, TableCache tableCache,
        QueryOptimizer queryOptimizer) {
      return getBrokerRequest(_offlineTableConfig, serverPinotQuery, schema, tableCache, queryOptimizer);
    }

    @Nullable
    public BrokerRequest getRealtimeBrokerRequest(PinotQuery serverPinotQuery, Schema schema, TableCache tableCache,
        QueryOptimizer queryOptimizer) {
      return getBrokerRequest(_realtimeTableConfig, serverPinotQuery, schema, tableCache, queryOptimizer);
    }

    public void setFanoutTypeAndTenants(RequestContext requestContext, TableCache tableCache) {
      if (canRouteHybrid()) {
        requestContext.setFanoutType(RequestContext.FanoutType.HYBRID);
      } else if (canRouteOffline()) {
      ...
      if (canRouteOffline()) {
        requestContext.setOfflineServerTenant(getServerTenant(getOfflineTableName(), tableCache));
      }
      if (canRouteRealtime()) {
        requestContext.setRealtimeServerTenant(getServerTenant(getRealtimeTableName(), tableCache));
      }
    }
  }

  private TableRoutingContext getTablesHitByRequest(long requestId, String tableName, String query) {

    boolean realtimeRoutingExists = false;
    boolean offlineRoutingExists = false;
    TableType tableType = TableNameBuilder.getTableTypeFromTableName(tableName);

    if (tableType == TableType.OFFLINE) {
      // Offline table
      if (_routingManager.routingExists(tableName)) {
        offlineRoutingExists = true;
      }
    } else if (tableType == TableType.REALTIME) {
      // Realtime table
      if (_routingManager.routingExists(tableName)) {
        realtimeRoutingExists = true;
      }
    } else {
      // Hybrid table (check both OFFLINE and REALTIME)
      if (_routingManager.routingExists(TableNameBuilder.OFFLINE.tableNameWithType(tableName))) {
        offlineRoutingExists = true;
      }
      if (_routingManager.routingExists(TableNameBuilder.REALTIME.tableNameWithType(tableName))) {
        realtimeRoutingExists = true;
      }
    }
    ...
    TableRoutingContext tableRoutingContext = new TableRoutingContext();
    if (!offlineRoutingExists && !realtimeRoutingExists) {
      ...
      if (realtimeTableConfig == null && offlineTableConfig == null) {
        ...
        tableRoutingContext._exception = QueryException.TABLE_DOES_NOT_EXIST_ERROR;
        return tableRoutingContext;
      }
      ...
      tableRoutingContext._exception = QueryException.BROKER_RESOURCE_MISSING_ERROR;
      return tableRoutingContext;
    }

    if (offlineRoutingExists) {
      tableRoutingContext._offlineTableConfig = offlineTableConfig;
    }

    if (realtimeRoutingExists) {
      tableRoutingContext._realtimeTableConfig = realtimeTableConfig;
    }

    if (tableRoutingContext.canRouteHybrid()) {
      TimeBoundaryInfo timeBoundaryInfo = _routingManager.getTimeBoundaryInfo(offlineTableConfig.getTableName());
      if (timeBoundaryInfo == null) {
        LOGGER.debug("No time boundary info found for hybrid table: {}", rawTableName);
      }

      tableRoutingContext._timeBoundaryInfo = timeBoundaryInfo;
    }
  ...
  private static void handleTimestampIndexOverride(PinotQuery pinotQuery, TableCache tableCache,
  ...
  private static boolean isFilterAlwaysFalse(PinotQuery pinotQuery) {
  ...
  private static boolean isFilterAlwaysTrue(PinotQuery pinotQuery) {
    return TRUE.equals(pinotQuery.getFilterExpression());
  }

  private static String getServerTenant(String tableNameWithType, TableCache tableCache) {
   ...
   */
  private static void attachTimeBoundary(PinotQuery pinotQuery, @Nullable TimeBoundaryInfo timeBoundaryInfo,
