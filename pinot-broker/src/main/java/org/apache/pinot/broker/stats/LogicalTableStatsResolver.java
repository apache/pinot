...
public class LogicalTableStatsResolver {
  ...
  public OptionalLong estimateRowsInTimeRange(String tableName, long startMs, long endMs) {
    ...
    if (boundaryMs != null) {
      // A side whose sub-range is empty contributes exactly zero rows, and that holds whether or
      // not the store knows anything about it: no rows can fall in an empty interval. A side whose
      // sub-range is non-empty must be answered from statistics, so an empty optional there means
      // "unknown" and must not be read as zero -- summing it in would report a confident undercount
      // (worst case, a confident 0 for a table the store simply has no rows for), which is a far
      // worse input to the planner than no estimate at all. Both sides exist by construction here,
      // since a time boundary is only defined for a hybrid table.
      ...
      long totalRows = 0;
      if (startMs < offEnd) {
        OptionalLong offlineRows = storeEstimate(offlineTable, startMs, offEnd);
        if (!offlineRows.isPresent()) {
          return OptionalLong.empty();
        }
        totalRows += offlineRows.getAsLong();
      }
      if (rtStart < endMs) {
        OptionalLong realtimeRows = storeEstimate(realtimeTable, rtStart, endMs);
        if (!realtimeRows.isPresent()) {
          return OptionalLong.empty();
        }
        totalRows += realtimeRows.getAsLong();
      }
