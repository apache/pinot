...
/// _CONFIG_OF_MULTI_STAGE_ENGINE_MAX_SERVER_QUERY_THREADS \* numServers / numBrokers_. Note that the config
/// value, number of servers, and number of brokers are all dynamically updated here.
///
...
public class MultiStageQueryThrottler implements ClusterChangeHandler {
  ...
  ///
  /// [#release(int)] should be called after the query is done executing. It is the responsibility of the caller
  /// to ensure that [#release(int)] is called exactly once for each call to this method.
  ///
  ...
  /// _CONFIG_OF_MULTI_STAGE_ENGINE_MAX_SERVER_QUERY_THREADS \* numServers / numBrokers_
  ...
  /// method is called exactly once for each call to [#tryAcquire(int, long, TimeUnit)].
  ...
  int calculateMaxServerQueryThreads() {
    ...
    return Integer.parseInt(_helixAdmin.getConfig(_helixConfigScope,
        List.of(CommonConstants.Helix.CONFIG_OF_MULTI_STAGE_ENGINE_MAX_SERVER_QUERY_THREADS))
