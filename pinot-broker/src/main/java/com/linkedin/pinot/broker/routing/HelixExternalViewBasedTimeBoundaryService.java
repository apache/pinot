...
public class HelixExternalViewBasedTimeBoundaryService implements TimeBoundaryService {
  ...
  public synchronized void updateTimeBoundaryForTable(String tableName) {
    if (_propertyStore == null) {
      return;
    }
    // Do nothing for realtime table.
    ...
    // Choose the min value among all the segments' endtime because it is the safe time boundary to query the offline
    // time. Notice that this is different from the current practice of choosing the max value among the segments' end
    // time. In practice, the choice of min or max may not matter too much because the query processor anyhow subtracts
    // one day from any boundary time returned -- so the max time should be OK although it is not mathematically sound.
    // We could use the Max time to maintain the consistency with the current code.
    long minTimeValue = computeMinSegmentEndTimeForTable(segmentZKMetadataList, tableTimeUnit);
    TimeBoundaryInfo timeBoundaryInfo = new TimeBoundaryInfo();
    timeBoundaryInfo.setTimeColumn(offlineTableConfig.getValidationConfig().getTimeColumnName());

    timeBoundaryInfo.setTimeValue(Long.toString(minTimeValue));
    _timeBoundaryInfoMap.put(tableName, timeBoundaryInfo);

    LOGGER.info("Updated time boundary service for table '{}', minTime: {}", tableName, minTimeValue);
  }

  /**
   * Compute minimum end time across a list of segment zk-metadata.
   *
   * @param segmentZKMetadataList List of Segment zk metadata for which to compute the min end time.
   * @param tableTimeUnit Time Unit for table.
   * @return Min end time across all segments.
   */
  private long computeMinSegmentEndTimeForTable(List<OfflineSegmentZKMetadata> segmentZKMetadataList,
    ...
    for (OfflineSegmentZKMetadata metadata : segmentZKMetadataList) {
      ...
      minTimeValue = Math.min(minTimeValue, endTime);
    }
    return minTimeValue;
  }
