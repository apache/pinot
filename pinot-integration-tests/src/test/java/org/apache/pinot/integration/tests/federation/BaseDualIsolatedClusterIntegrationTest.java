...
public abstract class BaseDualIsolatedClusterIntegrationTest extends ClusterTest {
  ...
  protected void startBroker(ClusterComponents cluster, ClusterComponents secondaryCluster,
    ...
    String secondaryClusterName = CLUSTER_1_NAME.equalsIgnoreCase(config._name) ? CLUSTER_2_NAME : CLUSTER_1_NAME;
  ...
  protected void setupFederationTable() throws Exception {
    createSchemaAndTableOnBothClusters(FEDERATION_TABLE);
  }
  ...
  protected long getCount(String tableName, ClusterComponents cluster, boolean enableFederation) throws Exception {
    String query = "SET enableFederation=" + enableFederation + "; SELECT COUNT(*) as count FROM " + tableName;
