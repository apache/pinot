 ...
 * with the k-way merge on and off. The merge needs both {@code sortedSelectionMergeEnabled} (planner marks the receive
 * node {@code sortedOnSender}) and {@code streamingSortedMailboxReceive} (runtime activates the merge given that
 * marking), so each parity test compares an all-off arm against an all-on arm; see {@link #mergeOptions}.
 *
 * <p>Also asserts that the streaming sorted leaf — the precondition behind the {@code sortedOnSender} marking — is
 * opt-in, appearing in {@code EXPLAIN PLAN FOR} output only when the option is set.
 *
...
public class StreamingSortedMailboxReceiveTest extends CustomDataQueryClusterIntegrationTest {
   ...
   * merge needs BOTH options: {@code sortedSelectionMergeEnabled} is what lets the planner mark the receive node
   * {@code sortedOnSender} (there is no rel-level sort exchange here to declare it), and {@code
   * streamingSortedMailboxReceive} is what turns the merge on given that marking. Setting only the latter is a silent
   * no-op, so the "merge" arm must set both or the test compares the accumulate path against itself.
   */
  ...
  /**
   * Asserts whether any MAILBOX_RECEIVE node in the response {@code stageStats} tree reports {@code kWayMergeUsed}.
   *
   * <p>This is the runtime half of the anchor (see {@link #assertStreamingSortedLeafPlanned} for the planner half),
   ...
   *
   * <p>That is the precondition the planner requires before it marks the receive node {@code sortedOnSender}, which in
   * turn is what activates the k-way merge. The {@code sortedOnSender} flag itself is not rendered by any EXPLAIN mode
   * (the asking-servers explain renders only the leaf stage; the intermediate exchange stage stays a logical
   * {@code PinotLogicalExchange}), so this covers the planner half; {@link #assertKWayMergeUsed} covers the runtime
   * half by reading the {@code kWayMergeUsed} stat out of the response {@code stageStats}. Without both anchors a
