...
public class UpsertTableSegmentPreloadIntegrationTest extends BaseClusterIntegrationTestSet {
  ...
  protected void waitForSnapshotCreation()
    ...
    sendPostRequest(getOrCreateAdminClient().getControllerRequestURLBuilder().forPauseConsumption(rawTableName));
    TestUtils.waitForCondition(aVoid -> {
    ...
    sendPostRequest(getOrCreateAdminClient().getControllerRequestURLBuilder().forResumeConsumption(rawTableName));
