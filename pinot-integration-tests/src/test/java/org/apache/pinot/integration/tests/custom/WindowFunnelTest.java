...
public class WindowFunnelTest extends CustomDataQueryClusterIntegrationTest {
  ...
  public void testFunnelMatchStepWithMultiThreadsReduce(boolean useMultiStageQueryEngine)
      throws Exception {
    setUseMultiStageQueryEngine(useMultiStageQueryEngine);
    int numThreadsForServerFinalReduce = 2 + new Random().nextInt(10);
    LOGGER.info("Running testFunnelMatchStepWithMultiThreadsReduce with numThreadsForServerFinalReduce: {}",
        numThreadsForServerFinalReduce);
    String query =
        String.format("SET numThreadsForServerFinalReduce=" + numThreadsForServerFinalReduce + "; "
            + "SELECT "
            + "userId, funnelMatchStep(timestampCol, '1000', 4, "
            + "url = '/product/search', "
            + "url = '/cart/add', "
