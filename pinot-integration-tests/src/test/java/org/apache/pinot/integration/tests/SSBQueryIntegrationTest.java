...
public class SSBQueryIntegrationTest extends BaseClusterIntegrationTest {
  ...
  private void loadSSBToPinot()
      throws Exception {
    for (String tableResource : SSB_QUICKSTART_TABLE_RESOURCES.values()) {
      URL resourceUrl = getClass().getClassLoader().getResource(tableResource);
      Assert.assertNotNull(resourceUrl);
      File resourceFile = new File(resourceUrl.getFile());
      BootstrapTableTool bootstrapTableTool = new BootstrapTableTool("http", "localhost", _controllerPort,
          resourceFile.getAbsolutePath(), null);
      LOGGER.warn("bootstrap table from " + resourceUrl + " successful!");
      if (!bootstrapTableTool.execute()) {
        throw new RuntimeException("Failed to bootstrap table " + tableResource);
      }
    }
  }

  @SuppressWarnings({"SqlDialectInspection", "SqlNoDataSourceInspection"})
  private void loadSSBToH2()
      throws Exception {
    setUpH2Connection();
    Map<String, String> createTableSqlMap = new HashMap<>();
    // TODO: refactor these out to .sql files
    createTableSqlMap.put("dates",
        "CREATE TABLE dates(D_DATEKEY INT, D_DATE VARCHAR(18), D_DAYOFWEEK VARCHAR(9), D_MONTH VARCHAR(9), "
            + "D_YEAR INT, D_YEARMONTHNUM INT, D_YEARMONTH VARCHAR(7), D_DAYNUMINWEEK INT, D_DAYNUMINMONTH INT, "
    ...
    createTableSqlMap.put("customer",
        "CREATE TABLE customer(C_CUSTKEY INT, C_NAME VARCHAR(25), C_ADDRESS VARCHAR(25), C_CITY VARCHAR(10), "
            + "C_NATION VARCHAR(15), C_REGION VARCHAR(12), C_PHONE VARCHAR(15), C_MKTSEGMENT VARCHAR(10))");
    createTableSqlMap.put("lineorder",
        "CREATE TABLE lineorder(LO_ORDERKEY INT, LO_LINENUMBER INT, LO_CUSTKEY INT, LO_PARTKEY INT,LO_SUPPKEY INT, "
            + "LO_ORDERDATE INT, LO_ORDERPRIORITY VARCHAR(15), LO_SHIPPRIORITY VARCHAR(1), LO_QUANTITY INT, "
            + "LO_EXTENDEDPRICE INT, LO_ORDTOTALPRICE INT, LO_DISCOUNT INT, LO_REVENUE INT, LO_SUPPLYCOST INT, "
            + "LO_TAX INT, LO_COMMITDATE INT, LO_SHIPMODE VARCHAR(10))");
    createTableSqlMap.put("part",
        "CREATE TABLE part(P_PARTKEY INT, P_NAME VARCHAR(22), P_MFGR VARCHAR(6), P_CATEGORY VARCHAR(7), "
            + "P_BRAND1 VARCHAR(9), P_COLOR VARCHAR(11), P_TYPE VARCHAR(25), P_SIZE INT, P_CONTAINER VARCHAR(10))");
    createTableSqlMap.put("supplier",
        "CREATE TABLE supplier(S_SUPPKEY INT,S_NAME VARCHAR(25), S_ADDRESS VARCHAR(25), S_CITY VARCHAR(10), "
            + "S_NATION VARCHAR(15), S_REGION VARCHAR(12), S_PHONE VARCHAR(15))");
    for (Map.Entry<String, String> tableResource : SSB_QUICKSTART_TABLE_RESOURCES.entrySet()) {
      ...
      File resourceFile = new File(resourceUrl.getFile());
      File dataDir = new File(resourceFile.getAbsolutePath(), "rawdata");
      String createTableSql = createTableSqlMap.get(tableName) + " AS SELECT *  FROM CSVREAD('"
          + dataDir.getAbsolutePath() + "/" + tableName + ".csv', null, 'fieldSeparator=|');";
      _h2Connection.prepareCall("DROP TABLE IF EXISTS " + tableName).execute();
      _h2Connection.prepareCall(createTableSql).execute();
      LOGGER.warn("load table from " + resourceUrl + " to H2 successful!");
    }
  }
