...
public abstract class BaseClusterIntegrationTestSet extends BaseClusterIntegrationTest {
  ...
  public String reloadTableAndValidateResponse(String tableName, TableType tableType, boolean forceDownload)
    ...
    String jobStatusResponse = getOrCreateAdminClient().getSegmentClient().getSegmentReloadStatus(jobId);
  ...
  public boolean isReloadJobCompleted(String reloadJobId)
      throws Exception {
    String jobStatusResponse = getOrCreateAdminClient().getSegmentClient().getSegmentReloadStatus(reloadJobId);
    JsonNode jobStatus = JsonUtils.stringToJsonNode(jobStatusResponse);
  ...
  protected String triggerTableRebalance(RebalanceConfig rebalanceConfig, TableType tableType)
    ...
    try {
      ...
      return getOrCreateAdminClient().getTableClient().rebalanceTable(getTableName(), queryParams);
  ...
  protected void waitForTableEVISConverge(String tableName, long timeoutMs) {
    TestUtils.waitForCondition(aVoid -> {
      try {
        String idealStateJson = getOrCreateAdminClient().getTableClient().getIdealState(tableName);
        TableViews.TableView idealState = JsonUtils.stringToObject(idealStateJson, TableViews.TableView.class);

        String externalViewJson = getOrCreateAdminClient().getTableClient().getExternalView(tableName);
        TableViews.TableView externalView = JsonUtils.stringToObject(externalViewJson, TableViews.TableView.class);
  ...
  private void performForceCommitSegmentMovingTest(RebalanceConfig rebalanceConfig, TableConfig tableConfig,
    ...
    if (originalConsumingSegmentsToMove != null && !originalConsumingSegmentsToMove.isEmpty()) {
      response = getOrCreateAdminClient().getTableClient().getConsumingSegmentsInfo(getTableName());
