...
/// End-to-end coverage for `realtimeConfig.consumingSegmentIndexConfig` on a realtime table.
///
/// The table persists `profiledString` as RAW with no inverted index. The consuming segment profile upgrades that
/// column to DICTIONARY + INVERTED only while the segment is mutable and consuming. After force-commit, the immutable
/// segment must retain the persisted RAW/no-inverted shape on disk.
@Test(suiteName = "CustomClusterIntegrationTest")
public class ConsumingSegmentIndexProfileRealtimeTest extends CustomDataQueryClusterIntegrationTest {
  private static final String TABLE_NAME = "ConsumingSegmentIndexProfileRealtimeTest";
  ...
  protected TableConfig createRealtimeTableConfig(File sampleAvroFile) {
    ...
    RealtimeConfig realtimeConfig = new RealtimeConfig(new ConsumingSegmentIndexConfig(List.of(
        new ConsumingSegmentFieldConfig(PROFILED_STRING_COLUMN, FieldConfig.EncodingType.DICTIONARY, indexes))));
  ...
  @Test
  public void testConsumingSegmentIndexProfileEndToEnd()
      throws Exception {
    ...
    assertEquals(alphaCountAfterCommit, alphaCountBeforeCommit,
        "Consuming profile must not change query semantics after commit");
