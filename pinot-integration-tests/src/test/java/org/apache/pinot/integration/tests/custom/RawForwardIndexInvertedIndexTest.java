...
public class RawForwardIndexInvertedIndexTest extends CustomDataQueryClusterIntegrationTest {
  ...
  public void testSegmentMetadataForRawForwardColumn()
    ...
    List<String> noDictionaryColumns = offlineTableConfig.getIndexingConfig().getNoDictionaryColumns();
    assertNotNull(noDictionaryColumns, "No-dictionary columns should be configured to keep forward index raw");
    assertTrue(noDictionaryColumns.contains(RAW_DIMENSION),
        "Forward index should remain raw via no-dictionary configuration");
  }
}
