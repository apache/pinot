        ...
        while ((line = reader.readLine()) != null) {
          ...
          for (int i = 0; i < fields.length; i++) {
            ...
            switch (fieldType) {
              case LONG_TYPE:
                if (isMultiValue) {
                  record.put(fieldName, List.of(Long.parseLong(fields[i])));
                } else {
              ...
              case DOUBLE_TYPE:
                if (isMultiValue) {
                  record.put(fieldName, List.of(Double.parseDouble(fields[i])));
                } else {
              ...
              case STRING_TYPE:
                if (isMultiValue) {
                  record.put(fieldName, Collections.singletonList(fields[i]));
                } else {
