...
//    waitForAllDocsLoaded(600_000L);
    client_ = new ServerGrpcQueryClient("localhost", 7100);
  }
  ServerGrpcQueryClient client_;
  public ServerGrpcQueryClient getGrpcQueryClient() {
//    return new ServerGrpcQueryClient("localhost", 7100);
    return client_;
  }

  public static String buildRandomStringQuery(int n) {
    if (n <= 0) {
    ...
    for (int i = 1; i <= n; i++) {
      cols.append("randomString(10000000) as s").append(i);
      if (i < n) {
        cols.append(", ");
      }
    }
  ...
  public void testGrpcQueryServer()
    ...
    while (true) {
      ...
      for (Future<Void> future : futures) {
        try {
          future.get();  // blocks and rethrows any exceptions
  ...
  private void sendQuery() throws Exception {
      ...
      String sql = "SELECT randomString(id) FROM githubComplexTypeEvents_OFFLINE LIMIT 500 OPTION(timeoutMs=800000)";
      List<String> segments = List.of("githubComplexTypeEvents_OFFLINE_1514804400000_1514805700000_0");

      ServerGrpcRequestBuilder requestBuilder = new ServerGrpcRequestBuilder();
      requestBuilder
          .setSegments(segments);
      requestBuilder.setEnableStreaming(true);
    try {
      Iterator<Server.ServerResponse> responses = queryClient.submit(requestBuilder.setSql(sql).build());
      int numOfRows = 0;
      boolean sleeped = false;
      while (responses.hasNext()) {
        Server.ServerResponse streamingResponse = responses.next();
        DataTable dataTable = DataTableFactory.getDataTable(streamingResponse.getPayload().asReadOnlyByteBuffer());
        if (!sleeped) {
          sleeped = true;
        }
        numOfRows += dataTable.getNumberOfRows();
      }
//    DataTable dataTable = collectNonStreamingRequestResult(responses);
//    testNonStreamingRequest(queryClient.submit(requestBuilder.setSql(sql).build()));
//    System.out.println(dataTable.getDataSchema());

      System.out.println("row in total: " + numOfRows);
    } finally {
//      queryClient.close();
    }
  }
