...
public class SparkSegmentMetadataPushIntegrationTest extends BaseClusterIntegrationTest {
  ...
  private void runMetadataPushWithoutConsistentPushTest(boolean batchSegmentUpload)
    ...
    tableSpec.setTableConfigURI(
        getOrCreateAdminClient().getControllerRequestURLBuilder().forUpdateTableConfig(_testTableWithType));
  ...
  private long getNumDocs(String segmentName)
    ...
    return JsonUtils.stringToJsonNode(
            sendGetRequest(
                getOrCreateAdminClient().getControllerRequestURLBuilder().forSegmentMetadata(_testTable, segmentName)))
