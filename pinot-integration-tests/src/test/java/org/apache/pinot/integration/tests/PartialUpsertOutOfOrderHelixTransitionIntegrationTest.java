...
public class PartialUpsertOutOfOrderHelixTransitionIntegrationTest extends BaseClusterIntegrationTest {
  ...
  /**
   * Waits until the expected player 101 score appears on the enforced table. With enforcement,
   * Server 0 serializes segments 1→2→3 after the consuming delay, so we poll until the correct
   * score propagates rather than relying on a fixed sleep.
   */
  private void waitForCorrectEnforcedScores(long timeoutMs) {
    TestUtils.waitForCondition(aVoid -> {
      try {
        float score = getPlayerScore(TABLE_NAME, 101);
        return Math.abs(score - EXPECTED_SCORE_101) < 1.0f;
      } catch (Exception e) {
        return null;
      }
    }, timeoutMs, "Timed out waiting for enforced table player 101 score to reach "
        + EXPECTED_SCORE_101 + " (enforcement serializes segment processing on Server 0)");
  }
  ...
  public void testOutOfOrderTransitionWithEnforcedOrdering()
    ...
    // Verify correct number of distinct records (deduplication working)
    ResultSet countResult = getPinotConnection().execute(
        "SELECT COUNT(*) FROM " + TABLE_NAME).getResultSet(0);
    assertEquals(countResult.getLong(0), DISTINCT_KEYS,
        "Expected " + DISTINCT_KEYS + " distinct records after upsert");

    // Query individual player scores to verify INCREMENT merges happened correctly.
    // With enforcement, both servers produce correct results regardless of the delay.
    assertPlayerScore(TABLE_NAME, 102, EXPECTED_SCORE_102,
        "Player 102 score should be 70 (30 + 40)");
    assertPlayerScore(TABLE_NAME, 100, EXPECTED_SCORE_100,
        "Player 100 score should be 120 (10 + 50 + 60)");
    assertPlayerScore(TABLE_NAME, 101, EXPECTED_SCORE_101,
        "Player 101 score should be 1020 "
            + "(20 + 100 + 200 + 300 + 400)");
  ...
  public void testOutOfOrderTransitionWithoutEnforcedOrdering()
    ...
    // Verify correct number of distinct records (deduplication working)
    ResultSet countResult = getPinotConnection().execute(
        "SELECT COUNT(*) FROM " + TABLE_NAME_NO_ENFORCE).getResultSet(0);
    assertEquals(countResult.getLong(0), DISTINCT_KEYS,
        "Expected " + DISTINCT_KEYS + " distinct records after upsert");

    // Players 100 and 102 are unaffected by the out-of-order transition because segment 1 only
    // contains player 101 records. Their scores should be correct on both servers.
    assertPlayerScore(TABLE_NAME_NO_ENFORCE, 102, EXPECTED_SCORE_102,
        "Player 102 score should be 70 (30 + 40)");
    assertPlayerScore(TABLE_NAME_NO_ENFORCE, 100, EXPECTED_SCORE_100,
        "Player 100 score should be 120 (10 + 50 + 60)");

    // Player 101 may show correct or degraded score depending on broker routing:
    // - Server 1 (correct): 20 + 100 + 200 + 300 + 400 = 1020
    // - Server 0 (degraded): 20 + 400 = 420 (missing segment 1's +100 +200 +300)
    float score101 = getPlayerScore(TABLE_NAME_NO_ENFORCE, 101);
    assertTrue(
        Math.abs(score101 - EXPECTED_SCORE_101) < 1.0f
            || Math.abs(score101 - DEGRADED_SCORE_101) < 1.0f,
        "Player 101 score should be either 1020 (correct, routed to "
            + "Server 1) or 420 (degraded, routed to Server 0 where "
            + "segment 1 data was missing), got: " + score101);
  ...
  private void assertPlayerScore(String tableName, int playerId,
      float expectedScore, String message) {
    ResultSet result = getPinotConnection().execute(
        "SELECT score FROM " + tableName
            + " WHERE playerId = " + playerId).getResultSet(0);
    assertEquals(result.getRowCount(), 1);
    float actualScore = result.getFloat(0, 0);
    assertTrue(Math.abs(actualScore - expectedScore) < 1.0f,
        message + ", got: " + actualScore);
  }

  private float getPlayerScore(String tableName, int playerId) {
    ResultSet result = getPinotConnection().execute(
        "SELECT score FROM " + tableName
            + " WHERE playerId = " + playerId).getResultSet(0);
    assertEquals(result.getRowCount(), 1);
