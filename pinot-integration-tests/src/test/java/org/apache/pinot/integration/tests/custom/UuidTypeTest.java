...
public class UuidTypeTest extends CustomDataQueryClusterIntegrationTest {
  ...
  ///
  /// UUID byte order determines sort order: the last segment of each test UUID differs only in the final byte
  /// (00–03), so the four distinct values have a well-defined byte-ordered range.
  ///
  /// Only the single-stage engine is tested here; MSQE range pushdown for UUID columns is verified separately
  /// through the SSE/MSE parity checks in [#testSseMseParity].
  @Test
