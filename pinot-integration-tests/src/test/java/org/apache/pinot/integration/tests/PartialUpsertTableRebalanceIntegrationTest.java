...
public class PartialUpsertTableRebalanceIntegrationTest extends BaseClusterIntegrationTest {
   ...
   * <ul>
   *   <li>IdealState has {@code expectedSegments} entries.</li>
   *   <li>No segment in IdealState has any instance in OFFLINE state.</li>
   *   <li>ExternalView equals IdealState (queries route via ExternalView, so this is required
   *       before asserting on IdealState).</li>
   * </ul>
   * All conditions must hold for {@code stableSamples} consecutive polls, because IdealState can
   * transiently look correct between two intermediate states during force-commit + new-consuming-segment
   * creation.
   */
