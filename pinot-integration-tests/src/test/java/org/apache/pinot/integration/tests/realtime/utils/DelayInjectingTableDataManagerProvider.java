...
public class DelayInjectingTableDataManagerProvider implements TableDataManagerProvider {
  public static final String DELAY_CONFIG_KEY = "delay.config";
  ...
  public TableDataManager getTableDataManager(TableConfig tableConfig,
    ...
    switch (tableConfig.getTableType()) {
      ...
      case REALTIME:
        ...
        String delayConfigKey =
            DELAY_CONFIG_KEY + "." + TableNameBuilder.extractRawTableName(tableConfig.getTableName());
        DelayInjectingTableConfig delayInjectingTableConfig = null;
        if (_pinotConfiguration.containsKey(delayConfigKey)) {
          try {
            delayInjectingTableConfig =
                new ObjectMapper().readValue(_pinotConfiguration.getProperty(delayConfigKey),
                    DelayInjectingTableConfig.class);
          } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
          }
        }
        tableDataManager =
            new DelayInjectingRealtimeTableDataManager(_segmentBuildSemaphore, isServerReadyToServeQueries,
                delayInjectingTableConfig);
        break;
      default:
