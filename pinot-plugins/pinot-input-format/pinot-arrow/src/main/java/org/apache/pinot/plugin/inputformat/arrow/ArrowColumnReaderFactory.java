...
/// [ColumnReaderFactory] backed by a caller-managed [ArrowReader].
///
/// The caller supplies an already-open [ArrowReader] (any subclass — `ArrowFileReader` , `ArrowStreamReader` , or a
/// custom subclass that yields in-process record
/// batches) plus the [BufferAllocator] the reader was opened against. Per-column accumulator
/// vectors are allocated against that allocator and concatenated from the reader's batches via the
/// shared [ArrowAccumulators] helper, then exposed as one [ArrowColumnReader] per
/// accumulator.
///
/// **Ownership:** the supplied [ArrowReader] and [BufferAllocator] are owned by
/// the caller and are NOT closed by this factory. Per-column accumulator vectors created during
/// [#init] ARE owned by the factory and are released on [#close]. For the file-backed
/// convenience that opens and owns its own reader and allocator, see [ArrowFileColumnReaderFactory].
///
/// This class is not thread-safe.
///
/// `@SuppressWarnings("serial")`: [ColumnReaderFactory] is [java.io.Serializable]
...
public class ArrowColumnReaderFactory implements ColumnReaderFactory {
  ...
  ///
  /// @param reader Caller-managed Arrow reader (e.g. `ArrowStreamReader` over a byte buffer,
  ...
  /// non-virtual columns from `targetSchema` that the Arrow source actually contains"; pass a
  /// non-empty set to restrict to a subset. Allocator sizing is the caller's responsibility for this
  /// factory; the only honored `configs` key is [#CONFIG_EXTRACT_RAW_TIME_VALUES].
  @Override
