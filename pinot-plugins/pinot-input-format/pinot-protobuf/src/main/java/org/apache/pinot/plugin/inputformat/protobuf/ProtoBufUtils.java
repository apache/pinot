...
public class ProtoBufUtils {
  ...
  /// On success the caller is responsible for managing the lifetime of the returned file and its parent directory;
  /// on copy failure the temp directory is removed (the copy may have left partial content or filesystem sidecars
  /// such as Hadoop `.crc` files). This is a plain download with no outage fallback (a jar cannot be validated as
  /// a descriptor set); descriptors should be resolved through [#getDescriptor(String, String, boolean)] instead.
  ...
  ///
  /// The descriptor set is read fresh on every call via [#openDescriptorFile(String)] — no temp files — so
  /// in-place updates of the file keep propagating. When `fallbackToLastKnownGood` is set, the last remote content
  /// that both fetched and resolved successfully is remembered per URI and served ONLY when the fetch itself fails
  /// (e.g. a transient DNS or object-store outage), so decoder creation survives outages instead of permanently
  /// marking the CONSUMING segment ERROR. Content that fetches successfully but does not resolve (corrupt, empty,
  /// or missing the requested message type) always fails the call and leaves the remembered copy untouched: a bad
