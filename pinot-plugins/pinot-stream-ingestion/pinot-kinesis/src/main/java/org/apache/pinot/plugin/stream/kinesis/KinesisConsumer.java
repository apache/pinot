...
public class KinesisConsumer extends KinesisConnectionHandler implements PartitionGroupConsumer {
  ...
  public KinesisMessageBatch fetchMessages(StreamPartitionMsgOffset startMsgOffset, int timeoutMs) {
    ...
    try {
      ...
      // TODO: Consider caching the next shard iterator to avoid the overhead of fetching it every time
      GetShardIteratorRequest getShardIteratorRequest =
          GetShardIteratorRequest.builder().streamName(_config.getStreamTopicName()).shardId(shardId)
              ...
              .build();
      String shardIterator = _kinesisClient.getShardIterator(getShardIteratorRequest).shardIterator();
      ...
      throw new PermanentConsumerException(e);
    }
    List<Record> records = getRecordsResponse.records();
    List<BytesStreamMessage> messages = new ArrayList<>(records.size());
    if (!records.isEmpty()) {
    ...
    // Calculate offset of next batch
    ...
    boolean isEndOfShard = getRecordsResponse.hasChildShards() && !getRecordsResponse.childShards().isEmpty();

    return new KinesisMessageBatch(messages, offsetOfNextBatch, isEndOfShard);
  }
  ...
  private BytesStreamMessage extractStreamMessage(Record record, String shardId) {
    ...
    if (_config.isPopulateMetadata()) {
      builder.setMetadata(
          ImmutableMap.of(KinesisStreamMessageMetadata.APPRX_ARRIVAL_TIMESTAMP_KEY, String.valueOf(timestamp),
