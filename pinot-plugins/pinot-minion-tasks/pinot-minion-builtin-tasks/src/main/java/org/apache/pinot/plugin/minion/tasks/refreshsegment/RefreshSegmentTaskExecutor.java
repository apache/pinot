...
public class RefreshSegmentTaskExecutor extends BaseSingleSegmentConversionExecutor {
  ...
  protected SegmentConversionResult convert(PinotTaskConfig pinotTaskConfig, File indexDir, File workingDir)
    ...
    // Transform BACKFILL/UPDATE must not drive record-replay rebuild: ExpressionTransformer does not overwrite existing
    // derived values, and SegmentIndexCreationDriverImpl then stamps autoGenerated=false and freezes the column.
    // Server SegmentPreProcessor + DefaultColumnHandler is the apply path.
    boolean needPreprocess = ImmutableSegmentLoader.needPreprocess(segmentDirectory, indexLoadingConfig, false);
