...
public class UpsertCompactionTaskGenerator extends BaseTaskGenerator {
  ...
  static boolean validate(TableConfig tableConfig, String taskType) {
    ...
    if (!compactionConfigs.containsKey(UpsertCompactionTask.MAX_NUM_RECORDS_PER_SEGMENT))
    {
      LOGGER.warn("Skip generation task: {} for table: {}, unable to find {} key in compaction task config",
          taskType, tableNameWithType, UpsertCompactionTask.MAX_NUM_RECORDS_PER_SEGMENT);
      return false;
    }
