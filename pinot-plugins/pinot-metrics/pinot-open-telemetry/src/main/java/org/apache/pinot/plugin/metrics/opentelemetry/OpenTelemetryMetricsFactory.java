...
public class OpenTelemetryMetricsFactory implements PinotMetricsFactory {
  ...
  public PinotMetricName makePinotMetricName(Class<?> klass, String name) {
    // It's OK to just throw away the class name here. Adding the class name to into pinot metric name will make
    // the dimension/attribute parsing logic more complicated as not all metric having it.
  ...
  public <T> PinotGauge<T> makePinotGauge(Function<Void, T> condition) {
    return new OpenTelemetryGauge(new OpenTelemetryMetricName("none"), condition);
  }

  @Override
  public <T> PinotGauge<T> makePinotGauge(String metricName, Function<Void, T> condition) {
    return new OpenTelemetryGauge(new OpenTelemetryMetricName(metricName), condition);
  }
