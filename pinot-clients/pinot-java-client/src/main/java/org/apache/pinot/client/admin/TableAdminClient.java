...
public class TableAdminClient extends BaseServiceAdminClient {
  ...
  public TableView getTableViewObject(String tableName, String view, @Nullable String tableType)
    ...
    JsonNode response = _transport.executeGet(_controllerAddress, path, queryParams, _headers);
    try {
      return JsonUtils.jsonNodeToObject(response, TableView.class);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize table view for table: " + tableName, e);
    }
  }
  ...
  public String createTable(String tableConfig, @Nullable String validationTypesToSkip,
    ...
    JsonNode response =
        _transport.executePost(_controllerAddress, "/tables", tableConfig, queryParams, mergeHeaders(headers));
    return response.toString();
  }
  ...
  public String getInstancePartitions(String tableName, @Nullable String instancePartitionsType)
    ...
    JsonNode response = _transport.executeGet(_controllerAddress, "/tables/" + tableName + "/instancePartitions",
        queryParams.isEmpty() ? null : queryParams, _headers);
    return response.toString();
  }
  ...
  public String assignInstances(String tableName, @Nullable String instancePartitionsType, boolean dryRun)
    ...
    JsonNode response = _transport.executePost(_controllerAddress, "/tables/" + tableName + "/assignInstances", null,
        queryParams.isEmpty() ? null : queryParams, _headers);
    return response.toString();
  }
  ...
  public String deleteInstancePartitions(String tableName, @Nullable String instancePartitionsType)
    ...
    JsonNode response = _transport.executeDelete(_controllerAddress, "/tables/" + tableName + "/instancePartitions",
        queryParams.isEmpty() ? null : queryParams, _headers);
    return response.toString();
  }
  ...
  public String replaceInstance(String tableName, @Nullable String instancePartitionsType, String oldInstanceId,
    ...
    JsonNode response = _transport.executePost(_controllerAddress, "/tables/" + tableName + "/replaceInstance", null,
        queryParams, _headers);
    return response.toString();
  }
  ...
  public String updateInstancePartitions(String tableName, String instancePartitionsJson)
    ...
    JsonNode response = _transport.executePut(_controllerAddress, "/tables/" + tableName + "/instancePartitions",
        instancePartitionsJson, null, _headers);
    return response.toString();
  }
  ...
  public TableSizeInfo getTableSize(String tableName)
    ...
    try {
      return JsonUtils.jsonNodeToObject(response, TableSizeInfo.class);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize table size for table: " + tableName, e);
    }
  }
  ...
  public TableMetadataInfo getTableMetadata(String tableName)
    ...
    try {
      return JsonUtils.jsonNodeToObject(response, TableMetadataInfo.class);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize table metadata for table: " + tableName, e);
    }
  }
  ...
  public TableIndexMetadataResponse getTableIndexes(String tableName)
    ...
    try {
      return JsonUtils.jsonNodeToObject(response, TableIndexMetadataResponse.class);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize table indexes for table: " + tableName, e);
    }
  }
  ...
  public TableMetadataInfo getAggregateMetadata(String tableName, @Nullable String columns)
    ...
    try {
      ...
      throw new PinotAdminException("Failed to deserialize aggregate metadata for table: " + tableName, e);
    }
  }

  private String buildAggregateMetadataPath(String tableName, @Nullable String columns) {
    String rawTableName = TableNameBuilder.extractRawTableName(tableName);
    TableType tableType = TableNameBuilder.getTableTypeFromTableName(tableName);

    List<String> queryParams = new ArrayList<>();
    if (tableType != null) {
      queryParams.add("type=" + tableType.name());
    }
    if (StringUtils.isNotBlank(columns)) {
      for (String column : StringUtils.split(columns, ',')) {
        String trimmedColumn = StringUtils.trimToNull(column);
        if (trimmedColumn != null) {
          queryParams.add("columns=" + URLEncoder.encode(trimmedColumn, StandardCharsets.UTF_8));
        }
      }
    }

    String path = "/tables/" + rawTableName + "/metadata";
  ...
  public PauseStatusDetails getPauseStatusDetails(String tableName)
    ...
    JsonNode response = _transport.executeGet(_controllerAddress, "/tables/" + tableName + "/pauseStatus",
        null, _headers);
    try {
      return JsonUtils.jsonNodeToObject(response, PauseStatusDetails.class);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize pause status for table: " + tableName, e);
    }
  }
  ...
  public String getForceCommitJobStatus(String jobId)
      throws PinotAdminException {
    JsonNode response =
        _transport.executeGet(_controllerAddress, "/tables/forceCommitStatus/" + jobId, null, _headers);
    return response.toString();
  }
  ...
  public TableView getExternalViewObject(String tableResourceName)
    ...
    try {
      ...
      throw new PinotAdminException("Failed to deserialize external view for table: " + tableResourceName, e);
    }
  }
  ...
  public TableView getIdealStateObject(String tableResourceName)
    ...
    try {
      ...
      throw new PinotAdminException("Failed to deserialize ideal state for table: " + tableResourceName, e);
    }
  }
  ...
  public <T> T getConsumingSegmentsInfo(String tableName, Class<T> responseType)
    ...
    JsonNode response = _transport.executeGet(_controllerAddress, "/tables/" + tableName + "/consumingSegmentsInfo",
        null, _headers);
    try {
      return JsonUtils.jsonNodeToObject(response, responseType);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize consuming segments info for table: " + tableName, e);
    }
  }
  ...
  public String validateTableConfigs(String tableConfigsJson, @Nullable String validationTypesToSkip,
    ...
    JsonNode response =
        _transport.executePost(_controllerAddress, "/tableConfigs/validate", tableConfigsJson, queryParams,
