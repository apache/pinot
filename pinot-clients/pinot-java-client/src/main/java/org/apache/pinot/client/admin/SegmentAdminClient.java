...
public class SegmentAdminClient extends BaseServiceAdminClient {
  ...
  public Map<String, Object> getSegmentMetadata(String tableName, String segmentName,
    ...
    JsonNode response = _transport.executeGet(_controllerAddress,
        buildSegmentMetadataPath(tableName, segmentName, columns), null, _headers);
  ...
  public byte[] downloadSegment(String tableName, String segmentName)
    ...
    String path = "/segments/" + rawTableName + "/" + encodePath(segmentName);
  ...
  public String getSegmentsMetadata(String tableName, List<String> columns, List<String> segments, String tableType)
      throws PinotAdminException {
    JsonNode response =
        _transport.executeGet(_controllerAddress, buildSegmentsMetadataPath(tableName, columns, segments, tableType),
            null, _headers);
    return response.toString();
  }
  ...
  public TableSegmentsReloadCheckResponse checkIfReloadIsNeeded(String tableNameWithType, boolean verbose)
    ...
    JsonNode response = _transport.executeGet(_controllerAddress, "/segments/" + tableNameWithType + "/needReload",
        queryParams, _headers);
    try {
      return JsonUtils.jsonNodeToObject(response, TableSegmentsReloadCheckResponse.class);
    } catch (IOException e) {
      throw new PinotAdminException(
          "Failed to deserialize reload check response for table: " + tableNameWithType, e);
    }
  }
  ...
  public PinotTableReloadStatusResponse getSegmentReloadStatusObject(String jobId)
    ...
    JsonNode response =
        _transport.executeGet(_controllerAddress, "/segments/segmentReloadStatus/" + jobId, null, _headers);
    try {
      return JsonUtils.jsonNodeToObject(response, PinotTableReloadStatusResponse.class);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize segment reload status for job: " + jobId, e);
    }
  }
  ...
  public TableTierInfo getStorageTiers(String tableName)
    ...
    try {
      return JsonUtils.jsonNodeToObject(response, TableTierInfo.class);
    } catch (IOException e) {
      throw new PinotAdminException("Failed to deserialize storage tiers for table: " + tableName, e);
    }
  }
  ...
  public TableTierInfo getSegmentStorageTiers(String tableName, String segmentName, String tableType)
    ...
    try {
      ...
      throw new PinotAdminException(
          "Failed to deserialize storage tiers for segment: " + segmentName + " in table: " + tableName, e);
    }
  }
  ...
  private static String buildSegmentMetadataPath(String tableName, String segmentName, @Nullable List<String> columns) {
    String columnsParam = UrlBuilderUtils.generateColumnsParam(columns);
    String path = "/segments/" + tableName + "/" + encodePath(segmentName) + "/metadata";
    return columnsParam == null ? path : path + "?" + columnsParam;
  }

  private static String buildSegmentsMetadataPath(String tableName, @Nullable List<String> columns,
      @Nullable List<String> segments, @Nullable String tableType) {
    StringBuilder path = new StringBuilder("/segments/").append(tableName).append("/metadata");
    boolean hasQuery = appendQueryParam(path, "type", tableType);
    String columnsAndSegmentsParam = UrlBuilderUtils.generateColumnsAndSegmentsParam(columns, segments);
    if (columnsAndSegmentsParam != null) {
      path.append(hasQuery ? "&" : "?").append(columnsAndSegmentsParam);
    }
    return path.toString();
  }

  private static boolean appendQueryParam(StringBuilder path, String key, @Nullable String value) {
    if (value == null) {
      return false;
    }
    path.append("?").append(URLEncoder.encode(key, StandardCharsets.UTF_8)).append("=")
