...
public class TableNameExtractor {
  ...
  public static String[] resolveTableName(String query) {
    ...
    try {
      Set<String> tableNames = extractTableNamesFromMultiStageQuery(sqlNodeAndOptions.getSqlNode());
      if (tableNames != null) {
        return tableNames.toArray(new String[0]);
      }
    } catch (Exception e) {
      LOGGER.error("Cannot extract table name from query: {}. Fallback to broker selector default.", query, e);
    }
    return null;
  }
   ...
   */
  private static Set<String> extractTableNamesFromMultiStageQuery(SqlNode sqlNode) {
  ...
  private void visitNodeOperands(SqlNode node) {
    try {
      ...
      LOGGER.debug("Exception encountered while visiting node operands: {}", node, e);
    }
  }
}
