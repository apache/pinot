...
/// Benchmarks UUID grouping and lookup hot paths using Pinot's current [ByteArray]-based representation versus a
/// two-long UUID key representation and [UUID].
///
/// The benchmark is intentionally engine-adjacent:
///
/// - V1 grouping uses the same `Object2IntOpenHashMap<ByteArray>` shape as no-dictionary UUID group-by.
/// - V2 grouping uses [OneUuidKeyGroupIdGenerator].
/// - Lookup uses [UuidLookupTable], which is the UUID-specific path used by MSE hash join.
