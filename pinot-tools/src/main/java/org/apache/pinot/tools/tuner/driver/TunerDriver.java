...
TunerDriver is an executable interface, has three pluggable modules:
    ...
    Strategy, which has four user defined functions operating on a map of Map<Long, Map<String, Map<String, BasicMergerObj>>>:
                                                                                |		       |				    |				  |
        ...
        Accumulator: A function to process BasicQueryStats and MetaDataProperties; then accumulate stats to corresponding BasicMergerObj entry.
        Merger: A function to merge two BasicMergerObj entries having the same TableName/ColumnName from different threads.
