...
public class RebalanceCommand extends AbstractBaseAdminCommand implements Command {
  ...
  private int _minReplicasForNoDowntime;
  ...
  public boolean execute()
    ...
    final PinotTableRebalancer tableRebalancer =
        new PinotTableRebalancer(_zkAddress, _clusterName, _dryRun, _noDowntime, _includeConsuming,
            _minReplicasForNoDowntime);
  ...
  public String description() {
    final String description =
        "Rebalances segments of a table. " + "By default, rebalancing is done with downtime where there is no guarantee"
            + "of any existing serving replica being alive for serving queries. "
            + "Use the -nodowntime option to rebalance without downtime. It takes"
            + "care of keeping at least 1 serving replica alive while rebalancing. "
            + "For better QPS, use the -minReplicasForNoDowntime <val> option along with"
            + "-nodowntime to ensure that rebalancer keeps at least the required number of "
            + "replicas alive for each segment while segments are being moved between hosts";
    return description;
  }
