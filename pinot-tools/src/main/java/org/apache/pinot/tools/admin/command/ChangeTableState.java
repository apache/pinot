...
public class ChangeTableState extends AbstractDatabaseBaseAdminCommand implements Command {
  ...
  @CommandLine.Option(names = {"-tableType"}, required = false,
      description = "Table type (OFFLINE|REALTIME). Optional if tableName already has a type suffix.")
  ...
  public boolean execute()
    ...
    try (PinotAdminClient adminClient = getPinotAdminClient()) {
      switch (stateValue) {
        case "enable":
          adminClient.getTableClient().setTableState(_tableName, tableType, true);
          break;
        case "disable":
          adminClient.getTableClient().setTableState(_tableName, tableType, false);
          break;
        case "drop":
          adminClient.getTableClient().deleteTable(_tableName, tableType, null, null);
          break;
        default:
