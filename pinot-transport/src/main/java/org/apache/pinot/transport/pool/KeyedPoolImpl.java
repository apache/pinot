...
public class KeyedPoolImpl<T> implements KeyedPool<T> {
  ...
  public ServerResponseFuture<T> checkoutObject(ServerInstance key, String context) {
    ...
    AsyncResponseFuture<T>
        future = new AsyncResponseFuture<T>(key, "ConnPool checkout future for key " + key + "(" + context + ")");
  ...
  public ServerResponseFuture<NoneType> shutdown() {
    synchronized (_mutex) {
      ...
      _shutdownFuture = new CompositeFuture<NoneType>("Shutdown For Pool", CompositeFuture.GatherModeOnError.AND);
